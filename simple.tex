...
The deliberated choice of $i$ given the exposure protocol $(I, \allargs, {\allleadsto})$ is denoted $p_i \in P$, with the semantics that $p_i$ represents the preference of $i$ over the proposed set of two acts, after deliberation, given the exposure protocol. In other words, if $p_i = \phi$, the individual considers the first act strictly better than the second one, after deliberation (and conversely for $p_i = ¬\phi$); and if $p_i = 0$, the deliberated choice of $i$ considers neither no act as strictly better than the alternative. This latter possibility covers both the situation of indifference between both options (in the sense of being evaluated by $i$ as being exactly equally worth), which will be further developed in \cref{sec:multiple}, or the situation that no act is deliberately strictly better than the other one and that they are also not deliberately exactly equivalent to each other.
\footnote{It should be clear that $P$ as used as the codomain of ${\leadsto}$ and as the set of possible values of $p_i$ denote different entities. The letter is reused simply to reduce notations. The value ${\leadsto}(i, \alpha) \in P$ designates the directly observed choice of $i$ whereas the value $p_i \in P$ designates the non directly observable deliberated choice of $i$%; furthermore, $0$ in particular may acquire a very different meaning as the value ${\leadsto}(i, \alpha)$ than as the value $p_i = 0$
.}
...
The formal definition of $p_i$ given $(I, \allargs, {\allleadsto})$ requires to define the notion of decisive argument.
...
The deliberated judgment of $i$ is then considered as the set of propositions supported by decisive arguments.
\begin{axiom}[Decisive arguments define deliberated judgments]
	\label{ax:decisive}
	$\forall i \in I, p \in P: 
		p \in P_i ⇔ [\exists \ar \in \allargs \suchthat \ar \ileadstost \prop].$
\end{axiom}
...
\label{sec:static}
\Cref{ex:tutor} suggests a simple form for theories of deliberated choice: suffice to determine an argument $\ar \in \allargs$ and an alternative $\prop \in \Phi$ such that the theory claims that $\forall i \in I: \ar \ileadstost \prop$ and that $\prop \in \iprops$.
...
The following example illustrates this same point in a simpler setting which permits a complete definition of $\allargs$ and $\leadsto$.%, which permits defining the setting completely at the price of a reduced realism.
