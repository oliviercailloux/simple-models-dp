...
Given $\phi \in {\gleadsto}(\allargs)$, define ${\ibeatse[\phi]} \subseteq \allargs × \gargs[\phi]$ such that $\ar' \ibeatse[\phi] \ar$ iff $(\ar', \ar) \ileadstoe ¬\phi$. 
...
\begin{definition}[Bounded length]
	$\forall n \in \N$, if $n > b_l$, there is no sequence of arguments $(\ar_k)_{0 ≤ k < n}$ such that $[\exists \phi \suchthat \forall (0 ≤ k < 2n): \ar_{2k} \in \gargs[\phi]]$ and $(\ar_{k + 1}, \ar_k) \ileadstoe \phi$ and $(\ar_{k + 1}, \ar_k) \ileadstoe ¬\phi$.
\end{definition}
...
\begin{theorem}
	If a theory is answerable, satisfies reinstatement, has bounded width and length, and is operationally valid, it is valid.
\end{theorem}
\begin{proof}
	...
	Fix $\phi \in {\gleadsto}(\allargs)$. For all $k \in \N$, define $\gargs[k] = [\gbeatsinv]^{2k}({\gleadstoinv}(\phi))$.
	Note that $\ar \in \gargs[\phi] ⇔ \exists k \in \N \suchthat \ar \in \gargs[k]$.
	
	Lemma: $\gargs[k+2]$ $\phi$-defends $\gargs[k]$. To prove this, fix $\ar_1 \in \allargs, \ar_0 \in \gargs[k] \suchthat (\ar_1, \ar_0) \ileadstoe ¬\phi$. We need to exhibit $\ar_2 \in \gargs[k+2] \suchthat (\ar_2, \ar_1) \ileadstoe \phi$. Indeed, we know that $\ar_1 \ibeatse[\phi] \ar_0$, and we can use Operational validity as $\ar_0 \in \gargs[\phi]$, thus, $\ar_1 \gbeats \ar_0$ and $\exists \ar_2 \in {\gbeatsinv}(\ar_1) \suchthat (\ar_2, \ar_1) \ileadstoe \phi$.
	
	\commentOC{I suppose that, therefore, $\gargs$ $\phi$-defends itself!}
	
	Pick any even $b > b_l$. Lemma: $\gargs[b] = \emptyset$, and $\gargs[b]$ defends $\gargs[b-2]$, thus, $\gargs[b-2]$ has no attacker.
	
	Th: $\gargs[4]$ decisively defends $\gargs[2]$ (easy if $j = 4$).
	Then: if $\gargs[4]$ decisively defends $\gargs[2]$, then anything $\gargs[2]$ defends, it decisively defends.
\end{proof}
