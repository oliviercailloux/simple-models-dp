...
with ${\ileadstost[i]} \subseteq (\allargs × \allargs) × \Phi$, ${\gleadsto[q(i)]} \subseteq \allargs × \Phi$, ${\gbeats} \subseteq \allargs × \allargs$,
where $\Phi = \allprops × \set{\text{possible}, \text{sure}}$, and $+$ a commutative associative internal binary operation on $\allargs$. The notation $\sum \args = \sum_{\ar \in \args} \ar$ thus designates a well-defined argument.  
...
Define $\mbeats[a] \subseteq \allargs × \gargs$ as $\ar_1 \mbeats[a] \ar_0$ iff $\exists \ar \in c(\ar_0) \suchthat \ar_1 \mbeats \ar$ and $\nexists \ar \in c(\ar_0) \suchthat \ar \mbeats \ar_1]$. Define $\mbeats[*] \subseteq \allargs × \allargs$ as $\gbeatsstar = {\mbeats[g]} \cup {\mbeats[a]}$.
...
\begin{definition}[Answerability]
	$\forall \phi \in \Phi, \ar_1 \in \allargs, \ar_2 \in \gargs[\phi] \cap {\gbeatsstarinv}(\ar_1): (\ar_2, \ar_1) \ileadstoe ¬\phi ⇒ [\exists \ar_3 \in {\gbeatsstarinv}(\ar_2) \suchthat (\ar_3, \ar_2) \ileadstoe \phi]$.
\end{definition}
...
\begin{definition}[Operational validity]
	A theory is \emph{operationally valid} iff $\forall \phi \in {\gleadsto}(\allargs), \ar_0 \in \gargs[\phi], \ar_1 \in \allargs: (\ar_1, \ar_0) \ileadstoe ¬\phi ⇒ [\ar_1 \gbeats \ar_0] \land [\exists \ar_2 \in {\gbeatsinv}(\ar_1) \suchthat (\ar_0 + \ar_2, \ar_1) \ileadstoe \phi]$.
\end{definition}
...
\subsection{Theorem}
\begin{theorem}
	If a theory is answerable, has bounded length, and is operationally valid, it is valid.
\end{theorem}
\begin{proof}
	TODO adapt to new definitions!
