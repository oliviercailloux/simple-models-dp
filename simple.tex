...
\end{example}  
\begin{remark}[no HAI thanks to condition]
  Whether the condition $\pi(i_1) \neq \emptyset$ in \ref{it:condNotHAI} is required remains an open question. Showing that it is required requires an example where,
  defining $Q(\hist, i_1)$ as $\exists i_2 \in I \suchthat \pi(i_1) \neq \pi(i_2) \land \forall \histp \wsub \hist: {\ileadsto[i_1]}(\histp) = {\ileadsto[i_2]}(\histp)$,
  the condition
  $\forall \hist \in \allhists, i_1 \in I \suchthat \pi(i_1) \neq \emptyset: Q(\hist, i_1)$ is satisfied, even though
  $[\forall \hist \in \allhists, i_1 \in I: Q(\hist, i_1)]$ is not, thus satisfying $\exists \hist, i_0 \suchthat \pi(i_0) = \emptyset \land ¬Q(\hist, i_0)$.
  
  Define $I = \set{i_1, …, i_5}$, $\ileadsto[i_1], \ileadsto[i_2], \ileadsto[i_3]$ as in \cref{ex:noHAI} and $\ileadsto[i_4]$ and $\ileadsto[i_5]$ as: $.* \ileadsto[i_4] ¬\phi$, $\emptyset \ileadsto[i_4] ¬\phi$, $a.* \ileadsto[i_4] \phi$, $b.* \ileadsto[i_4] ¬\phi$, $\emptyset \ileadsto[i_5] ¬\phi$, $a.* \ileadsto[i_5] ¬\phi$, $b.* \ileadsto[i_5] \phi$. 
  This satisfies \ref{it:condNotHAI} for the reason given in \cref{ex:noHAI}, and fails $Q(\emptyset, i_4)$.
  This is not possible by starting from \cref{ex:noHAIwithDJ}: given any infinite set $\allargs$, defining some adequate $J = \set{i_0, …}$ and $I = I^\text{DJ} \cup J$ and $\forall i \in I: {\ileadsto} = i$, we get that $i_0$ conflicts on every paths with some individuals in $I^\text{DJ}$.
\end{remark}
\begin{remark}
