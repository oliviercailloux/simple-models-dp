...
Given $i \in I, \phi \in \Phi, \ar_1, \ar_0 \in \allargs, \args \subseteq \allargs$, with $\args$ finite, define recursively, on the basis of the cardinality of $\args$: $\emptyset \nibeatsi \ar_0$, and $\args \ibeatsi \ar_0$ iff $[\args \subseteq {\gbeatsinv}(\ar_0)] \land [\exists \hist \in \allhist \suchthat (\hist \cap {\gbeatsinv}(\ar_0) \subseteq \args \land \ar_0 \in \histend)] \land [\forall \args' \subset \args: \args' \nibeatsi \ar_0]$.

TODO claim that if $\hist \nileadstoi \phi$, with $\ar_1, \ar'_1 \in \hist$ and $\ar_0 \in \histend$, then $\ar_2$ is among the defensors, or $\ar'_2$ is.
