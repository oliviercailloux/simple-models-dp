...
We now say that $C$ is \emph{falsifiable} iff $\forall {\allleadstog} \in P^{I × \allhist}: ¬C({\allleadstog}) \implies \exists O \subseteq {\allleadstog} \suchthat O \implies ¬C$, meaning that
...
\begin{example}
	...
	By definition of a general observation, $O$ may not contain both $((\ar), i, \phi)$ and $((\ar'), i, \phi)$, thus letting $t$ denote one of these triples not in $O$, pick any ${\allleadstog'}$ such that $O \subset {\allleadstog'} \land t \in {\allleadstog'}$. We have ${\allleadstog'} \in S_O \cap C ≠ \emptyset$.
\end{example}
\begin{example}
...
\end{proof}
Therefore, falsifiability guarantees that a claim holds if, and only if, it is not a posteriori falsifiable. (TODO syntactically fals VS empirically fals.)
\begin{theorem}
	\label{th:holdsiff}
...
Given $\hist \in \allhist$ and $\ar_k, \ar_l \in \allargs$, let $(\hist, \ar_k, ?, \text{\ding{55}}, …, \ar_l, ?)$ denote the set $\set{\histl \suchthat \exists \histk \suchthat \hist \subset \histk \subset \histl \suchthat \ar_k \in \histkend \land \histk \nileadsto \phi \land \ar_l \in \histlend}$.

\begin{theorem}
	Given any $\phi \in P, \ar_k, \ar_l \in \allargs$, consider the following claim $C$: 
	\begin{equation}
		\left\{\begin{aligned}
			&\forall i \in I, \hist \in \allhist \suchthat \ar_k \in \histend \land \hist \nileadstog \phi: \set{\histp \suchthat \hist \subseteq \histp \land \ar_l \in \histpend} \ileadstog \phi;\\
			&\forall i \in I, \hist \in \allhist \suchthat \ar_l \in \histend \land \hist \nileadstog \phi: \set{\histp \suchthat \hist \subseteq \histp \land \ar_k \in \histpend} \ileadstog \phi.
		\end{aligned}\right.
	\end{equation}
	In short,
	$\forall \hist: [\ar_k \in \histend \land \hist \nileadstog \phi] \implies (\hist, …, \ar_l, ?) \ileadstog \phi$.
	In shorter,
	$(\hist, \ar_k, ?, \text{\ding{55}}, …, \ar_l, ?) \ileadstog \phi$, which is $\set{\histl \suchthat \exists \histk \suchthat \hist \subset \histk \subset \histl \suchthat \ar_k \in \histkend \land \ar_l \in \histlend \land \histk \nileadsto \phi}$.
	
	Assume that ${\allleadsto} \in C$. Then 
	$[\ar_k \nileadstost \phi \land \ar_l \nileadstost \phi]$ 
	is equivalent to: 
	\begin{equation}
		\left\{\begin{aligned}
			&\exists \histk \suchthat \ar_k \in \histkend \land \histk \nileadsto \phi \land \set{\histp \suchthat \histk \subseteq \hist' \land \ar_l \in \histpend} \ileadsto \phi;\\
			&\exists \histl \suchthat \ar_l \in \histlend \land \histl \nileadsto \phi \land \set{\histp \suchthat \histl \subseteq \hist' \land \ar_k \in \histpend} \ileadsto \phi.
		\end{aligned}\right.
	\end{equation}
\end{theorem}
\begin{proof}
	$\ar_k \nileadstost \phi$ iff $\exists \histk \suchthat \ar_k \in \histkend \land \histk \nileadsto \phi$.
\end{proof}
\begin{remark}
	Idea.
