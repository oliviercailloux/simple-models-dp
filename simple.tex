...
where $\allhist$ denotes all finite sequences of elements in $\allargs$ (including the empty sequence) and $\Phi = \allprops × \set{\text{possible}, \text{sure}}$. 
...
\begin{proposition}[Restricted interpretability satisfaction]
	All models satisfying \cref{ax:nocontr,ax:norm} also satisfy Restricted interpretability.
\end{proposition}
\begin{proof}
...
An empirical theory is always defined for some normative theory. In the following, any reference to an empirical theory should always be understood as a reference to an implicit normative theory and an empirical theory for that normative theory.
...
Given $\phi \in {\gleadsto}(\allargs)$, define $\gargs[\phi, 0] \subseteq \allargs$ as $\gargs[\phi, 0] = {\gleadstoinv}(\phi)$; and, for $k \in \N$, define $\gargs[\phi, k + 1] = \gargs[\phi, k] + [{\gbeatsinv} \circ {\ibeatseinv[!\phi]}](\gargs[\phi, k]) = \set{\ar_k + \ar \suchthat \ar_k \in \gargs[\phi, k], \ar  \in [{\gbeatsinv} \circ {\ibeatseinv[!\phi]}](\gargs[\phi, k])}$. (Throughout the article, $\N$ includes zero.) 
...
\commentOC{TODO also: in general, the model defines $\gargs[\phi]$ as a function of $\ileadstoe$? Start with $\gargs[\phi, 0]$, then observe all $\ar_1$ attacking those, and the model may define $\gargs[\phi, 1]$ depending on this observation, and so on. The condition is that one such argument be decisive.}
...
\begin{remark}
	...
	It follows that trying different $\ar_2, \ar'_2, …$ is also admissible.
	
	I need a (more) basic observable: a path $p$ is a sequence of arguments given to $i$, and I assume that whether $p \ileadsto[i] \phi$ is observable. But we can only assume that one path and its extensions is observable.
...
\end{remark}
I think I need the following. Define $\ar_1 \ibeatse \ar_0$ iff $\exists \hist \suchthat (\hist, \ar_0, \ar_1) \ileadstoe ¬\phi$. The model claims that if $\ar_1 \ibeatse \ar_0$ and $\ar_3 \ibeatse \ar_0 + \ar_2$, then $(\hist, \ar_0 + \ar_2 + \ar_4) \ileadstoe \phi$ (for some suitable $\hist$ that does not contain yet this arg or their c-a, or perhaps just claim that $\exists \hist \suchthat \hist \ileadstoe \phi$ with $\hist$ containing this arg). 

Better: the model claims that if $\ar_1 \ileadstoe ¬\phi$ and if $\ar_0$ and $\ar_1$ are in $\hist$ and none of the anticipated attackers of $\ar_0 + \ar_2$ are, then $(\hist, \ar_0 + \ar_2) \ileadstoe \phi$ (or, at least, that the model has at least one available candidate).
...
There must exist a sequence $\ar_0, \ar_2, \ar_4, …$ such that $\ar_0$ is defended by $\ar_2$, and so on.

\subsection{Conditions with history}
$S_4$ defends $S_2$ under protection of $S_6$ iff: $[\exists \hist, \ar_3 \suchthat \hist, \ar_2, \ar_3 \ileadstoe ¬\phi] ⇒ [\forall \hist \suchthat \set{\ar_5 \suchthat f(\ar_5, \ar_4) \cap \args_6 ≠ \emptyset} \cap \hist = \emptyset: \hist, \ar_4 \ileadstoe \phi]$. No, my condition is about defending when required, so just need to defend in the sense: $\forall \hist \suchthat \set{\ar_5 \suchthat f(\ar_5, \ar_4) \cap \args_6 ≠ \emptyset} \cap \hist = \emptyset: \hist, \ar_4 \ileadstoe \phi$. This may be adequate as if $\ar_0$ seems suitable but some unplanned attacker $\ar_1$ attacks it, then use $\ar_1 \in \hist$ and observe that $¬(\hist, \ar_0) \ileadstoe \phi$ to falsify the theory.

$\ar_4$ defends $\phi$ under protection of $S_6$ iff: $\forall \hist \in \allhist \suchthat \set{\ar_5 \in \allargs \suchthat f(\ar_5, \ar_4) \cap \args_6 ≠ \emptyset} \cap \hist = \emptyset: \hist, \ar_4 \ileadstoe \phi$.

\begin{definition}[Claim]
	$\exists k \in 2\N$ and a finite sequence $(\args_i)_{i \in 2\N, i ≤ k} \suchthat \args_0 \subseteq {\gleadstoinv}(\phi)$ and $\forall i \in 2\N, i ≤ k, \forall \ar_i \in S_i: \ar_i$ defends $\phi$ under protection of $S_{i + 2}$ (defining $S_{k + 2} = \emptyset$) and $S_i \subseteq \cup_{\ar_{i - 2} \in S_{i - 2}, \ar_{i - 1} \in \allargs} f(\ar_{i - 1}, \ar_{i - 2})$.
\end{definition}
Note that requiring that $\args_0$ be a singleton does not modify the condition, in the sense that any theory satisfying the condition also satisfies its restriction.

No, rather: if $\ar_2$ can’t be defended, and $\ar'_2$ can’t either (all those attacking $\ar_1$, in fact), then $\ar_0$ is not necessary; thus some other $\ar'_0$ is enough. (Or is this equivalent?) Theimportant stuff is to anticipate attacks: if not $\ar_0$ decisive, then $\ar_1$ attacks $\ar_0$ and $\ar_2$ is decisive.

I need a condition of the form: $\forall i \suchthat …, \ar$ decisive. And the above claim is of that form (?): given $i$, either $\ar_0$ is decisive, or $\ar_1$ attacks it but then $\ar_2$ is decisive, …
