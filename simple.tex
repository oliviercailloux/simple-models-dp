...
\end{proposition}
\begin{proof}
	Let us prove that $\set{\dbeatsinv(\args) \suchthat \emptyset ≠ \args \subseteq \gbeatsinv(\ar)} = \powersetz{\df(\ar)}$, from which the conclusion follows, and which can be useful to clarify the relationship between $\di(\ar)$ and $\df(\ar)$.

	First (from left to right), $\forall \args \suchthat \emptyset ≠ \args \subseteq \gbeatsinv(\ar_0): \dargs = \dbeatsinv(\args) \in \powersetz{(\dbeatsinv \circ \gbeatsinv)(\ar)}$. 
	Indeed, as $\args ≠ \emptyset$, we can pick some $\ar_1 \in \args$, and $\ar_1 \gbeats \ar_0$, hence, for some $\ard \in \alldargs$, $\ard \dbeats \ar_1$ (as mandated by ${\dbeats}$), thus $\ard \in \dargs$, hence $\dargs ≠ \emptyset$. Also, picking any $\ard \in \dargs$, we see that for some $\ar_1 \in \args$, $\ard \dbeats \ar_1 \gbeats \ar_0$. 
	
	Second (from right to left), $\dargs \in \powersetz{(\dbeatsinv \circ \gbeatsinv)(\ar)} ⇒ \exists \emptyset ≠ \args \subseteq \gbeatsinv(\ar_0) \suchthat \dbeatsinv(\args) = \dargs$. 
	Defining $\args = {\dbeats}(\dargs) \cap \gbeatsinv(\ar_0)$, we see that $\dbeatsinv(\args) = \dargs$: considering any $\ard \in \dargs$, $\ard \dbeats \ar_1 \gbeats \ar_0$ for some $\ar_1 \in \allargs$, thus, $\ar_1 \in \args$, thus, $\ard \in \dbeatsinv(\args)$; and considering any $\ard \in \dbeatsinv(\args)$, $\ard \dbeats \ar_1 \in \args$, $\ar_1 \gbeats \ar_0$, thus, $\ard \in \dargs$ \commentOC{This is incorrect.}. 
