...
\RequirePackage{silence}\WarningFilter{typearea}{Maybe no optimal type area settings}
\documentclass[version=last, pagesize, twocolumn, twoside=off, bibliography=totoc, 12pt, a4paper, french, english]{scrartcl}
...
\footnote{Here is an example of severe $\prop$-unstability. $\forall (\ar, \ar'), (\ar, \ar') \ileadstose ¬\prop, (\ar, \ar') \ileadstose \prop$.}
...
\footnote{Here is an example of non $\prop$-clear-cut situation with no severe $\prop$-unstability. $\forall (\ar, \ar'), (\ar, \ar') \ileadstose ¬\prop, (\ar, \ar') \ileadstope \prop, (\ar, \ar') \ileadstopst ¬\prop$.}
\footnote{Something like Axiom JU should be sufficient for CC. JU: If $(\ar, \ar')$ is $\prop$-unstable, then $\exists \ar_2 \in \allargs$ such that for $(\ar_1, \ar_0) = (\ar, \ar')$ or for $(\ar_1, \ar_0) = (\ar', \ar)$, $\ar_2$ attacks decisively $\ar_1$, where we mean by this that $\ar_2$ combined with $\ar_0$ is decisive. Means that if not CC, then not JU. Hence, infinite discussion happens, or irreducible unstability. Or, the memory buffer of $i$ is too short to hold adequate reinstated arguments, in which case our approach based on decisive arguments is not adequate.}
\end{definition}
...
Given a decision situation, define $\wibeatse[\prop], \nwibeatse[¬\prop] \subseteq \mbeats$ as follows.

Given $\ar_3 \mbeats \ar_2, \prop \in \alltopic$: $\ar_3 \wibeatse[\prop] \ar_2$ iff $[\exists \ar_1 \in \mbeats(\ar_2) \suchthat (\ar_2 \wibeatse[\prop] \ar_1 ∧ \ar_2 \nwibeatse[\prop] \ar_3 + \ar_1)] ∨ [\ar_2 \ileadsto \prop ∧ (\ar_3, \ar_2) \ileadstose ¬\prop]$.
...
This is well defined thanks to the following construction.
Given a model. 
\begin{itemize}
	\item Define a root as an argument that $\mbeats$-attacks nobody. 
	\item Associate to each $\ar \in \clargs$ its depth $d(\ar)$, the distance to the farthest root: $d(\ar) = 0$ iff $\ar$ is a root and $d(\ar) = k+1$ iff $\ar$ $\mbeats$-attacks some $\ar'$ of depth $k$ and $\mbeats$-attacks no $\ar'$ of depth superior to $k$ (thus $d(\ar) = 1$ iff $\ar$ attacks some and only root nodes; $d(\ar) = 2$ iff $\ar$ attacks some node of depth 1 and only nodes of depth 1 or 0; and so on). 
	\item Observe that if $\ar_3 \mbeats \ar_2$ and $d(\ar_3) = k+1$, then $d(\ar_2) ≤ k$. 
	\item Define $\wibeatse[\prop]_{(0)} = \nwibeatse[\prop]_{(0)} = \emptyset$. %as $\ar_3 \wibeatse[\prop]_{(1)} \ar_2$ iff $d(\ar_3) = 1 ∧ \ar_2 \ileadsto \prop ∧ (\ar_3, \ar_2) \ileadstose ¬\prop$. 
%	\item Define $\nwibeatse[\prop]_{(1)}$ as $\ar_3 \nwibeatse[\prop]_{(1)} \ar_2$ iff $d(\ar_3) = 1 ∧ \ar_2 \mleadsto \prop ∧ (\ar_3, \ar_2) \ileadstope \prop$. 
	\item Define $\wibeatse[\prop]_{(k+1)} \subseteq \mbeats$, $k ≥ 0$, as $\ar_3 \wibeatse[\prop]_{(k+1)} \ar_2$ iff, first, $d(\ar_3) = k+1$, and second, $[\exists \ar_1 \in \mbeats(\ar_2) \suchthat (\ar_2 \wibeatse[\prop]_{(d(\ar_2))} \ar_1 ∧ \ar_2 \nwibeatse[\prop]_{(d(\ar_2))} \ar_3 + \ar_1)] ∨ [\ar_2 \ileadsto \prop ∧ (\ar_3, \ar_2) \ileadstose ¬\prop]$. Proceed similarly for $\nwibeatse[\prop]_{(k+1)}$.
