...
Given an exposure protocol $(I, \allargs, {\allleadsto})$, recall that ${\allleadsto} \in \allallleadsto$.

Given a pair $(I, \allargs)$, a claim $C$ is a set of possibilities for $\allleadsto$, thus, $C \subseteq \allallleadsto$.
Given a formula $\psi$, let the corresponding claim be $C_\psi = \set{{\allleadstog} \in \allallleadsto \suchthat {\allleadstog} \text{ satisfies } \psi}$.
...
Let $S_θ = \set{{\allleadstog} \in \allallleadsto \suchthat θ \subseteq {\allleadstog}}$ denote the possibilities for $\allleadsto$ compatible with the observations.
...
Let $\incr(\allhist) = \set{\emptyset} \cup \cup_{\emptyset \neq N \subseteq \N} \set{A(N) \suchthat A \in (\allhist)^N \land [\card{A_i} = i] \land [i < j \implies A_i \subsetneq A_j]} \subseteq \powerset{\allhist}$ denote the possibly empty embedded sequences of paths.
An element $\theta^{-1}(P)$ can also be seen as a (finite) function associating to each $i \in I$ an element of $\incr(\allhist)$.
Therefore, we let $\finite(\incr(\allhist)^I) = \set{\theta^{-1}(P) \suchthat \theta \in \Theta}$ denote the set of possible observations restricted to $I × \allhist$, the elements of which being called \emph{a priori observation}, where $\finite(\incr(\allhist)^I)$ denote the functions $f: I \mapsto \incr(\allhist)$ with finite support, thus $\card{f^{-1}(\incr(\allhist) \setminus \emptyset)} < \N$.
Note that $\finite(\incr(\allhist)^I)$ and $\incr(\allhist)^I$ are downwards closed, meaning that $\forall O \in \finite(\incr(\allhist)^I): O' \subseteq O \implies O' \in \finite(\incr(\allhist)^I)$ and $\forall O \in \incr(\allhist)^I: O' \subseteq O \implies O' \in \incr(\allhist)^I$.
...
Let the \emph{simple a priori observations} be defined as $\cup_{J \subseteq_\finite I} (\allhist)^J$, equivalently, as the right-unique finite relations over $I × \allhist$.
Given a simple a priori observation $O: J \mapsto \allhist$, we can define the a priori observation $O_O = \set{(i, \hist) \in J × \allhist \suchthat \forall i \in J: \hist \subseteq O(i)}$ as the completion of $O$ to all sub-paths in $O$.
...
\begin{definition}
  ...
  %$\forall {\allleadstog} \in \allallleadsto: ¬C({\allleadstog}) \implies \exists θ \subseteq {\allleadstog} \suchthat θ \implies ¬C$, meaning that
...
\end{remark}
\begin{theorem}
  A claim $C$ is falsifiable iff its complement $\Cbar = \allallleadsto \setminus C$ satisfies:
...
\end{proof}
\begin{theorem}[To check considering finiteness of observations]
  Given a claim $C$, consider the sets $S \subseteq C^\subseteq$ such that $\set{\theta^{-1}(P) \suchthat \theta \in S} = \incr(\allhist)^I$ and such that $\cup S$, considered as a relation on $(I × \allhist) × P$, is right-unique (that is, $\cup S$ forms a new relation $\allleadstog$).
  $C$ is falsifiable iff all such sets $S$ satisfy $\cup S \in C$.
\end{theorem}
\begin{proof}
  Consider such $S$.
  Assume that $\cup S$ is not in $C$.
  Let us show that $C$ is not falsifiable.
  Define ${\allleadstog} = \cup S$, pick any $\theta \in \Theta, \theta \subseteq {\allleadstog}$ and let us show that $\theta \in C^\subseteq$.
  Define $O \in (\incr(\allhist))^I \suchthat \theta^{-1}(P) \subseteq O$. Thus, $O \in \set{\theta^{-1}(P) \suchthat \theta \in S}$. Pick an element $\theta' \in S \suchthat (\theta')^{-1}(P) = O$. We have $\theta' \in C^\subseteq$ and $\theta^{-1}(P) \subseteq (\theta')^{-1}(P)$.
  Also, $\theta \subseteq \cup S$, and $\cup S$ is right-unique, so $\theta$ coincides with $\theta'$ on its domain (meaning that $\theta = \restr{\theta'}{\theta^{-1}(P)}$). It follows that $\theta \subseteq \theta'$. As $C^\subseteq$ is downwards-closed, this concludes.

  Now, assume that $C$ is not falsifiable and pick any ${\allleadstog} \notin C \suchthat \forall \theta \in \Theta, \theta \subseteq {\allleadstog}: \theta \in C^\subseteq$.
  Given any $O \in (\incr(\allhist))^I$, we have $\restr{\allleadstog}{O} \in C^\subseteq$.
  Define $S = \set{\restr{\allleadstog}{O} \suchthat O \in (\incr(\allhist))^I}$.
  The set $S$ satisfies the required conditions and $\cup S = {\allleadstog}$, thus, $\cup S \notin C$.
\end{proof}
Here is another way (perhaps more illuminating) of stating the same thing.
\begin{remark}
  Any relation $\allleadstog$ is equivalent to $\cup \set{\restr{\allleadstog}{O} \suchthat O \in (\finite(\incr(\allhist))^I)}$.
  (Also, any relation $\allleadstog$ is equivalent to $\cup \set{\restr{\allleadstog}{O} \suchthat O \in \cup_{J \subseteq_\finite I} (\allhist)^J}$.)
  Conversely, any set $S$ of observations such that $\set{\theta^{-1}(P) \suchthat \theta \in S} = \incr(\allhist)^I$ and $\cup S$ is right-unique corresponds to a unique relation $\cup S = {\allleadstog}$.
...
\begin{theorem}
  A claim $C$ is falsifiable iff \[\forall f \in C^{\finite(\incr(\allhist)^I)}: \cup_{O \in \finite(\incr(\allhist)^I)} \restr{f(O)}{O} \in C.\]
\end{theorem}
\begin{proof}
  ...
  The theorem states that a claim $C$ is falsifiable iff $\forall {\allleadstog} \in \allallleadsto: \set{\restr{\allleadstog}{O} \suchthat O \in \finite(\incr(\allhist)^I)} \subseteq C^\subseteq \implies {\allleadstog} \in C$.

  This holds because $\forall {\allleadstog} \in \allallleadsto: \set{\restr{\allleadstog}{O} \suchthat O \in \finite(\incr(\allhist)^I)} = \set{θ \subseteq {\allleadstog}}$.
\end{proof}
...
\begin{definition}[Anonymity]
  Given a permutation $\sigma$ over $I$ and any ${\allleadstog} \in \allallleadsto$, let ${\allleadstog} \circ \sigma$ denote $\set{(i, \hist, \phi) \suchthat (\sigma(i), \hist, \phi) \in {\allleadstog}}$ and let $C \circ \sigma$ denote $\set{{\allleadstog} \circ \sigma \suchthat {\allleadstog} \in C}$.
...
$((a_j), …, a_k)$ convincing ; $a_k$ not decisive (or $(a_k)$ not convincing?); $\nexists \ar \suchthat [(\ar, …, a_k)$ not convincing $\land(\ar, …,  a_j \cup a_k)$ not convincing].
  ...
  \begin{example}
    ...
    %	The claim represents the set $\set{{\allleadsto} \in \allallleadsto \suchthat (\ar) \ileadsto \phi \lor  (\ar, \ar) \ileadsto \phi \lor …}$.
