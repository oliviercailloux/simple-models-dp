...
\subsection{Negation coherence and analysis of normative adequacy}
The propositions such that $\phi \in \Box\Phi \lor ¬\phi \in \Box\Phi$ are those on which the model permits to take a position. For that subset of propositions (which can be empty): $\phi \in \Box\Phi \oplus ¬\phi \in \Box\Phi$, where $\oplus$ denotes the exclusive disjunction; equivalently: $\phi \in \Box\Phi ⇔ ¬\phi \notin \Box\Phi$.
\begin{property}[Protocol coherence]
	\label{prop:protcoh}
	$[\exists \ar \in \allargs \suchthat \ar \ileadstost[pos] \phi] ⇒ [\nexists \ar \in \allargs \suchthat \ar \ileadstost[pos] ¬\phi]$.
\end{property}
\begin{property}[Internal negation coherence]
	\label{prop:intnegcoh}
	$\forall \phi \suchthat [\phi \in \Box\Phi \lor ¬\phi \in \Box\Phi]: \phi \in \Box\Phi \oplus ¬\phi \in \Box\Phi$.
\end{property}
\begin{property}[Restricted negation coherence]
	\label{prop:restrnegcoh}
	$\forall \phi \suchthat [\phi \in \Box\Phi \lor ¬\phi \in \Box\Phi]: \phi \in \iPhi \lor ¬\phi \in \iPhi$.
\end{property}
\begin{proposition}[Restricted negation coherence satisfaction]
	A normatively adequate model satisfies Internal negation coherence and Restricted negation coherence.
\end{proposition}
\begin{proof}
	First, observe that Protocol coherence is satisfied by definition of $\ileadstost[pos]$. Indeed, if for some $\ar \in \allargs$, $\ar \ileadstost[pos] \phi$, then $\forall \ar_1 \in \allargs: (\ar_1, \ar) \nileadstoe ¬\phi$; thus, for any $\ar' \in \allargs$, $\ar' \ileadstost[pos] ¬\phi$ is false, because it requires that $\forall \ar_1 \in \allargs: (\ar_1, \ar') \ileadstoe ¬\phi$, and thus in particular, that $(\ar', \ar) \ileadstoe ¬\phi$ (using the fact that the pair to the left of $\ileadstoe$ is unordered).
	
	Consider $\phi \suchthat [\phi \in \Box\Phi \lor ¬\phi \in \Box\Phi]$. Proving \cref{prop:intnegcoh} only requires to show that $\phi \in \Box\Phi ⇒ ¬\phi \notin \Box\Phi$. By definition, $\phi \in \Box\Phi$ implies that $\exists \ar \suchthat \ar \ileadstost[pos] \phi$, and the implication is thus proven by using \cref{prop:protcoh}: $\phi \in \Box\Phi ⇒ [\exists \ar \suchthat \ar \ileadstost[pos] \phi] ⇒ ¬[\exists \ar \suchthat \ar \ileadstost[pos] ¬\phi] ⇒ \phi \notin \Box\Phi$.
	
	Finally, \cref{prop:restrnegcoh} follows from \cref{prop:intnegcoh} thanks to normative adequacy.
\end{proof}

We want to adopt a normative axiom satisfying the following meta-axioms.
\begin{definition}[Meta-axiom: negation coherence]
	$\forall \phi \in \Phi: ¬(\phi \in \iPhi \land ¬\phi \in \iPhi)$.
\end{definition}
\begin{definition}[Meta-axiom: certainty implies possibility]
	$\forall \phi \in \Phi: \propsure \in \iPhi ⇒ \propposs \in \iPhi$.
\end{definition}
Also, we want the normative axiom to be based on observation, and to allow for incompleteness. And the meta-axioms must hold for any possible observations (they are not empirical claims).

From all this, hopefully, I can justify my normative axiom; by justifying its form $\ar \ileadstost[undef] \phi ⇒ \phi \in \iPhi$ in terms of a yet-undefined $\ileadstost[undef]$, then by proving that $\ileadstost[undef]$ must satisfy Protocol coherence and Certainty implies possibility.

\begin{definition}[Certainty implies possibility]
	$\forall \ar \in \allargs: \ar \ileadstost[undef] \propsure ⇒ \ar \ileadstost[undef] \propposs$.
\end{definition}
