...
${\gbeats} \subseteq \allargs × \allargs$,
...
$\forall \ar \in \gPhi: \exists k \in \N^* \suchthat \dfp^k(\ar) = \emptyset$.
...
Given $i \in I, \phi \in \Phi, \ar_1, \ar_0 \in \allargs$, define $\ar_1 \ibeatsi \ar_0$ iff $\exists \hist \in \allhist \suchthat {\gbeatsinv}(\ar_0) \cap \hist = \set{\ar_1} \land \ar_0 \in \histend \land \hist \nileadstoi \phi$.
...
$\ar$ $i$-defends $\phi$ iff $\forall \hist \in \allhist \suchthat \left[\ar \in \histend \land {\gbeatsinv}(\ar) \cap \hist = \emptyset\right]: \hist \ileadstosti \phi$. $\ar_0$ recursively $i$-defends $\phi$ iff $\forall \ar \in \cup_{k \in \N}(\dip)^k(\ar_0)$: $\ar$ $i$-defends $\phi$.

\begin{definition}[Convincingness]
	$\forall i \in I, \phi \in \gPhi, \ar \in \gargs[i, \phi]: \ar$ $i$-defends $\phi$.
\end{definition}

A \emph{convincingness falsification instance} of an empirical theory is a tuple $(i, \phi, \hist) \in I × \gPhi × \allhist$ such that $\exists \ar \in (\histend \cap \gargs[i, \phi]) \suchthat {\gbeatsinv}(\ar) \cap \hist = \emptyset \land \hist \nileadstosti \phi$.

\begin{theorem}
	An empirical theory is convincing iff it has no convincingness falsification instances.
\end{theorem}
\begin{proof}
	For all $i \in I, \phi \in \gPhi$: $\exists \ar \in \gargs[i, \phi]$ that does not $i$-defend $\phi$ iff $\exists \ar \in \gargs[i, \phi], \hist \in \allhist \suchthat \left[\ar \in \histend \land {\gbeatsinv}(\ar) \cap \hist = \emptyset \land \hist \nileadstosti \phi\right]$ iff $\exists \hist \in \allhist$ such that $(i, \phi, \hist)$ is a convincingness falsification instance.
\end{proof}

\begin{definition}[Unnecessary coalitions]
	$\forall i \in I, \phi \in \gPhi, \hist \in \allhist, \ar_0 \in \gargs[i, \phi], \set{\ar_1, \ar'_1} \subseteq {\gbeatsinv}(\ar_0) \cap \hist, \ar_0 \in \histend, \hist \nileadstoi \phi$: $\exists \ar_2 \in {\dbeatsinv}(\set{\ar_1, \ar'_1})$ such that $\ar_2$ recursively $i$-defends $\phi$.
\end{definition}
...
Possible easy approach: If $\args \ibeatsi \ar_0$, then both $\ar_1 \ibeatsi \ar_0$ and $\ar'_1$.

Sophistication: when $\args = \set{\ar_1, \ar'_1}$ attack $\ar_0$, it is too demanding to ask that both $\ar_2$ and $\ar'_2$ defend $\phi$. Perhaps $\ar_2$ does but $\ar'_2$ fails (and $\ar'_2 + \ar_2$ as well) because it confuses $i$, which finds $\ar'_1$ irrelevant or hard to understand. To relax the claim, could define that $\ar_1$ seems ineffective against $\ar_0$ iff $\exists \hist \suchthat \set{\ar_1, \ar_0} = \histend \land \hist \ileadstosti \phi$. If so, and if $\args \ibeatsi \ar_0$, then $\ar'_2$ must be in the defensors. This supplementary condition (about ineffectivity) is there to show that $\ar'_1$ plays at least some role in the attack, because the theory must not claim that $\ar'_2$ defends $\phi$ when $\ar'_1$ is not really an attacker, which it would seem to be if $\args \ibeatsi \ar_0$ because of $\ar_1$. More generally, $\args$ seems ineffective against $\ar_0$ iff $\exists \hist = (…, \args, \ar_0) \suchthat \hist \ileadstosti \phi$. If so, and if $\args \cup \set{\ar_1} \ibeatsi \ar_0$, then $\ar_2$ must be in the defensors.

TODO implement the sophisticated approach with: $\args \cup \ar_1 att \ar_0 \land \args$ seems uneff implies $\ar_1 \ibeatsi \ar_0$. (Here att stands for $\ibeatsi$ for coalitions.) Check that it works for coalitions of more than two.

\begin{remark}[TODO]
	I could make that claim (unnecessary coalitions) stronger by mandating that if $\ar_0$ fails, that must be because some $\ar_1$ always attacks $\ar_0$, in the sense that when $\ar_1$ and $\ar_0$ are in $\histend$, then $¬\phi$. Such a claim largely eases falsification. But this is too demanding: $\ar_1$ may be a not-so-great argument that does sometimes contradict $\ar_0$ but not systematically, and that $\ar_2$ can reply to convincingly.
	
	$\ar_1 \ibeatsi \ar_0$ only when $\ar_1$ att $\ar_0$ alone (among those planned attacks of $\ar_0$), but $\ar_1$ need not necessarily be at the end of $\hist$. Thus, ${\ibeatsi}$ is a theoretical claim (that other arguments do not matter and that $\ar_1$ is indeed responsible for the change of mind), and $\ar_1 \ibeatsi \ar_0 ⇒ \ar_1 \gbeats \ar_0$.
	
	Also, a coalition can attack $\ar_0$. Required because could have coal together attacks $\ar_0$ but individual arguments do not. Thus, I am interested in minimal coalitions that attack. For those, the theory must be able to answer with the sum of its replies (in which order?).
	Define $\set{\ar_1, \ar'_1} \ibeatsi \ar_0$. Requires that each argument of the coalition be among the planned attackers; and that sub-coalitions do not attack. This requires a specific answer from the theory, namely, $\ar_0 + \ar_2 + \ar'_2$.
	
	Define that $\ar_0$ strongly defends $\phi$ iff defense works when using no planned attack, and that $\ar_0 + \ar_2$ works when using only $\ar_1$ and if $\ar_1 \ibeatsi \ar_0$, and that $\ar_0 + \ar_2 + \ar'_2$ works when using both $\ar_1$ and $\ar'_1$ and this is a minimal planned attacking coalition.

	Now, a falsification instance is $\hist$ containing $\ar_1$ and $\ar'_1$, two planned attacks, such that $\ar_2$ alone sometimes fails, $\ar'_2$ alone sometimes fails, and the addition sometimes fails: this proves the theory wrong in all cases, one argument alone attacking or both together being required.
	
	A coalition could have a synergic effect: theory can reply individually, but fail its collective reply. However, that should not deserve explicit consideration: if individual replies work, and individual argument attacks, then some decisive argument exists.
\end{remark}

I want that s1 attacks-i s0 only when it’s because of s1 (according to the theory). Otherwise, the theory is forced to answer s1, an irrelevant argument. Thus, s1 attacks-e s0 requires that no s1' (planned attack against s0) is used.

Also, everything must be falsifications except when it’s because of s1 i-attacking s0 (in which case the theory commits to answer). Thus, if two planned attacks are used, this is a falsification instance? No, because the theory commits to answer each one separately. Thus, if two planned attacks are used AND individual ones do not i-attack, then it is a falsification instance. Also, if (s1, s0, s0) leads to not phi, and not (s0, s1, s0), then this is a falsification instance.

Convincingness must happen exactly when there is no falsification instance. Thus, ar i-defends phi when all dangerous histories are answered to by ar and when two planned attacks i-attack, some individual one i-attacks and when (s1, s0, s0) leads to not phi, then (s0, s1, s0) leads to not phi.

Hence, the dangerous histories are those such that no reordering lead to $\set{\ar_1, \ar_0} = \histend$ for some $\ar_1$ that is a planned, effective attack. (involve no planned-attack or multiple irreducible planned attacks or an attack that happens only with a strange ordering).
Well, that’s not great: to know whether an instance falsifies, we’d need to ensure, for each reordering of planned attacks, that they are non effective attacks, which is basically impossible. NO suffices to exhibit some history hist' with ar1, ar0 at end and leading to phi.

Thus, falsification: hist, ar1=> hist', ar1' => hist'', where hist uses ar1 and ar1' (which are all the planned attacks in hist) and hist' ends with ar1, ar0, hist => notphi, hist' => phi, and so on.

%Rather keep falsification instances simple, and enlarge the situations that the theory commits to answer. If some history combine ar1 and ar1' and lead to not phi, the theory must answer either ar1 or ar1' (it can choose => if ar1, ar0 in histend lead to not phi, it must answer ar1; if ar1' etc.; and if none, then it has lost). Thus falsification happens when both ar1 and ar1' are used and some some hist, ar1 does not attack ar0 and for some hist, ar1' does not attack ar0. This can be checked. And about (s1, s0, s0) (with s1 not planned): this is a falsification instance. Define it as: no planned attack is used AND s1 in hist AND s0 in histend.

\begin{theorem}
	A normatively adequate and convincing empirical theory is valid.
\end{theorem}
\begin{proof}
	\commentOC{This might be false. Consider ${\gbeats} = \set{(\ar_1, \ar_0)}, {\dbeats} = \set{(\ar_2, \ar_1)}, (\ar_1, \ar_0, \ar'_1) \ileadstoi ¬\phi$. There is no decisive argument in $\gargs[i, \phi]$ as $\ar'_1 \ibeatsi \ar_0$, but no falsification instance as $\hist = (\ar_1, \ar_0, \ar'_1)$ includes $\ar_1$. In such a situation, $\ar_1$ should be considered as attacking $\ar_0$, as (from the model pov) it is $\ar_1$ which makes $i$ fail to adopt $\phi$.}
	
	For all $i \in I, \phi \in \gPhi$, suffices to show that for some $\ar \in \allargs$, $\ar \ileadstosti \phi$. (Actually, $\ar \in \dfp^k(\ar_0)$ for some $k$.)
	
	Lemma: if $\ar_0$ recursively $i$-defends $\phi$, then either $\ar_0$ is decisive, or $\ard_2 \dbeats \ar_1 \gbeats \ar_0$ with $\ar_2 = \ar_0 + \ard_2$ recursively $i$-defending $\phi$ and $\ar_2 \in \dfp(\ar_0)$.
	If $\ar_0$ is not decisive, and because $\ar_0$ defends, some $\hist$ ending with $\ar_0$ includes some $\ar_1 \gbeats \ar_0$. If this is the only one, then $\ar_1 \ibeatsi \ar_0$, and $\ar_2$ recursively defends. If it is not the only one, use the no coalition property.
\end{proof}
...
\begin{definition}[Conv.\ SA]
	$\forall i \in I, \phi \in \gPhi, k \in 2\N, \ar \in \gargs[i, \phi, k]: \ar$ $i$-defends $\phi$ ignoring ${\gbeats}(\gargs[i, \phi, k + 2])$.
\end{definition}
...
Given $\phi \in \gPhi$, define ${\ibeatse[!\phi]} \subseteq \allargs × \allargs$ such that $\ar' \ibeatse[!\phi] \ar$ iff $(\ar', \ar) \nileadstost \phi$. 
...
\begin{definition}[Answerability]
	$\forall \phi \in \gPhi, \ar_0 \in \gargs[\phi], \ar_1 \in \allargs: (\ar_1, \ar_0) \nileadstost \phi ⇒ [\exists \ar_1' \in {\gbeatsinv}(\ar_0)]$.
\end{definition}
...
\begin{definition}[Operational validity]
	$\forall \phi \in \gPhi, \ar_0 \in \gargs[\phi], \ar_1 \in \allargs: [\ar_1 \gbeats \ar_0 \land (\ar_1, \ar_0) \nileadstost \phi] ⇒ \exists \ar_2 \in {\gbeatsinv}(\ar_1)$.
\end{definition}
...
\end{theorem}
\begin{proof}
	Lemma: given $\phi \in \gPhi$ and $\ar[k] \in \gargs[\phi]$, if $\ar[k] \nileadstost \phi$, then: $\exists \ar[k + 1] \in \gargs[\phi] \suchthat \card{c(\ar[k + 1])} ≥ \card{c(\ar[k])} + 1$.
	To prove the lemma, let us pick any $\phi \in \gPhi$ and $\ar[k] \in \gargs[\phi]$ such that $\ar[k] \nileadstost \phi$. 
	...
	To prove the theorem, given $\phi \in \gPhi$, we need to exhibit an argument $\ar$ such that $\ar \ileadstost \phi$.
