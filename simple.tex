...
\begin{axiom}[Normative adequacy of $\allargs$ and $\ileadstost$ 2]
	$\forall \phi \in \Phi$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \ileadstost \phi] ⇒ \lnot\phi \in \iphibar.
	\end{equation}
\end{axiom}
\begin{axiom}[Coherent complement]
...
\end{theorem}
\begin{proof}
	Coherent complement is equivalent to $\forall \phi \in \phi: ¬(\phi \in \iphi \land \phi \in \iphibar)$.
	Coherent protocol is equivalent to $\forall \phi \in \Phi: ¬(\ar \ileadstost \phi \land \ar \ileadstost ¬\phi)$, which implies (using Norm ad 1 and 2) $\forall \phi \in \Phi: ¬(\phi \in \iphi \land \phi \in \iphibar)$.
\end{proof}
...
\begin{definition}[Claiming certainty requires claiming possibility]
	$\forall (\prop, \text{sure}) \in {\gleadsto}(\allargs): (\prop, \text{poss}) \in {\gleadsto}(\allargs)$.
\end{definition}

\begin{definition}[Challenging possibility implies challenging certainty]
