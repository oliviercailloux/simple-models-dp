...
Given an behavior function $\allleadstog$, deliberated choices are represented by an element $\pi({\allleadstog}) \subseteq I × P$ defined as $\pi({\allleadstog})(i) = P_i$. \Cref{th:sing} shows that $\pi({\allleadstog})$ is right unique (meaning that $\forall i \in I: \card{\pi({\allleadstog})(i)} ≤ 1$).
...
An element $Q \in \allpriobss$ \commentOC{previously denoted $O$ and called a priori observation} is called a query.
\commentOC[Note that $\incr(\allhists)^I$ and $\allpriobss$ are downwards closed, meaning that $\forall O \in \incr(\allhists)^I: \set{O' \subseteq O} \wsub \incr(\allhists)^I$ and $\forall O \in \allpriobss: \set{O' \subseteq O} \wsub \allpriobss$.]{To be deleted.}
...
\commentOC[Note that $(\cup_{H \in \max \incr(\allhists)}P^H)^I$ is the set of functions that associate to each $I$ a maximal increasing sequence of paths, together with values from $P$.]{To be deleted.}

\commentOC[Let also $\max\incr(\allhists) = \set{A \in \incr(\allhists) \suchthat \nexists A' \in \incr(\allhists) \suchthat A \subset A'}$ being set equal to $\set{A \in \incr(\allhists) \cap (\allhists)^\Z}$ denote the maximal elements of $\incr(\allhists)$, in the sense of containing sequences that are not a subsequence of another sequence; equivalently, containing those sequences whose sequence of cardinals of their elements equals $\Z$.]{To be deleted.}
...
\commentOC[Let the \emph{simple a priori observations} be defined as $\cup_{J \subseteq_\finite I} (\allhists)^J$, equivalently, as the right-unique finite relations over $I × \allhists$.
  Given a simple a priori observation $O: J \mapsto \allhists$, we can define the a priori observation $O_O = \set{(i, \hist) \in J × \allhists \suchthat \forall i \in J: \hist \subseteq O(i)}$ as the completion of $O$ to all sub-paths in $O$.]{To be deleted.}
...
Such an observation $\theta \in \Theta \cap \powerset{\allleadsto}$ is called a feasible observation.
...
Verifiability mandates that some (possibly counter-factual) observation proves the claim (makes sure that it holds), thus, $C$ is verifiable iff $\exists \theta \in \Theta \suchthat \forall \theta \wsub {\allleadstog}: {\allleadstog} \in C$.
...
\begin{lemma}
  ...
  If $\card{\allargs} ∉ ℕ$, given any $θ ∈ Θ$ and $f: I \rightarrow (P \cup \set{\emptyset})$, $∃ {\allleadstog} \in \allleadstos \suchthat θ \wsub {\allleadstog} \land \pi({\allleadstog}) = f$.
\end{lemma}
\begin{proof}
  For each $i \suchthat f(i) ∈ P$, define $a_i ∉ (θ^{-1}(P))(i)(ℕ)$ as an argument not used for $i$;
  for each $i \suchthat f(i) = \emptyset$, for each $p ∈ P$, define $k_i^p ∉ (θ^{-1}(P))(i)$ as a path not used for $i$, with all $k_i^p$ different from each other ($∀ p, p' ∈ P, k_i^p ≠ k_i^{p'}$).

  Define $f' = θ ∪_{i \suchthat f(i) ∈ P, α \suchthat a_i ∈ \histend} \set{(i, \hist, f(i))} ∪_{i \suchthat f(i) = \emptyset, p ∈ P, \ar \in \allargs} \set{(i, (k_i^p, a), p)}$.
...
\begin{proof}
  Consider any $θ ∈ Θ$.
  By informativeness, $∃ f: I \rightarrow (P \cup \set{\emptyset}) \suchthat f ∉ \pi(C)$.
...
$\exists \theta \in \Theta \cap D^⊆ \suchthat \forall \theta \wsub {\allleadstog} \in D: {\allleadstog} \in C$.

$S ⊆ P^{\allhists × I}$ is observationally excludable iff 
$∃ θ ∈ Θ \suchthat θ ∉ S^⊆$.
$D$ is observationally completeable to $E$ iff $D \setminus E$ is observationally excludable.
...
\end{theorem}
\begin{proof}
  That claim $C$ is verifiable under prior $D$, in other words, that $\exists \theta \in \Theta \cap D^⊆ \suchthat \forall \theta \wsub {\allleadstog} \in D: {\allleadstog} \in C$, implies that $D \setminus C$ is obs exc, equivalently, that $∃ θ ∈ Θ \suchthat θ ∉ (D \setminus C)^⊆$.
\end{proof}
...
Consider a prior $D$: we know that ${\allleadsto} \in D$.
...
\begin{theorem}
  ...
  then 
  the claim $C = \set{{\allleadstog} \suchthat π(\allleadstog)(I) = \set{\emptyset}}$ is verifiable under prior $D$.
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  $\forall {\allleadstog} \notin C: ∃ θ \in \Theta \cap \powerset{\allleadstog} \suchthat \theta \notin C^\wsub$.
...
\Cref{th:AHFimpl} and related show that anonymous and falsifiable claims that hold exist iff the situation does not satisfy $\forall \hist: P^{\intvl{\hist}} \wsub \restr{{\allleadsto}(I)}{\intvl{\hist}}$.
...
\begin{example}
  Assume that $∃ \ar, \phi \suchthat \forall i: \ar \ileadstost \phi$.
...
$\set{\histl \suchthat ∃ \histk \suchthat \hist \subset \histk \subset \histl \land \ar_k \in \histkend \land \histk \nileadstog \phi \land \ar_l \in \histlend}$.
Given $\ar_k, \ar_l \in \allargs$, let
$(…, \ar_k, ?, \nileadstog \phi, …, \ar_l, ?)$ denote the set
$\cup_{\hist \in \allhists} (\hist, \ar_k, ?, \nileadstog \phi, …, \ar_l, ?) =
  \set{\histl \suchthat ∃ \hist, \histk \suchthat \hist \subset \histk \subset \histl \land \ar_k \in \histkend \land \histk \nileadstog \phi \land \ar_l \in \histlend} =
  \set{\histl \suchthat ∃ \histk \suchthat \histk \subset \histl \land \ar_k \in \histkend \land \histk \nileadstog \phi \land \ar_l \in \histlend}$.
...
\begin{theorem}
  ...
  $∃ i \in I \suchthat [\ar_k \nileadstost p \land \ar_l \nileadstost p]$
  is equivalent to:
  \begin{multline}
    \label{eq:cln}
    ∃ i \in I \suchthat \forall \set{\ar_1, \ar_2} \suchthat \set{\ar_1, \ar_2} = \set{\ar_k, \ar_l}:\\
    ∃ \hist \in (…, \ar_1, ?) \cap \nileadstoinv(p) \suchthat
    (\hist, …, \ar_2, ?) \subseteq \ileadstoinv(p).
  \end{multline}
\end{theorem}
\begin{proof}
  $\ar_k \nileadstost p$ iff $∃ \hist \suchthat \ar_k \in \histend \land \hist \nileadsto p$.
\end{proof}
\begin{remark}
  Idea.
...
\begin{theorem}[Formal]
  ...
  \begin{multline}
    \label{eq:answerable}
    \forall i \in I, ∃ \set{\ar_1, \ar_2} = \set{\ar_k, \ar_l} \suchthat
    \forall \hist \in (…, \ar_1, ?) \cap \nileadstoinv(p): \\
...
\Cref{th:hsif,th:hif,th:asif} show their compatibility when one of holding or anonymity is not required (except that informativeness beyond counting cannot be made strong in the edge case where $\forall i \in I: P_i = \emptyset$).
...
It is possible under reasonable conditions of regularity to satisfy anonymous and holding together with indicativeness. This happens for example when everybody has a deliberate judgment and a finite set of decisive arguments suffices to lead everybody to their deliberate judgment (\cref{th:suffIndicDec}).
  ...
  \end{theorem}
  \begin{remark}
    Define $C^\text{A} = \set{{\allleadstog} \in \allleadstos \suchthat {\allleadstog}(I) = {\allleadsto}(I)}$. Observe that $C^\text{A}$ is anonymous, that $C^\text{HA} \wsub C^\text{A}$, and that $C^\text{HA} \neq C^\text{A}$ iff $\exists i, j \in I: {\ileadsto} \neq {\ileadsto[j]}$ (to be checked).
  \end{remark}
  ...
  \begin{proof}
    ...
    define $p = P_{i_1}$.
    Consider any claim $C$ such that $C \wsub \set{{\allleadstog} \in \allleadstos \suchthat {\ileadsto[i_1]} \in {\allleadstog}(I) \wsub {\allleadsto}(I)}$ and $C$ is anonymous (for example, $C^\text{HA}$ is suitable).
    % Define the claim $C$ as $\set{{\allleadstog} \in \allleadstos \suchthat \forall i \in I: {\ileadstog} \in \set{{\ileadsto[j]} \suchthat j \in I}}$ (or as anything included in it that includes $C^\text{HA}$ and is anonymous).
...
The following definition assumes a set $B \wsub P^{\allhists}$ of behaviors. We take $B = P^{\allhists}$ if nothing is specified about $B$.
\begin{definition}
  Say that an attitude $g \in B$ is observably included in a set of attitudes $S \wsub B$ iff $\forall H \in \finite(\incr(\allhists)): \exists f \in S \suchthat \restr{g}{H} = \restr{f}{H}$.

  Define the extension of a set of attitudes $S \wsub B$ to its observably included attitudes as the set of attitudes $R(S) = \set{g \in B \suchthat \forall H \in \finite(\incr(\allhists)): \exists f \in S \suchthat \restr{g}{H} = \restr{f}{H}}$.
  Note that $S \wsub R(S)$.
  Also note that $S_1 \wsub S_2 \implies R(S_1) \wsub R(S_2)$.
\end{definition}
\begin{theorem}
  \label{th:RR}
  $\forall S \wsub B: R(R(S)) \wsub R(S)$.
\end{theorem}
\begin{proof}
...
\begin{example}
  ...
  \Cref{th:caractR} indicates that $R({\allleadsto}(I)) = \set{f \suchthat l^S_f(\emptyset) \in \set{F^\phi, F^{¬\phi}} \land \forall \ar \in \allargs: [\forall \hist \suchthat \hist_1 = \ar: l^S_f(\hist) \in l^S_f(\emptyset)] \land [\forall \hist, \histp \suchthat \hist_1 = \histp_1 = \ar: l^S_f(\hist) = l^S_f(\histp)]}$.
...
\begin{theorem}
  \label{th:falsclosed}
  Under any $B \wsub P^{\allhists}$ (thus recall $R(S) \wsub B$), for any anonymous claim $C \wsub B^I$ that is falsifiable or trivial, 
  $\forall {\allleadstog} \in C, F \wsub {\allleadstog}(I) \suchthat [\forall f \in F: \card{({\allleadstoginv}(f))} \notin \Z]: R(F)^I \wsub C$.

  $\exists {\allleadstog} \in C, \emptyset ≠ F \wsub {\allleadstog}(I) \suchthat \forall f \in F: \card{({\allleadstoginv}(f))} \notin \Z$.
  Then, $R(F)^I \wsub C$.
\end{theorem}
\begin{proof}
  Consider any anonymous claim $C$, any ${\allleadstog} \in C, F \wsub {\allleadstog}(I) \suchthat [\forall f \in F: \card{({\allleadstoginv}(f))} \notin \Z]$ and any ${\allleadstog'} \in R(F)^I \setminus C$.
  To show that $C$ is not falsifiable,
  consider any observable $\theta \wsub {\allleadstog'}$ and let us show that it is completable in $C$. Let $\theta(P^{\allhists})$ denote the (finite) set of individuals that $\theta$ interrogates.
  
  From the antecedent we see that $I$ is infinite.
  We know that $\forall i \in \theta(P^{\allhists}): \exists g \in R(F) \suchthat \theta(i) \wsub g$, thus, by definition of $R$, $\exists f \in F \suchthat \theta(i) \wsub f$.
  Define $\gamma: \theta(P^{\allhists}) \to F$ that picks one such $f$ for each $i$.
  Thus, $(\gamma^{-1}(f), f \in F)$ is a partition of $\theta(P^{\allhists})$ such that $\forall i \in \gamma^{-1}(f): \theta(i) \wsub f$.

  Consider sets of same sizes $I'_f$ among the non observed individuals (meaning those in $I \setminus \theta(P^{\allhists})$) and with corresponding types in ${\allleadstog}$ (so that $\forall f \in F, i \in I'_f: {\ileadstog} = f$).
  Define a bijection $\sigma$ that for each $f \in F$ bijects $I_f$ to $I'_f$ and that sends the elements of $I \setminus (\cup_f I_f \cup I'_f)$ to themselves.
...
\begin{theorem}
  Assume that for some $H \in \finite(\incr(\allhists))$ and $i_1 \in I \suchthat p = P_{i_1} \in P$, we have $\forall i_2 \suchthat P_{i_2} \neq P_{i_1}$ that $\exists \hist \in H \suchthat {\ileadsto[i_1]}(\hist) \neq {\ileadsto[i_2]}(\hist)$.

  Then $\forall H' \in \incr(\allhists), P^{H'} \wsub \restr{{\allleadsto}(I)}{H'}$ iff $\forall H \wsub H', f \in P^{H'}$: $\exists i \in I \suchthat f \wsub {\ileadsto} \land [\restr{f}{H} = \restr{\ileadsto[i_1]}{H} \implies \exists \ar \in \allargs \suchthat \ar \ileadstost p]$.
\end{theorem}
\begin{proof}
...
\begin{theorem}
  \label{th:noindic}
  If $a_1 \ileadstost[f_1] \phi$ and $a_2 \ileadstost[f_2] ¬\phi$ with $\ar_1 ≠ \ar_2$ and $f_1(\emptyset) = f_2(\emptyset)$ and $\forall f \in \set{f_1, f_2}: \card{{\allleadstoinv}(f)} \notin \N$, then there is no anonymous, falsifiable and indicative about $i_1$ claim that holds (here, considering indicativeness as mandating to know that $P_{i_1} = \phi$).
\end{theorem}
\begin{proof}
  Any claim that holds must include $\allleadsto$.
  Define $F = \set{f_1, f_2}$.
  
  From indicativeness about $i_1$, $\exists \theta \in \Theta \cap (\allhists × \set{i_1} × P) \wsub {\allleadsto} \suchthat \emptyset ≠ \pi(\set{{\allleadstog} \suchthat \theta \wsub {\allleadstog}} \cap C)(i_1) = \phi$.
  Define $H \in \incr(\allhists)$ as the query corresponding to $\theta$, thus $H = \theta(i_1, P)$.

  Define $g \in P^{\allhists}$ as $g = \set{(\hist, f_1(\hist)) \suchthat \exists \emptyset ≠ \histp \in \intvl{\hist} \cap H} \cup \set{(\hist, f_2(\hist)) \suchthat \intvl{\hist} \cap H \wsub \set{\emptyset}}$.
  Then, $g \in R(F)$. To show this, we need to show that $\forall H' \in \finite(\incr(\allhists)): \exists f \in F \suchthat \restr{g}{H'} = \restr{f}{H'}$.
  If $\intvl{\max H'} \cap H \wsub \set{\emptyset}$, then $f_2$ satisfies the condition (consider any $\hist \in H'$, we have $\intvl{\hist} \cap H \wsub \set{\emptyset}$); otherwise (thus $\emptyset ≠ \hist' \in \intvl{\max H'} \cap H$), $f_1$ does (consider any $\hist' \in H'$).

  Now let us show that $P_g ≠ \phi$.
  That is because at least one of $\hist = (\ar_2, \ar_1)$ or $\hist = (\ar_1, \ar_2)$ satisfies $\intvl{\hist} \cap H \wsub \set{\emptyset}$ and in both cases, $\hist \ileadsto[f_2] ¬\phi$, thus, $g(\hist) = ¬\phi$.
  
  Define ${\allleadstog'}$ as $\forall i \in I: {\ileadstog'} = g$.
  By \cref{th:falsclosed}, any claim that is anonymous and falsifiable must include ${\allleadstog'}$.
  
  Also, $g$ is compatible with $\theta$.
  It follows that $\pi(\set{{\allleadstog} \suchthat \theta \wsub {\allleadstog}} \cap C)(i_1) ≠ \phi$.
\end{proof}

I need some restriction on the set of a priori possible behaviors.
Define $B \wsub P^{\allhists}$ as the set of a priori possible behaviors.
For example, define $B_1$ as the set of behaviors such that $\forall \ar_1, \ar_2, \hist: (\hist, \ar_1, \ar_2) = (\hist, \ar_2, \ar_1)$ (where equality, abusing notation, means that they lead to the same result).
...
We used to consider also situations $\overline{S} = \set{{\allleadstog} \suchthat \exists i \in I \suchthat {\ileadstog} \in \overline{B}}$ where some $i$ is in $\overline{B}$.
...
I might need something like the following. $\ar$ is apparently innocuous iff $\forall \hist, {\allleadstog}: {\allleadstog}(\hist, \ar) = {\allleadstog}(\hist)$. And $\ar$ is innocuous iff $\forall \hist, \histp: \hist, \ar, \histp = \hist, \histp$. The required hypothesis might be $B_2$: if $\ar$ is apparently innocuous, then it is innocuous.

\begin{theorem}
  Under some $B$, 
  if $a_1 \ileadstost[f_1] \phi$ and $a_2 \ileadstost[f_2] ¬\phi$ and ${\ileadsto[f_1]}(\emptyset) = {\ileadsto[f_2]}(\emptyset)$ and infinitely many individuals have behavior $f_1$ and infinitely many have behavior $f_2$, then there is an anonymous, falsifiable and indicative about $i_1$ claim that holds (here, considering indicativeness as mandating to know that $P_{i_1} = \phi$).
\end{theorem}
\begin{proof}
...
\subsection{Extensions}
  ...
  \begin{conjecture}
    ...
    % Given $k \in \Z$ and a family $(O_l)_{l \in \intvl{1, k}}$, consider $\exists f \in C \suchthat 
    Say that $R$ is coverable iff $\exists k \in \Z, (O_l)_{l \in \intvl{1, k}}, (o_l)_{l \in \intvl{1, k}} \suchthat
      \cup_{l \in \intvl{1, k}} O_l = R \land
      \forall l \in \intvl{1, k}: o_l \in O_l \land
      \forall o \in O_l: \set{o_l, o} \in \allpriobss \land
