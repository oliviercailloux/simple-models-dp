...
\begin{example}
  ...
  The claim associated to $R({\allleadsto}(I))$ is not trivial: $\forall f \suchthat [\exists \hist, \histp ≠ \emptyset \suchthat \hist_1 = \histp_1 \land f(\hist) ≠ f(\histp)]: f \notin R({\allleadsto}(I))$.
\end{example}
\begin{example}
...
\end{remark}
\begin{conjecture}
  Given a non empty set of attitudes $S \wsub P^{\allhists}$, the claim associated to $R(S)$ is not trivial iff [there exists some $H \in \finite(\incr(\allhists))$ such that some non empty equivalence class $S_H \in S / =_H$ on $S$ over $H$ does not permit all continuations, meaning that $\exists \ar, p \suchthat \forall f \in S_H: f(\max H, \ar) ≠ p$]; or $\exists p \suchthat \forall f \in S: f(\emptyset) ≠ p$.
\end{conjecture}
\begin{proof}
  We want to show that $\exists f \in P^{\allhists}, \hist \in \allhists \suchthat \cap_{\histp \wsub \hist} l^S_f(\histp) = \emptyset$ is equivalent to the stated consequent.

  Assuming that the consequent holds, consider any $f$ such that $\restr{f}{H} = \restr{g}{H}$ for some (equivalently, for all) $g \in S$ and such that $f(\max H, \ar) = p$ (or pick $H = \ar = \emptyset$).
  We have $\cap_{\histp \wsub (\max H, \ar)} l^S_f(\histp) = \emptyset$.

  Consider any $f, \hist \suchthat \cap_{\histp \wsub \hist} l^S_f(\histp) = \emptyset$ and let’s prove that the consequent holds.
  As $S ≠ \emptyset$, $l^S_f(\hist) ≠ \emptyset$.
  Consider the longest subsequence of $\hist$ such that $l^S_f(\histp) ≠ \emptyset$.
\end{proof}

Generally,
define $S^p = \set{f \in P^{\allhists} \suchthat f(\emptyset) = p}$.
Then \cref{th:caractR} indicates that $R({\allleadsto}(I)) = \set{f \suchthat l^S_f(\emptyset) \in \set{S^p \suchthat p \in P} \land \forall \ar \in \allargs: [\forall \hist \suchthat \hist_1 = \ar: l^S_f(\hist) \in l^S_f(\emptyset)] \land [\forall \hist, \histp \suchthat \hist_1 = \histp_1 = \ar: l^S_f(\hist) = l^S_f(\histp)]}$.
TODO correct this.
...
\begin{remark}
  ...
  For example,  and ${\ileadstog}$ is $\phi$ when the path starts with $\ar_1$ (or is empty) and is $¬\phi$ when the path starts with $\ar_2$ and 
