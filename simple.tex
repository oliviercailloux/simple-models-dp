...
In other words, given $\ard \in \dargs$ and $\ar_0 \in \allargs$, $\ar_2 \in \df(\ar_0)$ iff $\exists \ar_1 \in \allargs \suchthat \ar_2 \dbeats \ar_1 \gbeats \ar_0$.
...
\begin{proposition}
	$\dargs \in \di(\ar) ⇒ \emptyset ≠ \dargs \subseteq \df(\ar)$.
\end{proposition}
\begin{proof}
	Let us prove that $\set{\dbeatsinv(\args) \suchthat \emptyset ≠ \args \subseteq \gbeatsinv(\ar)} = \powersetz{\df(\ar)}$, from which the conclusion follows, and which can be useful to clarify the relationship between $\di(\ar)$ and $\df(\ar)$. Note that by definition of $\df(\ar)$, $\dargs \in \powersetz{\df(\ar)}$ iff $\emptyset ≠ \dargs \subseteq \dbeatsinv(\gbeatsinv(\ar))$.

	First (from left to right), given any $\args \suchthat \emptyset ≠ \args \subseteq \gbeatsinv(\ar_0)$, define $\dargs = \dbeatsinv(\args)$, and let us show that $\dargs ≠ \emptyset$ and that $\dargs \subseteq \dbeatsinv(\gbeatsinv(\ar))$. 
	...
	Second (from right to left), $\emptyset ≠ \dargs \subseteq \dbeatsinv(\gbeatsinv(\ar)) ⇒ \exists \emptyset ≠ \args \subseteq \gbeatsinv(\ar_0) \suchthat \dbeatsinv(\args) = \dargs$. 
	Defining $\args = {\dbeats}(\dargs) \cap \gbeatsinv(\ar_0)$, we see that $\dbeatsinv(\args) = \dargs$: considering any $\ard \in \dargs$, $\ard \dbeats \ar_1 \gbeats \ar_0$ for some $\ar_1 \in \allargs$, thus, $\ar_1 \in \args$, thus, $\ard \in \dbeatsinv(\args)$; and considering any $\ard \in \dbeatsinv(\args)$, $\ard \dbeats \ar_1 \in \args$, $\ar_1 \gbeats \ar_0$, thus, $\ard \in \dargs$ \commentOC{This is incomplete, should mention injectivity.}. 
	That $\args ≠ \emptyset$ follows from $\dargs ≠ \emptyset$.
\end{proof}

\begin{proof}[Alternative proof]
	Given $\args, \dargs, \ar$ such that $\emptyset ≠ \args = {\dbeats}(\dargs) \cap \gbeatsinv(\ar)$:
	\begin{equation}
		\dbeatsinv(\args) = \dargs ⇔ \emptyset ≠ \dargs \subseteq \dbeatsinv(\gbeatsinv(\ar)).
	\end{equation}

	LHS equiv $\ard \in \dargs ⇔ \exists \ar_1 \in \args \suchthat \ard \dbeats \ar_1$.
	Using $\args \subseteq \gbeatsinv(\ar)$, it is also equivalent to: $\ard \in \dargs ⇔ \exists \ar_1 \in \args \suchthat \ard \dbeats \ar_1 \gbeats \ar$.
	
	RHS equiv $\ard \in \dargs ⇒ \exists \ar_1 \in \allargs \suchthat \ard \dbeats \ar_1 \gbeats \ar$.
	
	LHS implies RHS. Obvious.
	
	RHS implies LHS. First, $\dargs \subseteq \dbeatsinv(\gbeatsinv(\ar))$ (by RHS), and $\ard \in \dbeatsinv(\gbeatsinv(\ar)) ⇔ \exists \ar_1 \in \gbeatsinv(\ar) \cap {\dbeats}(\ard) ⇔ \exists \ar_1 \in \gbeatsinv(\ar) \cap {\dbeats}(\dargs) \suchthat \ard \dbeats \ar_1 ⇔ \ard \in \dbeatsinv(\args)$.
	
	Second, $\exists \ar_1 \in \args \suchthat \ard \dbeats \ar_1 ⇒ \ard \in \dargs$: it follows from $\args \subseteq \dbeats(\dargs)$ and injectivity: $\ar_1 \in \args$ thus $\ar_1 \in {\dbeats}(\dargs)$ thus $\exists \ar_2 \in \dargs \suchthat \ar_2 \dbeats \ar_1$, and $\ar_2 = \ard$ by injectivity, thus $\ard \in \dargs$.
	
	Finally, prove that $\emptyset ≠ \dargs \subseteq \dbeatsinv(\gbeatsinv(\ar)) ⇔ \exists \emptyset ≠ \args \subseteq \gbeatsinv(\ar_0) \suchthat \dbeatsinv(\args) = \dargs$.
	\commentOC{This equivalence is true, but the proof above might be insufficient: it only proves that RHS ⇒ LHS when $\args$ is chosen as defined, but it could be that sometimes $\args$ can be chosen differently, so that $\dbeatsinv(\args) = \dargs$ but $\args ≠ {\dbeats}(\dargs)$.}
\end{proof}

\begin{proof}[More promising]
	...
	LHS implies RHS. Prove that $\emptyset ≠ \dbeatsinv(\args) \subseteq \dbeatsinv(\gbeatsinv(\ar))$. $\args \subseteq \gbeatsinv(\ar)$, thus, $\dbeatsinv(\args) \subseteq \dbeatsinv(\gbeatsinv(\ar))$. Also, $\args ≠ \emptyset$, thus $\dbeatsinv(\args) ≠ \emptyset$, as mandated by $\dbeats$, by definition of empirical theories.
	
	RHS implies LHS. Define $\args = {\dbeats}(\dargs) \cap \gbeatsinv(\ar)$ and prove that $\args ≠ \emptyset$ and $\dbeatsinv(\args) = \dargs$.
	
	First, $\dargs \subseteq \dbeatsinv(\args)$. That is because $\dbeatsinv(\gbeatsinv(\ar)) = \dbeatsinv(\args)$. This can be seen by: $\ard \in \dbeatsinv(\gbeatsinv(\ar)) ⇔ \exists \ar_1 \in \gbeatsinv(\ar) \cap {\dbeats}(\ard) ⇔ \exists \ar_1 \in \gbeatsinv(\ar) \cap {\dbeats}(\dargs) \suchthat \ard \dbeats \ar_1 ⇔ \ard \in \dbeatsinv(\args)$.

	Second, $\dbeatsinv(\args) \subseteq \dargs$. That is because $\dbeatsinv(\args) \subseteq \dbeatsinv({\dbeats}(\dargs)) \subseteq \dargs$, by injectivity of $\dbeats$.
	
	RHS implies LHS (alt). RHS implies $\dargs \subseteq \dbeatsinv(\gbeatsinv(\ar))$, which implies that $\dbeatsinv({\dbeats}(\dargs)) = \dargs$, by injectivity of $\dbeats$, and $\dbeatsinv({\dbeats}(\dargs)) = \dbeatsinv({\dbeats}(\dargs) \cap \gbeatsinv(\ar))$. 
	Therefore, $\dargs = \dbeatsinv({\dbeats}(\dargs)) = \dbeatsinv({\dbeats}(\dargs) \cap \gbeatsinv(\ar)) = \dbeatsinv(\args)$.
\end{proof}
...
\begin{definition}[Convincingness]
	$\forall i \in I, \phi \in \gPhi, \ar \in \gargs: \ar$ $i$-defends $\phi$.
\end{definition}
...
\end{theorem}
\begin{proof}
	Given any $i \in I$ and $\phi \in \gPhi$, suffices to show that for some $\ar \in \allargs$, $\ar \ileadstost \phi$; the conclusion then follows thanks to normative adequacy. 
	...
	Define $\args = \gbeatsinv(\ar_0) \cap \hist$. Thus, $\args \ibeats \ar_0$.
	Also, because $\ar_0$ $i$-defends $\phi$, $\args ≠ \emptyset$. 
	Therefore, $\dbeatsinv(\args) \in \di(\ar_0)$.
	
	From this lemma, using convincingness, we deduce that $[\ar_0 \in \gargs \land \ar_0 \nileadstost \phi] ⇒ \exists \ar_2 \in \gargs \cap \dfp(\ar_0)$. That is because $\ar_0 \in \gargs ⇒ \ar_0$ $i$-defends $\phi$, by convincingness, and, considering any $\dargs \in \di(\ar_0)$ and $\ar_2 = \ar_0 + \dargs$, $\ar_2 \in \gargs$ by definition of $\gargs$ and $\ar_2 \in \dfp(\ar_0)$ because $\dargs \in \di(\ar_0) ⇒ \dargs \in \df(\ar_0)$.

	To prove the theorem, build a finite sequence of arguments in $\gargs$, starting with any $\ar_0 \in \gleadstoinv(\phi)$. Observe that $\ar_0 \in \gargs$. Given $k \in 2\N$, if $\ar_k \nileadstost \phi$, define $\ar_{k + 2}$ using the deduction following the lemma, obtaining $\ar_{k + 2} \in \gargs \cap (\dfp)^\frac{k + 2}{2}(\ar_0)$. 
...
Given $\phi \in \Phi$, $\hist \in \allhist$ and $\ar_\gamma \in \gargs$, say that $(\hist, \ar_\gamma)$ challenges $(\gamma, \phi)$ iff $\hist \nileadstost \phi$ and $\ar_\gamma \in \histend$.
Given $\phi \in \Phi, \ar_\gamma \in \gargs, \ar_\delta \in \gargsdelta$, say that $(\hist, \ar_\gamma, \ar_\delta)$ is a $(\gamma, \phi$ or $\delta, ¬\phi)$-challenge iff $(\hist, \ar_\gamma)$ challenges $(\gamma, \phi)$ or $(\hist, \ar_\delta)$ challenges $(\delta, ¬\phi)$. \commentOC{This should be called a $\phi$/$¬\phi$-challenge.}
...
a \emph{test procedure with $i$} is a finite sequence $({\hist}^k)_{k \in \intvl{1, N}}$, for some $N \in \N^*$, of sequences of arguments, 
