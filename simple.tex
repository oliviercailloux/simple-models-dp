...
% $S_\theta \cap C ≠ \emptyset$, equivalently, iff 
...
\begin{definition}
  ...
  % $\exists θ \in \Theta \cap \powerset{\allleadsto} \suchthat θ \notin C^\wsub$.
...
\begin{theorem}
  \label{th:ex-post}
  $\forall C \ni {\allleadsto}:
    % \nexists θ \in \Theta \suchthat \theta \subseteq {\allleadsto} \land θ \notin C^\wsub$.
...
\begin{proof}
  ...
  % Consider any claim $C$ with ${\allleadsto} \in C$.
  % Consider any $\theta \in \Theta$ with $\theta \wsub {\allleadsto}$.
  % We have $θ \subseteq {\allleadsto} \in C$, thus $θ \in C^\wsub$.
\end{proof}
...
\begin{example}[Disjunct with incompatible paths, not rejectable]
  ...
  In fact,
  % $\nexists \theta \in \Theta \suchthat \theta \notin C^\wsub$.
  $\Theta \wsub C^\wsub$.
...
\begin{remark}
  \Cref{ex:threeFs} provides an example where, similarly, the claim may fail to hold but still be not rejectable; but where rejection depends on ${\allleadsto}$.
\end{remark}

\Cref{def:fals} fills the gap to obtain that $C$ holds iff it is not rejectable.
...
\begin{definition}
  ...
  %Actually we need to exclude trivial claims with an exists in supplement to forall; so that trivial claims are not falsifiable. Then the property that we seek is trivial-or-falsifiable.
\end{definition}
...
\begin{definition}
  A claim is \emph{unfalsifiable} iff $\exists {\allleadstog} \notin C \suchthat \Theta \cap \powerset{\allleadstog} \wsub C^\wsub$.
\end{definition}
\begin{definition}
  A non-trivial claim is \emph{strongly unfalsifiable} iff $\Theta \wsub C^\wsub$.
\end{definition}

\begin{example}[Big disjunct, strongly unfalsifiable]
  \label{ex:bigor}
  ...
  % ${\allleadstog} \in S_θ \cap C ≠ \emptyset$.
  $\theta \in \powerset{\allleadstog}$ and ${\allleadstog} \in C$, thus, $\theta \in C^\wsub$.
\end{example}
\begin{example}[Disjunct with incompatible paths, not falsifiable]
  \label{ex:disj}
