...
A claim $C$ is \emph{indicative} iff $∃ i \in I, \theta \in \Theta \cap \powerset{\allleadsto} \suchthat \pi(\set{{\allleadstog} \suchthat \theta \wsub {\allleadstog}} \cap C) ≠ (P \cup \set{\emptyset})^I$. Indicativity mandates that some feasible observation permits to deduce something about DJ’s.
...
\begin{lemma}
  Given $f ∈ P^{I × \allhists}$,
  let $Q(f) = (f^{-1}(P))(I)(ℕ)$ denote the arguments used in $f$;
  let $K(f) = \max\set{\card{\hist} \suchthat \hist ∈ (f^{-1}(P))(I)} ∈ ℕ ∪ \set{∞}$ denote the length of the longest path in $f$.
  If $K(f) ∈ ℕ$, then $∀p, ∃ f ⊆ f' ∈ P^{I × \allhists} \suchthat p ∉ π(f') ∧ $.
  If $∃ \ar ∉ Q(f)$, then $∃ f ⊆ f' ∈ P^{I × \allhists} \suchthat φ ∈ π(f') ∧ Q(f') = Q(f) ∪ \set{\ar}$.
\end{lemma}
