...
In the following definition, $(\hist^1, …, \hist^j, …, \hist^n)$ represent the sequence $\hist^1$ to which is suffixed the sequence $\hist^2$ and so on.
\begin{definition}[Truthfulness]
	...
	$\forall i \in I, \hist \in \allhist \suchthat 
	k_j \in \intvl{1, \card \hist} \land
	\hist^j = (\hist_1, …, \hist_{k_j}) \land
	\forall j: [k_{j - 1} < k_j \land \hist^j \nileadsto \pga \land \ar_1 = \arga \land \ar_j \in \histend[j] \land \ar_{j + 1} = \dg(\ar_j, \hist^j)]: (\hist, \ar_{n + 1}) \ileadsto \pga]$.
\end{definition}
\begin{axiom}[Answerability]
	\label{ax:ans}
	$\forall \ar, \ar': [\exists \hist \suchthat (\hist, \ar, \ar') \nileadsto \pga \land \exists \hist \suchthat (\hist, \ar, \ar') \ileadsto \pga] ⇒ \exists \hist, \ar_c \suchthat (\hist, \ar, \ar', \ar_c) \nileadsto \pga$.
\end{axiom}
...
\begin{axiom}[Reinstatement]
	\label{ax:r}
	$\forall \hist, \ar_1 \in \gargsi: [(\hist, a_1, a) \nileadsto p \land (\hist, a_1, a, a_2, …) \ileadsto p] ⇒ [(\hist, a_2) \ileadsto p \land \forall \hist': (\hist', a_2, a') \ileadsto p ⇒ (\hist', a_1, a') \nileadsto p]$.
\end{axiom}
\begin{example}[No reinstatement]
	Reinstatement is required. Consider $a_1$ not decisive because of $a$ and then $(a_1, a, a_2) \ileadsto p$. But still $a_2$ not decisive: $a2 \nileadsto p$.
\end{example}
\begin{theorem}[Validity]
	If an argumentative theory $(\pga, \arga, \dg)$ is truthful and $\allargs$ is finite, then \cref{ax:ans,ax:r} imply that the theory is correct.
\end{theorem}
