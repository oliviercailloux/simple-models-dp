...
The claim holds iff ${\leadsto} \in C$.
...
\begin{example}[Fals. but shouldn’t]
	$\hist \ileadsto[j] \phi \land [(\ar) \ileadsto \phi \lor (\ar, \ar) \ileadsto \phi \lor …]$ is falsifiable.
	Consider $O = \set{(j, \hist, ¬\phi)}$.
\end{example}
\begin{example}[Fals. but shouldn’t, better]
	\label{ex:fbs}
...
\end{example}
\begin{example}[TODO]
	$\hist \ileadsto[j] \phi \land [(\ar) \ileadsto \phi \lor (\ar, \ar) \ileadsto \phi \lor …]$ is not falsifiable?
	Consider any ${\leadsto} \notin C$, thus satisfying $\hist \nileadsto[j] \phi \lor [(\ar) \nileadsto \phi \land (\ar, \ar) \nileadsto \phi \land …]$.
	Consider $O = \set{(j, \hist, ¬\phi)}$.
\end{example}
