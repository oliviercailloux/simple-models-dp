...
\begin{conjecture}
  ...
  \begin{enumerate}[label=T\ref{th:caractNotIndic}-{\arabic*}]
    ...
    % \item \label{it:condNotHAIAll} either all individuals have an empty deliberated preference, or all paths are such that for all $i_1$, there is an $i_2$ with another deliberated judgement that behaves identically on the whole path (formally: either $\pi(I) = \emptyset$ or $\forall \hist \in \allhists, i_1 \in I: \exists i_2 \in I \suchthat \pi(i_1) \neq \pi(i_2) \land \forall \histp \wsub \hist: {\ileadsto[i_1]}(\histp) = {\ileadsto[i_2]}(\histp)$).
...
\begin{proof}
  ...
  % Let us show that \ref{it:condNotHAI} implies \ref{it:condNotHAIAll}.
  % Assume that at least one individual $i$ has a non-empty deliberated preference (otherwise, the first disjunct of \ref{it:condNotHAIAll} holds).
  % Consider any path $\hist$ and $i_1$ with $\pi(i_1) = \emptyset$.
  % Apply \ref{it:condNotHAI} on $i$, obtaining that $\exists i_2 \in I \suchthat \emptyset \neq \pi(i) \neq \pi(i_2) \land \forall \histp \wsub \hist: {\ileadsto[i_1]}(\histp) = {\ileadsto[i_2]}(\histp)$.
  % NOPE, that’s wrong.

  TODO prove that \ref{it:condNotHAI} implies \ref{it:condNotHAIInf}.
\end{proof}
\begin{example}
  I believe that we can’t remove the $\pi(i_1) \neq \emptyset$ in \ref{it:condNotHAI}.
