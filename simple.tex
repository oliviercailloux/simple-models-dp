...
It can be expected that some behavior highlighted by psychologists \citep{kahneman_thinking_2012}, such as framing effects, would be rejected by the individuals themselves when pondering their decisions. 
...
\begin{remark}
	This does not work, because $\gamma$ supporting $\phi_{\set{a, b, c, d}, ¬b}$, $\phi_{\set{a, b, c, d}, ¬c}$ and $\phi_{\set{a, b, c, d}, ¬d}$ may be valid even if $i$ never chooses $a$: $i$ may satisfy $¬b$ (by picking $c$) when given an argument for $¬b$, satisfy both $¬b$ and $¬c$ (by picking $d$) when given arguments for $¬b$ and $¬c$, and so on. The individual $i$ is not obliged to satisfy the three arguments for $¬b$, $¬c$ and $¬d$ simultaneously, as these arguments are required to count only when they are at the end of the sequence.
\end{remark}

This defines normative theories about $\allalts$. Any empirical theory for a normative theory about $\allalts$ is said to be an empirical theory about $\allalts$.

Given an empirical theory about $\allalts$ and $a, b \in \allalts$, define $a \gpst b$ iff $\forall \set{a, b} \subseteq B: \phi_{B, ¬b} \in \gPhi$.
...
An empirical theory about $\allalts$ is said to be coherent iff $\forall \emptyset ≠ B \subseteq \allalts$: $¬(\forall a \in B: \phi_{B, ¬a} \in \gPhi) \land ¬(\exists a \in B \suchthat \phi_{B, a} \in \gPhi \land \phi_{B, ¬a} \in \gPhi)$. Coherence is required for validity. Note that it is not an empirical property, in the sense that it can be verified without experiments, as it is independent of $\ileadsto$.
...
When $\gpeq$ is not transitive, it still permits to define choice functions that could be said to respect, in some reasonable sense, the deliberated preference of the individuals. However, this claim is more arguable, as it is generally (though by all means not unanimously) considered in the literature that a reflective preference should be transitive. Further explorations of this possibility are left for future articles.

The claims of an empirical theory about $\allalts$ can sometimes be defined on the basis of a preference ordering $\peq$. In such a case, the claims …
