...
Given $a ≠ b$, define $a \peq b$ iff $\forall \set{a, b} \subseteq B: \phi_{B, a \ind b} \in \gPhi \lor \phi_{B, ¬b} \in \gPhi$.
...
Define $\Phi_\allalts = \set{\phi_{B, \mathcal{C}} \suchthat \emptyset ≠ B \subseteq \allalts, \mathcal{C} \subseteq \powersetz{B}}$.

$\gamma$ gives, for each 
Define $\phi_{(B_k), B}$: $\hist \ileadsto \phi$ iff given $B$, $i$ chooses some $B_{\hist, i} \subseteq B$ such that $\exists B_k \subseteq B_{\hist, i}$.

Define $a \peq b$ iff $\forall \set{a, b} \subseteq B: [\exists \set{a, b} \subseteq B_k \lor (\exists a \in B_k \land \nexists b \in B_k)]$.

Define $a \pst b$ iff $a \peq b \land b \npst a$.
