...
\end{remark}
  ...
  \end{theorem}
  \begin{proof}
    For \cref{it:condHAI} $\implies$ \cref{it:HAI},
    define $p = P_{i_1}$.
  ...
  \begin{remark}
    If there are only two types each with a DJ not empty, then the condition of \cref{it:condHAI} is satisfied. More formally, if ${\allleadsto}(I) = \set{{\ileadsto[i_1]}, {\ileadsto[i_2]}}$ and $\emptyset \notin \pi({\allleadsto})(I)$, then \cref{it:condHAI} is satisfied: define $\ar_1, \ar_2$ as some decisive arguments for $i_1, i_2$, respectively, and define $H = \set{(\ar_1, \ar_2)}$.
  \end{remark}

\begin{remark}
  Given $p \in P$, define $I_p = \set{i \in I \suchthat \pi(i) = p}$.
  If $I \setminus I_p$ all have $\ar$ as a decisive argument, then \cref{it:condHAI} is satisfied: if $I_p \neq \emptyset$, pick $i \in I_p$, let $\ar'$ be a decisive argument for $i$, and define $H = \set{(\ar, \ar')}$; otherwise, define $H = \set{(\ar)}$.
\end{remark}
