...
\end{corollary}
A relation $\allleadstog$ is observably in $C$ iff $\forall \theta \in \Theta: ({\allleadstog} \cap \theta) \in (C^\subseteq \cap \Theta)$.
...
\begin{remark}
  Consider a set $S \wsub \Theta$.
  We can define its border as follows.
  Define a converging sequence in $\Theta$ as a sequence of strictly embedded elements $\Theta$, meaning increasing per strict inclusion (so $\theta_1 \ssub \theta_2 \ssub \theta_3…$), and which include every elements of $\hist × I$ (thus $\cup_i(\theta_i)^{-1}(P) = \hist × I$).
  Such a sequence can be associated to a relation ${\allleadstog} \in \allleadstos$ in the natural way (${\allleadstog} = \cup_i(\theta_i)$).
  Define $s_S$ as the set of such sequences.
  The border of $S$ is the set $\set{\cup_i(\theta_i) \suchthat (\theta_i) \in s_S}$.
  Nope, that’s not useful. That border is empty as soon as $\card{\allargs} ≥ 2$.
\end{remark}
\begin{conjecture}
  Each element of $C$ is a forest (here, meaning, an association of each $i$ to a tree, whose nodes are all the triples in the element and related by “the pair in $\allhists × I$ is a close suffix of the pair in the parent”).
...
\end{proof}
TODO continue here. Use example of constant to $p$ and constant to $p'$ with or without the empty argument and singleton $I$ (falsifiable VS not).
\begin{example}
...
\begin{example}
  Consider any $i$, $\ar_1 ≠ \ar_2 \in \allargs$, $p, p' \in P$.
  The claim $C = \set{{\allleadstog} \suchthat (\ar_1) \ileadstog p \lor (\ar_2) \ileadstog p'}$ is unfalsifiable.

  We have $C^\subseteq \cap \Theta = \set{\theta \suchthat (\theta((\ar_1), i) = \emptyset \lor \theta((\ar_1), i) = p) \lor (\theta((\ar_2), i) = \emptyset \lor \theta((\ar_2), i) = p')}$.
  Observe that any $\theta \in \Theta$ satisfy $\theta((\ar_1), i) = \emptyset \lor \theta((\ar_2), i) = \emptyset$, thus, $C^\subseteq \cap \Theta = \Theta$.
  It follows that the obs-compatible relations are $\allleadstos$, which is bigger than $C$.

  The same holds if replacing $(\ar_1)$ and $(\ar_2)$ by any $\hist, \hist' \in \allhists$ that are not one a prefix of the other one.

  Now consider the smaller claim $C' = \set{{\allleadstog} \suchthat (\ar_1) \ileadstog p}$ (thus $C' \ssub C$).
  It yields a smaller set ${C'}^\subseteq \cap \Theta = \set{\theta \suchthat (\theta((\ar_1), i) = \emptyset \lor \theta((\ar_1), i) = p)}$.
  ...
  from $C \wsub C^{\prime\prime}$, we deduce $\Theta = C^\subseteq \cap \Theta \wsub {C^{\prime\prime}}^\subseteq \cap \Theta$, thus ${C^{\prime\prime}}^\subseteq \cap \Theta = \Theta$ and the obs-compatible relations are $\allleadstos$, which equals $C^{\prime\prime}$ only if $C^{\prime\prime}$ is trivial.

  Now consider the constant functions ${\allleadstog^p}$ to $p$ and ${\allleadstog^{p'}}$ to $p'$ and the alternative smaller claim $C^{(2)} = \set{{\allleadstog^p}, {\allleadstog^{p'}}}$ (thus $C^{(2)} \ssub C$).
  It yields a smaller set ${C^{(2)}}^\subseteq \cap \Theta = \set{\theta \suchthat \theta(\theta^{-1}(P)) = \set{p} \lor \theta(\theta^{-1}(P)) = \set{p'}}$.
  Consider any relation $\allleadstog$ that is obs-compatible with ${C^{(2)}}^\subseteq \cap \Theta$. Assume that for some $\hist, i$, $\hist \ileadstog p$. Then, pick any $j \neq i$, and observe that by obs-compatibility, for any $\hist'$, $\hist' \ileadstog[j] p$.
  ...
  It follows that the obs-compatible relations are $\set{{\allleadstog^p}, {\allleadstog^{p'}}}$, thus, $C^{(2)}$ is falsifiable.
\end{example}
\begin{remark}
