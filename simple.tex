...
\end{remark}
\begin{conjecture}
  Given a non empty set of attitudes $S \wsub P^{\allhists}$, the claim associated to $R(S)$ is not trivial iff $\exists \hist \in \allhists, f \in P^{\intvl{\hist}} \suchthat \forall g \in S: \restr{g}{\intvl{hist}} ≠ f$.
\end{conjecture}
\begin{proof}
  This follows from the equivalence of the antecedent with $\exists f \in P^{\allhists}, \hist \in \allhists \suchthat \cap_{\histp \wsub \hist} l^S_f(\histp) = \emptyset$.
\end{proof}
Thus, a situation $S$ does not admit falsifiable theories (meaning that $R(S)$ is associated to the trivial claim) iff $\forall \hist, f, \exists g \in S \suchthat \restr{g}{\intvl{\hist}} = f$.
