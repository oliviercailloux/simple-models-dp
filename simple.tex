...
\begin{definition}
  ...
  Define the extension of a claim about types $C^\text{types} \wsub P^{\allhists}$ to its observably included attitudes as the the claim about types $R(C^\text{types}) = \set{g \in P^{\allhists} \suchthat \forall H \in \finite(\incr(\allhists)): \exists f \in C^\text{types} \suchthat \restr{g}{H} = \restr{f}{H}}$.
  Note that $C^\text{types} \wsub R(C^\text{types})$. 
  Also note that $S_1 \wsub S_2 \implies R(S_1) \wsub R(S_2)$.

  Define the closure under observable inclusion of a claim about types $C^\text{types}$ as $R^\infty(C^\text{types})$, equivalently, as the smallest set $C^\text{types}_\text{ind}$ such that $R(C^\text{types}_\text{ind}) \wsub C^\text{types}_\text{ind}$.
...
\begin{conjecture}
  ...
  An attitude is observably included in a set of attitudes $S$ iff $\forall \hist \in \allhists: \cap_{\histp \wsub \hist} l^S_f(\histp) \neq \emptyset$.
\end{conjecture}
\begin{proof}
...
\begin{remark}
  ...
  TODO
  In that case, if restricting our focus to claims about types of individuals, we need to include the real types, then close under suitable indistinguishability relation (observable inclusion), so as to include for example $i$.
...
\begin{lemma}
  ...
  satisfies $\forall g \in P^{\allhists} \setminus C^\text{types}: \exists H \in \finite(\incr(\allhists)) \suchthat \forall f \in C^\text{types}: \restr{g}{H} \neq \restr{f}{H}$,
...
\begin{theorem}
  If a claim about types of individuals $C^\text{types} \wsub P^{\allhists}$ is not closed under observable inclusion, then it is not falsifiable and not trivial.
\end{theorem}
\begin{theorem}
...
\begin{theorem}[To be deleted, probably]
  ...
  Consider a claim about types of individuals containing all those ones. Assume that for some attitude $i$ not in the claim, no $H$ permit to observably exclude $i$ from the claim. Then from the lemma above, the claim is not falsifiable. So, we need to include all attitudes that no $H$ permit to observably exclude from the claim. Iterating, this is the closure under the observable inclusion relation.
  So, the claim about types must include the real ones and be closed under observable inclusion.
\end{theorem}
\begin{theorem}
