...
\begin{remark}
	I’d rather drop the negation and observe that if the protocol is defined so that $\hist \ileadsto \phi_1 ⇒ \hist \nileadsto \phi_2$, then $\phi_1 \in \iPhi ⇒ \phi_2 \notin \iPhi$. Observe that this is always possible. The definitions about completeness should use a negation operator in supplement to the normative theory tuple.
\end{remark}
...
Given $i \in I, \phi \in \gPhie, \ar \in \allargs$, define $\di(\ar) \subseteq \powersetz{\alldargs}$ as $\di(\ar) = \set{\dbeatsinv(\args) \suchthat \emptyset ≠ \args \subseteq \gbeatsinv(\ar) \land \args \ibeats \ar}$. It represents the coalitions of fragments defending $\ar$ (when arguing for $\phi$), from the point of view of $i$, against its effective planned attackers. 
The following proposition shows that these coalitions can be added to $\ar$, and will be useful for other results.
\begin{proposition}
	\label{th:indf}
	$\forall i \in I, \phi \in \gPhie, \ar \in \allargs: \dargs \in \di(\ar) ⇒ \emptyset ≠ \dargs \subseteq \df(\ar)$.
\end{proposition}
\begin{proof}
...
Given $i \in I, \phi \in \gPhie$, define $\gargs \subseteq \allargs$ as the transitive closure of ${\gleadstoinv}(\phi)$ under $\dip$: $\gargs = \cup_{k \in \N} (\dip)^k({\gleadstoinv}(\phi))$.

\begin{definition}[Convincingness]
	An empirical theory is \emph{convincing} iff $\forall i \in I, \phi \in \gPhie, \ar \in \gargs: \ar$ $i$-defends $\phi$.
\end{definition}

A \emph{falsification instance} of an empirical theory is a tuple $(i, \phi, \hist, \ar) \in I × \gPhie × \allhist × \gargs$ such that $\gbeatsinv(\ar) \cap \hist = \emptyset \land \ar \in \histend \land \hist \nileadstost \phi$. 
...
\end{theorem}
\begin{proof}
	Given any $i \in I$ and $\phi \in \gPhie$, suffices to show that for some $\ar \in \allargs$, $\ar \ileadstost \phi$; the conclusion then follows thanks to simple normative adequacy (that \cref{th:normeqsnorm} shows to be equivalent to normative adequacy). 
...
Given $\phi \in \Phi$ and two empirical theories $\gamma$ and $\delta$ for a given normative theory, say that they disagree about $\phi$ iff $\phi \in \gPhie[\gamma] \land ¬\phi \in \gPhie[\delta]$. Say that they are incompatible iff there is at least one $\phi \in \Phi$ that they disagree about.
...
Define $\Phi_\allalts = \set{\phi_{B, \choices} \suchthat \emptyset ≠ B \subseteq \allalts, \choices \subseteq \powersetz{B}}$.
...
Define $\hist \ileadsto \phi_{B, \choices}$ iff $c^\hist_i(B) \in \choices$.

This defines normative theories about $\allalts$. Any empirical theory for a normative theory about $\allalts$ that satisfies $\forall \emptyset ≠ B \subseteq \allalts: \exists ! \emptyset ≠ \choices \subseteq \powersetz{B} \suchthat \phi_{B, \choices} \in \gPhie$ is said to be an empirical theory about $\allalts$. The notation $\exists!$ means: there exists a unique element. The requirement that there is at least one is innocuous, as choosing $\choices = \powersetz{B}$ ensures that $\phi_{B, \choices} \in \iPhi$, by definition of $\ileadsto$.

An empirical theory about $\allalts$ thus associates to each possible bundle $B$ a set of possible choices $\choices$, whose semantics is that the theory claims that the deliberated choice of $i$, given the bundle $B$, lies among $\choices$. Let $\gPhi(B) \subseteq \powersetz{B}$ denote this set of possible choices, thus $\gPhi(B) = \choices  ⇔ \phi_{B, \choices} \in \gPhie$.

Given an empirical theory about $\allalts$ and $a, b \in \allalts$, define $a \gpst b$ iff $\forall \set{a, b} \subseteq B: b \notin \bigcup \gPhi(B)$. Thus, $a \gpst b$ iff $\gamma$ claims that $b$ is not deliberately chosen in presence of $a$.
...
$\exists \phi_{B, \choices} \in \iPhi \suchthat \forall C \in \choices: C \cap B' ≠ \emptyset$.
...
\begin{proof}
	...
	That $\gpst$ is asymmetric is seen by considering, given any $a, b \in \allalts$, the bundle $B = \set{a, b}$: pick any $C \in \gPhi(B) \subseteq \powersetz{B}$, thus, $C \in \powersetz{B}$, hence, $a \in C$ or $b \in C$, and therefore, $b \ngpst a$ or $a \ngpst b$.
	...
	$\exists \phi_{B, \choices} \in \iPhi \suchthat \forall C \in \choices: C \cap B' ≠ \emptyset$.
%	$[\forall a \in B \setminus B': \phi_{B, ¬a} \in \iPhi] \lor [\exists a \in B' \suchthat \phi_{B, a} \in \iPhi]$. 
	Because $\gamma$ is valid, suffices to show that, given $\choices = \gPhi(B) \subseteq \powersetz{B}$, 
	$\forall C \in \choices: C \cap B' ≠ \emptyset$.
%	$[\forall a \in B \setminus B': \phi_{B, ¬a} \in \gPhie] \lor [\exists a \in B' \suchthat \phi_{B, a} \in \gPhie]$.
...
\begin{proof}
	...
	Consider the bundle $B = \set{a, b, c}$ and let $\choices = \gPhi(B)$.
...
In what follows, the claims of an empirical theory about $\allalts$ will be defined on the basis of a transitive preference ordering $\peq$ on $\allalts$, as follows. Given $\emptyset ≠ B \subseteq \allalts$, define $\gPhi(B) = \set{\cup \choices' \suchthat \emptyset ≠ \choices' \subseteq \choices}$, where $\choices$ are the undominated equivalence classes in $B$ according to $\peq$ (thus $\choices = \max_{\pst^*}(B / {\ind})$, using the notations defined in \cref{th:dp}). The check that ${\peq} = {\gpeq}$ is left to the reader. Thus, all the examples that follow have a transitive $\gpeq$ and therefore claim to represent a deliberated preference (when valid) in the sense defined in this section.
...
The theory $\gamma$ claims, intuitively, that individuals $i \in I$ are satisficers using some defined criteria $C$ and thresholds $t = (t_c)_{c \in C}$. Define $\peq$ as $a \peq b$ iff $a \in A \lor b \notin A$. Note that $\peq$ is complete and contains at most two equivalence classes: $A$, and $\allalts \setminus A$. This defines the set of claims that $\gamma$ supports, namely, for any $\emptyset ≠ B \subseteq \allalts$, $\gPhi(B) = B \cap A$.
...
Define ${\gleadsto} = \set{(\ar_B, \gPhi(B)) \suchthat \emptyset ≠ B \subseteq \allalts}$.
...
Define $\allprops = \set{\phi, ¬\phi}$, $\phi$ representing intuitively the case that $i$ deliberately prefers $P$ to $D$, in the sense of bidding at least as much for $P$ than for $D$ after deliberation, and $¬\phi$, the opposite.

Define $\ileadstost$ as follows. Given $\hist = (…, \ar)$, $\hist \ileadstost \phi$ iff, in a situation of $i$ having been exposed to these arguments, saying: “Considering these arguments, would you please indicate your maximum selling price for $P$, and for $D$” leads $i$ to bid at least as much for $P$ than for $D$. $\hist \ileadstost ¬\phi$ is defined similarly.
%Note that $\hist \ileadstost \phi$ iff $\hist \nileadstost ¬\phi$.
...
A transitive empirical preference theory about $\allalts$ is an empirical theory for a normative theory about $\allalts$ such that $\gPhie$ is transitive.

Associate to such a theory a preference ordering $\succ$ on $\allalts$ as follows: $a \succ b$ iff $\ppstsure \in \gPhie$.
...
\begin{definition}[Convincingness]
	$\forall i \in I, \phi \in \gPhie, \exists k \in 2\N$ and a finite sequence $(\args_j)_{j \in 2\N, j ≤ k} \suchthat \emptyset ≠ \args_0 \subseteq {\gleadstoinv}(\phi)$ and $\forall j \in 2\N, j ≤ k, \forall \ar_j \in S_j: \ar_j$ $i$-defends $\phi$ under protection of $S_{j + 2}$ (defining $S_{k + 2} = \emptyset$) and $S_j \subseteq \cup_{\ar_{j - 2} \in \args_{j - 2}, \ar_{j - 1} \in \allargs} f(\ar_{j - 1}, \ar_{j - 2})$.
\end{definition}
...
\end{theorem}
\begin{proof}
	Consider $i \in I$ and $\phi \in \gPhie$. 
