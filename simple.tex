...
\begin{definition}[Answerability]
	$\forall \phi \in {\gleadsto}(\allargs), \ar_0 \in \gargs[\phi], \ar_1 \in \allargs: (\ar_1, \ar_0) \ileadstoe ¬\phi ⇒ [\exists \ar_1' \in {\gbeatsstarinv}(\ar_0) \suchthat (\ar_1', \ar_0) \ileadstoe ¬\phi]$.
\end{definition}
...
\begin{definition}[No self-attack]
	$\gbeats$ must be irreflexive.
\end{definition}

\begin{definition}[Operational validity]
	A theory is \emph{operationally valid} iff $\forall \phi \in {\gleadsto}(\allargs), \ar_0 \in \gargs[\phi], \ar_1 \in \allargs: (\ar_1, \ar_0) \ileadstoe ¬\phi ⇒ \exists \ar_2 \in {\gbeatsinv}(\ar_1) \suchthat (\ar_0 + \ar_2, \ar_1) \ileadstoe \phi$.
\end{definition}

\subsection{Theorem}
\begin{theorem}
	If a theory is answerable, has bounded length, does not self-attack, and is operationally valid, it is valid.
\end{theorem}
\begin{proof}
	...
	Pick any $\phi \in {\gleadsto}(\allargs)$ and $\ar[k] \in \gargs[\phi]$ such that $\ar[k] \nileadstost \phi$. We know that $\exists \ar_1 \in \allargs \suchthat \ar_1 \ibeatse[!\phi] \ar[k]$. Therefore, by Answerability, $\exists \ar_1' \in \allargs \suchthat \ar_1' \gbeatsstar \ar[k] \land \ar_1' \ibeatse[!\phi] \ar[k]$, and, by Operational validity, $\exists \ar_2 \in \gargs[\phi] \suchthat \ar_2 \gbeats \ar'_1$ and $\ar[k] + \ar_2 \ibeatse[\phi] \ar'_1$. 
	Define $\ar[k + 1] = \ar_2 + \ar[k]$. 
	
	\commentOC{Note that we do not have $\ar_1 \gbeats \ar[k]$.}
	By No self-attack, $¬(\ar[k] \gbeatsstar \ar'_1)$. Thus, $\ar_2 \notin c(\ar[k])$ (because $\ar_2 \in c(\ar[k])$ and $\ar_2 \gbeats \ar'_1$ implies $\ar[k] \gbeatsstar \ar'_1$ by definition of $\gbeatsstar$).
