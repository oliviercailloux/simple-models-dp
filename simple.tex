...
Let the \emph{simple a priori observations} be defined as $(\allhist)^I$.
...
Any relation $\allleadstog$ is equivalent to $\cup \set{\restr{\allleadstog}{O} \suchthat O \in (\incr(\allhist))^I}$.
Conversely, any set $S$ such that $\set{\theta^{-1}(P) \suchthat \theta \in S} = \incr(\allhist)^I$ and $\cup S$ is right-unique corresponds to a unique relation $\cup S = {\allleadstog}$.
Thus, there is a bijection between the set of such sets $S$ and the set of leads to relations.
We can then rephrase the above theorem as follows.
A claim $C$ is falsifiable iff $\forall S \in \mathcal{S}: S \subseteq C^\subseteq \implies \cup S \in C$.
Equivalently, as follows.
\begin{theorem}
  A claim $C$ is falsifiable iff $\forall {\allleadstog} \in P^{I × \allhist}: \set{\restr{\allleadstog}{O} \suchthat O \in (\incr(\allhist))^I} \subseteq C^\subseteq \implies {\allleadstog} \in C$.
\end{theorem}
\begin{proof}
  ...
  The theorem holds because $\forall {\allleadstog} \in P^{I × \allhist}: \set{\restr{\allleadstog}{O} \suchthat O \in (\incr(\allhist))^I} = \set{θ \subseteq {\allleadstog}}$.
\end{proof}
