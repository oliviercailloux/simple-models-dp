...
Given $i \in I, \phi \in \Phi, \ar_0 \in \allargs$ and $\args \subseteq \gbeatsinv(\ar_0)$, define $\args \ibeats \ar_0$ iff $\exists \hist \in \allhist \suchthat \args = \gbeatsinv(\ar_0) \cap \hist \land \ar_0 \in \histend \land \hist \nileadstost \phi$. 

Given $i \in I, \phi \in \gPhi, \ar \in \allargs$, define the \emph{$\phi$-defensors of $\ar$ required for $i$}, $\di(\ar) \subseteq \powersetz{\alldargs}$, as $\di(\ar) = \set{\dbeatsinv(\args) \suchthat \emptyset ≠ \args \subseteq \gbeatsinv(\ar_0) \land \args \ibeats \ar_0}$. 
...
A \emph{falsification instance} of an empirical theory is a tuple $(i, \phi, \hist) \in I × \gPhi × \allhist$ such that $\exists \ar \in (\histend \cap \gargs) \suchthat \gbeatsinv(\ar) \cap \hist = \emptyset \land \hist \nileadstost \phi$.

\begin{theorem}
	An empirical theory is convincing iff it has no falsification instances.
\end{theorem}
\begin{proof}
	For all $i \in I, \phi \in \gPhi$: $\exists \ar \in \gargs$ that does not $i$-defend $\phi$ iff $\exists \ar \in \gargs, \hist \in \allhist \suchthat \left[\ar \in \histend \land \gbeatsinv(\ar) \cap \hist = \emptyset \land \hist \nileadstost \phi\right]$ iff $\exists \hist \in \allhist$ such that $(i, \phi, \hist)$ is a falsification instance.
\end{proof}
...
If $\gamma$ and $\delta$ agree about all $\phi$, there is nothing to test as the theories are compatible. Otherwise, here is a test procedure that falsifies at least one of these theories, given any $\phi \in \Phi$ about which they disagree and any $i \in I$.
...
Lemma (variant of a previous one). $\ar_0$ $i$-defends $\phi$, and $\hist \nileadstost \phi$ with $\ar_0 \in \histend$, thus $\dargs = \dbeatsinv(\gbeatsinv(\ar_0) \cap \hist) \in \di(\ar_0)$, thus $\ar_2 = \ar_0 + \dargs \in \gargs \cap \dfp(\ar_0)$.

Consider $(…, \ar_0) \ileadstost \phi$ and $\hist = (…, \ar_0, \ar_{-1}) \nileadstost ¬\phi$, with $\ar_0 \in \gargs, \ar_{-1} \in \gargs[¬\phi]$. By the lemma (applied on $\ar_{-1}$), $\ar_1 = \ar_{-1} + \dbeatsinv(\gbeatsinv(\ar_{-1}) \cap \hist) \in \gargs[¬\phi] \cap \dfp(\ar_{-1})$. Iterating if necessary, we obtain $(…, \ar_1) \ileadstost ¬\phi$. Now either already the previous step was $(…, \ar_0, \ar_1)$, or we test $(…, \ar_1, \ar_0)$. If $(…, \ar_1, \ar_0) \ileadstost \phi$, apply the lemma again on $\ar_1$, and go on. Finally: $\hist = (…, \ar_1, \ar_0) \nileadstost \phi$. Similarly, then, obtain $\ar_2 = \ar_0 + \dargs \in \gargs \cap \dfp(\ar_0)$. If $\ar_0 \in (\dfp)^k(…)$, then $\ar_2 \in (\dfp)^{k'}(…)$, with $k' > k$. And $(…, \ar_2, \ar_1) \nileadstost ¬\phi$, so we can apply the strating step again.
