...
The deliberated judgment of $i$ maps any pair $(\prop, ¬\prop)$ to one of four possibilities: $\prop$ is sure, $¬\prop$ is sure, both are possible, or undecided. $\prop$ is sure iff $\exists \ar \ \propsuredec$; both are possible iff $\exists \ar \ \proppossdec$ and $\exists \ar \ ¬\proppossdec$; and undecided is when none of the other three cases hold.
For easier treatment, we summarize $i$’s deliberated judgment in a single set $\iPhi \subseteq \Phi$, as follows: $(\prop, \text{possible}) \in \iPhi ∧ (¬\prop, \text{possible}) \in \iPhi ∧ (\prop, \text{sure}) \notin \iPhi ∧ (¬\prop, \text{sure}) \notin \iPhi$ iff $(\prop, ¬\prop)$ maps to both possible; $(\prop, \text{possible}) \in \iPhi ∧ (\prop, \text{sure}) \in \iPhi ∧ (¬\prop, \text{possible}) \notin \iPhi ∧ (¬\prop, \text{sure}) \notin \iPhi$ iff $(\prop, ¬\prop)$ maps to $\prop$ is sure, $\phi \notin \iPhi$ for all $\phi$ about $\prop$ iff $(\prop, ¬\prop)$ maps to undecided.
...
Observe that (thanks to A2) $(\prop, \text{possible}) \in \iPhi$ iff $\exists \ar \ \proppossdec ∧ (\exists \ar \ \propsuredec ∨ \exists \ar \ ¬\proppossdec)$.

\begin{proposition}
	a) $\phi \in \iPhi ⇒ ¬\phi \notin \iPhi$. b) $(\prop, ¬\prop)$ maps to undecided $⇔ \phi \notin \iPhi ∧ ¬\phi \notin \iPhi$ for some $\phi$ about $\prop$. c) If $\phi \in \iPhi$ for some $\phi$ about $\prop$, then $\phi \in \iPhi ⇔ ¬\phi \notin \iPhi$ for all $\phi$ about $\prop$.
\end{proposition}
\begin{proof}
	a) is because some $\ar^+$ is then $\phidec$, thus given any $\ar^−$, $(\ar^+, \ar^−) \ileadstost \phi$ hence $(\ar^+, \ar^−) \ileadstoe \phi$ (using \cref{ft:sttoe}).
\end{proof}
