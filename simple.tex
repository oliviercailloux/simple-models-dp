...
It is desirable to check that \cref{ax:nocontr,ax:norm} do not hide a requirement on the content of $\ileadstostp$, thus, that they are together satisfiable without constraining $\ileadstostp$. To define this more precisely, consider temporarily these axioms as properties of a set $\iPhi \subseteq \Phi$. I write $A^1(\iPhi)$ if “axiom” 1 (interpreted as a property of $\iPhi$) holds on $\iPhi$, and similarly for $A^2(\iPhi)$. The required check amounts to prove that $\exists \iPhi \subseteq \Phi \suchthat A^1(\iPhi) \land A^2(\iPhi)$. 
...
\begin{proposition}[Restricted negation coherence satisfaction]
	Under \cref{ax:nocontr,ax:norm}, all models satisfy Restricted negation coherence.
\end{proposition}
\begin{proof}
	This follows from \cref{th:intnegcoh}.
\end{proof}
...
From all this, hopefully, I can justify my normative axiom; by justifying its form $\ar \ileadstost[undef] \phi ⇒ \phi \in \iPhi$ in terms of a yet-undefined $\ileadstost[undef]$, then by proving that $\ileadstost[undef]$ must satisfy Protocol coherence (applied to undef instead of the current one) and Certainty implies possibility.
...
\end{definition}
Defining $\ar \ileadstoe[undef] \phi$ iff $\ar \nileadstost[undef] ¬\phi$, the following should adequately replace Coherent protocol.
\begin{definition}[Always implies sometimes]
	\label{cl:ais}
	${\ileadstost[undef]} \subseteq {\ileadstoe[undef]}$.
\end{definition}
