...
\subsection{Classify propositions (mix attempt)}
Define $p \ileadstostw \phi$ iff $p \nileadstoe ¬\phi$.

\begin{axiom}[Normative adequacy of $\allargs$ and $\ileadstoe$ mix 1]
	$\forall \phi \in \Phi$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \nileadstoe ¬\phi] ⇒ \phi \in \iPhi.
	\end{equation}
\end{axiom}
\begin{axiom}[Normative adequacy of $\allargs$ and $\ileadstoe$ mix 2]
	$\forall \phi \in \Phi$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \nileadstoe ¬\phi] ⇒ ¬\phi \notin \iPhi.
	\end{equation}
\end{axiom}
These two axioms imply that not $(\ar \nileadstoe ¬\phi \land \ar' \nileadstoe \phi)$ (a form of coherent complement); equivalently, not $(\ar \ileadstostw \propsure \land \ar' \ileadstostw \notpropposs)$.

\commentOC{This is unsatisfactory because of a mix between normative and empirical assumptions; and because an axiom should be true, not empirically falsifiable. Also, if the protocol is such; then we should simply weaken the conclusion, not have to conclude that the axioms are false.}

To avoid risk of empirical falsification, we can weaken the axioms. Also, the first axiom seems too strong.

Recall that $\ar \ileadstoe \phi$ means $\exists \ar_1 \suchthat (\ar_1, \ar) \ileadstoe \phi$.

\begin{axiom}[Normative adequacy 1, weaker]
	$\forall \prop \in \allprops$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \nileadstoe \notpropposs \land \ar \ileadstoe \propsure \land \ar \nileadstoe \notpropsure] ⇒ \propsure \in \iPhi;
	\end{equation}
	$\forall \prop \in \allprops$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \nileadstoe \propsure] ⇒ \notpropposs \in \iPhi.
	\end{equation}
\end{axiom}
\begin{axiom}[Normative adequacy 2, weaker]
	$\forall \prop \in \allprops$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \nileadstoe \notpropposs \land \ar \ileadstoe \propsure \land \ar \nileadstoe \notpropsure] ⇒ \notpropposs \notin \iPhi;
	\end{equation}
	$\forall \prop \in \allprops$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \nileadstoe \propsure] ⇒ \propsure \notin \iPhi.
	\end{equation}
\end{axiom}

This version is still too strong: never observing $\propsure$ should not suffice to reject $\propsure$. Rejecting $\propsure$ is not only about observing that no decisive argument leads to it: it is about positively proving that $\notpropposs$ (that is, with a decisive argument). That is, if nothing is decisive in $\allargs$, the axioms should say nothing rather than claiming that $\propsure$ is rejected. This makes the required axioms weaker, thus, is good. And we don’t lose much as, under indecisiveness, there is no short proof of rejection of $\propsure$. It is also simpler, as it rejects $\propsure$ exactly when it accepts $\notpropposs$ exactly when some short proof of it exists.

Another problem is that this still asks the question whether these axioms could be empirically falsified. On the contrary, the axioms should obviously mandate nothing empirical. They should be weaker, and the resulting equivalent statement, looking like those above, should be obtained as a result.

\subsection{Classify propositions, mandating positive observations}
Define $(\ar_1, \ar) \ileadstostp \phi$ iff $(\ar_1, \ar) \nileadstoe ¬\phi \land (\ar_1, \ar) \ileadstoe \phi$. Define $\ar \ileadstostp \phi$ iff $\forall \ar_1 \in \allargs: (\ar_1, \ar) \ileadstostp \phi$.

\begin{axiom}[Normative adequacy 1, positive]
	$\forall \prop \in \allprops$: 
	\begin{equation}
	...
	$\forall \prop \in \allprops$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \ileadstostp \notpropposs] ⇒ \notpropposs \in \iPhi.
	\end{equation}
\end{axiom}
\begin{axiom}[Normative adequacy 2, positive]
	$\forall \prop \in \allprops$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \ileadstostp \propsure \land \ar \ileadstostp \propposs] ⇒ \notpropposs \notin \iPhi;
	\end{equation}
	$\forall \prop \in \allprops$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \ileadstostp \notpropposs] ⇒ \propsure \notin \iPhi.
...
Problem: we should deal with accepting and rejecting possibilities only. Thus, proving $\propsure$ only serves to reject $\notpropposs$.

\subsection{Classify propositions, only two implications}
\begin{axiom}[Normative adequacy in two implications]
	$\forall \prop \in \allprops$: 
	\begin{equation}
...
Given a model, define $\Box\Phi \subseteq \Phi$ as 
...
\begin{property}[Model completeness]
	$\forall \phi \in \Phi: \phi \in \Box\Phi \lor ¬\phi \in \Box\Phi$.
\end{property}
\begin{property}[Deliberation completeness]
	$\forall \phi \in \Phi: \phi \in \iPhi \lor ¬\phi \in \iPhi$.
\end{property}
\begin{proposition}
	Under Normative adequacy, Model completeness implies Deliberation completeness.
\end{proposition}
Models as defined here do not mandate Model completeness, and in fact, even the weaker Deliberation completeness is not mandated. This is good for two reasons. First, Deliberation completeness can appear too strong. Second, it is dubious that, for non trivial decision problems, models can be found for which both Normative adequacy is normatively compelling and Model completeness holds.

\subsection{Classify propositions, draft}
The notations $\iacc$ and $\irej$ designate the subsets of $\Phi$ whose propositions $i$ deliberately accepts and rejects, respectively.

\begin{axiom}[Normative adequacy of $\allargs$ and $\ileadstost$ 1]
	$\forall \phi \in \Phi$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \ileadstost \phi] ⇒ \phi \in \iacc.
	\end{equation}
\end{axiom}
\begin{axiom}[Normative adequacy of $\allargs$ and $\ileadstost$ 2]
	$\forall \phi \in \Phi$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \ileadstost \phi] ⇒ \lnot\phi \in \irej.
	\end{equation}
\end{axiom}

\begin{axiom}[Normative adequacy of $\allargs$ and $\ileadstost$ 1 weaker]
	$\forall \phi \in \Phi$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \ileadstost \propsure] ⇒ \propsure \in \iacc.
	\end{equation}
\end{axiom}
\begin{axiom}[Normative adequacy of $\allargs$ and $\ileadstost$ 2 weaker]
	$\forall \phi \in \Phi$: 
	\begin{equation}
		[\exists \ar \in \allargs \suchthat \ar \ileadstost \propposs] ⇒ \notpropsure \in \irej.
	\end{equation}
\end{axiom}

I suppose I can’t prove this.
\begin{property}[Coherent complement]
	$\iacc \cap \irej = \emptyset$.
\end{property}
...
I certainly want this (equivalent to coh co restricted to the props determined).
\begin{property}[Desired? 1]
	$\phi \in \mPhi ⇒ ¬\phi \notin \mPhi$.
\end{property}
\begin{property}[Desired? 2]
	$\propsure \in \mPhi ⇒ \propposs \in \iacc$.
\end{property}
...
\begin{property}[Exhaustivity]
	$\forall \phi \in \Phi: \phi \notin \iacc ⇒ \phi \in \irej$.
\end{property}
Exhaustivity can be considered a strong demand, as it requires every proposition to be deliberately accepted or rejected. \commentOC{I’m not so sure: could consider that $t$ is merely possible when not determined.}

Define the propositions whose status is determined as $\idet = \iacc \cup \irej$.

\begin{property}[Suppl]
	$\forall \prop \in \allprops: \propsure \notin \iacc ⇒ \propsure \in \irej$.
\end{property}
\begin{property}[Negation coherence]
	\label{prop:cohb}
	$\forall \phi \in \Phi: \phi \in \iacc ⇔ ¬\phi \in \irej$, and $\forall \prop \in \allprops: \propsure \in \iacc ⇒ (¬\prop, \text{sure}) \in \irej$.
\end{property}
This property entails the following one.
\begin{property}[Per-proposition coherence]
	$\forall \prop \in \allprops$, exactly one of the following four propositions is true: 
	\begin{itemize}
		\item $\propsure \in \iacc \land \propposs \in \iacc \land (¬\prop, \text{sure}) \in \irej \land (¬\prop, \text{poss}) \in \irej$;
		\item $\propsure \in \irej \land \propposs \in \iacc \land (¬\prop, \text{sure}) \in \irej \land (¬\prop, \text{poss}) \in \iacc$;
		\item $\propsure \in \irej \land \propposs \in \irej \land (¬\prop, \text{sure}) \in \iacc \land (¬\prop, \text{poss}) \in \iacc$;
		\item $\set{\prop, ¬\prop} × \set{\text{sure}, \text{poss}} \subseteq \Phi \setminus \idet$;
	\end{itemize}
\end{property}
\begin{proof}
	If $\propsure \in \iacc$, then $\notpropsure \in \irej$, thus $\propposs \in \iacc$ as $\notpropsure = ¬\propposs$ and $\phi \in \iacc ⇔ ¬\phi \in \irej$; and finally, $\notpropposs \in \irej$ as $\notpropposs = ¬\propsure$ and $\phi \in \iacc ⇔ ¬\phi \in \irej$.
	
	If $\propsure \notin \iacc$ and $\propposs \in \iacc$, then?
\end{proof}

This property in turn entails the following one.
\begin{property}[Per-proposition determination]
	$\forall \prop \in \allprops: [\set{\prop, ¬\prop} × \set{\text{sure}, \text{poss}} \subseteq \idet] \lor [\set{\prop, ¬\prop} × \set{\text{sure}, \text{poss}} \subseteq \Phi \setminus \idet]$.
\end{property}
\commentOC{There appears to be no reason to want per-proposition determination, even at the mere model level. I could have a model say that $\propposs$ should be accepted, but have nothing to say about whether $\propsure$ should as well, or $\notpropposs$ is okay as well.}

\Cref{prop:cohb} can’t be proven, but the following weaker versions can: $\forall \phi \in \Phi: \phi \in \mPhi ⇒ ¬\phi \in \irej$ and $\forall \phi \in \Phi: \propsure \in \mPhi ⇒ ¬\propsure \in \irej$.

\begin{property}[Per-proposition weaker coherence bis]
	$\forall \phi \in \Phi: \phi \in \mPhi ⇒ ¬\phi \notin \mPhi$.
\end{property}

A proposition $\phi$ has a determined status iff $\phi \in \mPhi \lor ¬\phi \in \mPhi$. (Not both?)

\begin{property}[Coherent determination]
...
\begin{definition}[Coherent protocol]
...
\end{theorem}
\begin{proof}
	Coherent complement is equivalent to $\forall \phi \in \phi: ¬(\phi \in \iacc \land \phi \in \irej)$.
	Coherent protocol is equivalent to $\forall \phi \in \Phi: ¬(\ar \ileadstost \phi \land \ar \ileadstost ¬\phi)$, which implies (using Norm ad 1 and 2) $\forall \phi \in \Phi: ¬(\phi \in \iacc \land \phi \in \irej)$.
\end{proof}
...
\end{theorem}
\begin{proof}
	This is false under the current definitions: if $\ileadstost = \emptyset$, coherent protocol, but $\phi \in \iacc \land \phi \in \irej$ possible.
\end{proof}

\begin{axiom}[Coherent negation]
	$\forall \phi \in \Phi: \phi \in \iacc ⇔ ¬\phi \in \irej$.
\end{axiom}

\begin{proposition}
	$\forall \ar \in \allargs: \ar \ileadstost \propsure ⇒ \ar \ileadstost \propposs$.
\end{proposition}
\begin{proof}
	By hypothesis, $\forall \ar_1 \in \allargs: (\ar_1, \ar) \ileadstost \propsure$, which implies, by definition of $\ileadstost$, that $\forall \ar_1 \in \allargs: (\ar_1, \ar) \nileadstoe (¬\prop, \text{sure})$. Therefore, $\ar \ileadstost \propposs$.
\end{proof}

This notation (writing $\phi \in \irej$ as equivalent to $\phi \notin \iacc$) is justified only if it is impossible that both $\phi \in \iacc$ and $\phi \notin \iacc$ be implied (thus, if Coherent complement). \commentOC{TODO clarify.} The following conditions permit to obtain this result. \commentOC{Cond. 1 is stronger than necessary: we could have ¬cond. 1 and still Coh compl: $(\ar_2, \ar_1) \ileadstost \phi$ and $(\ar_2, \ar_1) \ileadstost ¬\phi$ without problems if $\ar_2$ and $\ar_1$ are not decisive. In fact, $(\ar_2, \ar_1) \ileadstoe ¬\phi$ should be defined as $(\ar_2, \ar_1) \nileadstost \phi) \lor (\ar_2, \ar_1) \ileadstost ¬\phi$, just in case a strange protocol allows both leading always to $\phi$ and $¬\phi$, and I think the rest goes through.}

\begin{condition}[Always implies sometimes]
	\label{cl:ais}
	${\ileadstost} \subseteq {\ileadstoe}$.
\end{condition}

\begin{condition}[Sure implies possible]
	\label{cl:sip}
...
\begin{definition}[Claiming certainty also claims possibility]
	$\forall \ar_0 \in \allargs: \ar_0 \gleadsto (\prop, \text{sure}) ⇒ \ar_0 \gleadsto (\prop, \text{poss})$.
\end{definition}

\begin{proposition}[Arguments for certainty also argue for possibility]
...
\begin{proof}
	...
	We want to prove, first, that $(\ar_1, \ar[k]) \ileadstoe ¬\phi$. If $\phi = (\prop, \text{poss})$, that is clear from the definition of $\ileadstost$. If $\phi = (\prop, \text{sure})$, 
	
	\commentOC{With new definition of $\ileadstost$, when $\phi = (\prop, \text{sure})$, from $\ar \nileadstost \phi$ we only obtain that $\ar \ileadstoe ¬\phi \lor …$, thus, we still need to show that $\ileadstoe (\prop, \text{sure})$ and that $\nileadstoe (¬\prop, \text{sure})$.}
	We know that $\exists \ar_1 \in \allargs \suchthat \ar_1 \ibeatse[!\phi] \ar[k]$. Therefore, by Answerability, $\exists \ar_1' \in \allargs \suchthat \ar_1' \gbeatsstar \ar[k]$, and, by Operational validity, $\exists \ar_2 \in \gargs[\phi] \suchthat \ar_2 \gbeats \ar'_1$. 
	Define $\ar[k + 1] = \ar[k] + \ar_2$. 
	By No repetition, $\ar_2 \notin c(\ar[k])$.
	Thus, $\card{c(\ar[k + 1])} ≥ \card{c(\ar[k])} + 1$.
...
\begin{definition}[No self-attack]
