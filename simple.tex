...
${\dbeats} \subseteq \alldargs × \allargs$ and is injective (thus $\forall \ar \in \allargs: {\dbeatsinv}(\ar)$ is a singleton or empty). 
...
This can also be written: $\ar_0 \nileadstosti \phi ⇔ \exists \hist \in \allhist, \ar_1 \in \allargs \suchthat \set{\ar_1, \ar_0} \subseteq \histend \land \hist \nileadstosti \phi$. By definition of ${\ibeatsi}$, this is the proposition to be proven.
\end{proof}

Given $i \in I, \phi \in {\gleadsto}(\allargs), \ar \in \allargs$, define the \emph{$\phi$-defensors of $\ar$ required for $i$}, $d^{i, \phi}(\ar) \subseteq \alldargs$, as $d^{i, \phi}(\ar) = [{\dbeatsinv} \circ ({\gbeatsinv} \cap {\ibeatsiinv})](\ar)$.
%Given $i \in I, \phi \in {\gleadsto}(\allargs), \ar \in \allargs, \ard \in \df(\ar)$, define the \emph{successors} of $(\ar, \ard)$ as $\sigma^{i, \phi}(\ar, \ard) = \set{\ar + \ard} × d^{i, \phi}(\ar + \ard)$. 
Given $i \in I, \phi \in {\gleadsto}(\allargs)$, define $\gargs[i, \phi] \subseteq \allargs$ as the transitive closure of ${\gleadstoinv}(\phi)$ under $\dfp$: $\gargs[i, \phi] = \cup_{k \in \N} \dfp^k({\gleadstoinv}(\phi))$, where $\dfp^0$ is the identity function.
