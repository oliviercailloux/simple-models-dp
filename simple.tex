...
Given $\ar \in \gargsi$, say that $\ar$ $i$-defends $\pga$ iff $\forall \hist \in \allhist \suchthat \ar \in \histend \land \hist \nileadsto \pga: d(\ar, \hist) ≠ \emptyset$.
Note that if an argument fails to $i$-defend $\pga$, then the argumentative strategy of the theory has failed: some path challenge an argument of the theory, and the theory is not ready to counter-argue against that path.

Accordingly, an argumentative theory claims that all the arguments required to convince $i$ of choosing $\pga$ do $i$-defend $\pga$.
It is said to be truthful iff this claim is correct.
\begin{definition}[Truthfulness]
	\label{def:truth}
	An argumentative theory $(\pga, \arga, \dg)$ for the exposure protocol $(I, \allargs, {\leadsto})$ is \emph{truthful} iff $\forall i \in I, \ar \in \gargsi: \ar$ $i$-defends $\pga$.
\end{definition}

NB we can check $a_1, a_x, a_y, a_2, …$ but not other paths after $a_2$.
Thus, among the paths that contain $a_2$, they all contain $a_1$ before $a_2$.
...
\begin{axiom}[Omitting the defended argument]
	\label{ax:omitting}
	If the theory is truthful, then $\forall \ar_1, \ar_2 \in \gargsi, \hist \suchthat \ar_2 \in \dgi(\ar_1) \land \ar_2 \in \histend: (\ar_1, \hist) \ileadsto \pga ⇒ \hist \ileadsto \pga$.
\end{axiom}
...
\end{theorem}
\begin{proof}
	Assume, to start with, that $\arga$ is not decisive because $\hist \nileadsto \pga, \arga \in \histend$.
	Assume that $\dgi(\arga) = \ar_2$ with no further hist against it.
	…
\end{proof}
