...
\section{Motivation}
TODO The model should be: (m, s) leadsto (t, sure) where m can be for example: after having presented s0, s1, s2, present s3, and tell the individual to wait for one week.
...
\section{General normative theories}
A general normative theory is a tuple $(\alltopic, \allargs, I, (\ileadsto, i \in I), \gargs, \gleadsto)$.
...
Then the normative criterion is: $t$ in DJ of $i$ when $\exists s \suchthat \forall s': (s', s) \ileadstost^i t$ (the theory provides a part of this guarantee under falsification, claiming for all $i$, the model makes the rest under validation for this $i$).
...
The relation $\ileadstost$ must satisfy the two following properties, or axioms. (A1) $\ileadstost \subseteq \ileadstoe$
\footnote{Equivalently: $\ileadstosst \subseteq \ileadstose$, $\ileadstopst \subseteq \ileadstope$, $(\ar, \ar') \ileadstose ¬\prop ∨ (\ar, \ar') \ileadstope \prop$, $\ileadstopst \prop ⇒ ¬ (\ileadstosst ¬\prop)$, $\ileadstosst ¬\prop ⇒ ¬ (\ileadstopst \prop)$.} 
and (A2) $\forall \prop \in \alltopic: (\ar, \ar') \ileadstost (\prop, \text{sure}) ⇒ (\ar, \ar') \ileadstost (\prop, \text{possible})$
\footnote{Equivalently: $\ileadstose \subseteq \ileadstope$.}. 
Observe that (A1) can equivalently be formulated as $\forall \prop \in \alltopic: [(\ar, \ar') \ileadstost (\prop, \text{sure}) ⇒ (\ar, \ar') \ileadstoe (\prop, \text{sure})]$, or equivalently, $\forall \prop \in \alltopic: [(\ar, \ar') \ileadstost (\prop, \text{possible}) ⇒ (\ar, \ar') \ileadstoe (\prop, \text{possible})]$, and (A2) is equivalent to $\forall \prop \in \alltopic: [(\ar, \ar') \ileadstoe (\prop, \text{sure}) ⇒ (\ar, \ar') \ileadstoe (\prop, \text{possible})]$.
