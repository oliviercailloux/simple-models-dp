...
\section{Motivation}
Talk about \citet{von_neumann_theory_2004}.
...
Let $\allalts$ be a set of alternatives. Define $\Phi_\allalts$ as follows. Let $\phi_{a > b} \in \iPhi$ represent $i$ deliberately preferring strictly $a$ to $b$. Let $\phi_{a ≥ b} \in \iPhi$ represent $i$ deliberately preferring weakly $a$ to $b$. Define $¬\phi_{a > b} = \phi_{b ≥ a}$.
...
Define $≥_i$, the deliberated preference of $i$, as $a ≥_i b$ iff $\phi_{a ≥ b} \in \iPhi$. 

Given an empirical theory for a normative theory of preferences, say that it claims a complete preference iff $\forall a, b \in \allalts: \phi_{a ≥ b} \in \gPhi \lor \phi_{b ≥ a} \in \gPhi$. Similarly, it claims a transitive preference iff $\forall a, b, c \in \allalts: \phi_{a ≥ b} \in \gPhi \land \phi_{b ≥ c} \in \gPhi ⇒ \phi_{a ≥ c} \in \gPhi$. Observe that establishing whether an empirical theory claims a complete or transitive preference can be determined a priori (without empirical observations).

Given a bundle $B \subseteq \allalts$ and $i \in I$, $b \in B$ is a deliberated preferred choice for $i$ among $B$ iff $\forall b' \in B: b ≥_i b'$.
