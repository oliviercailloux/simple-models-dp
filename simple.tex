...
%	Let ${\leadsto}(i, a_e)$ represent $i$ being given an explanation about the psychological phenomenon described in the quote above.
	...
	If the prediction described in the quote applies to $i$, the relation $\ileadsto$ is as follows, $\forall k \in \N^*$ (recall that for $\args \subseteq \allargs$, $\args^k$ represents the set of sequences of length $k$ using arguments from $\args$): $\forall \hist \in \set{a_e}^k, \hist \ileadsto \prop_2$; $\forall \hist \in \set{a_1, a_2}^k, \hist \ileadsto \prop_1$; $\forall \hist \in \set{a_1, a_e}^k \suchthat a_e \in \histend, \hist \ileadsto \prop_2$; $\forall \hist \in \set{a_2, a_e}^k \suchthat a_e \in \histend, \hist \ileadsto \prop_2$; $\forall \hist \in \allargs^k \suchthat a_e \in \histend, \hist \ileadsto \prop_2$. The values of ${\leadsto}(i, \hist)$ for $\hist = \emptyset$ and for $\hist \in \allargs^k \suchthat a_e \notin \histend$ are undetermined by the prediction, but whatever they are, $a_e$ is a decisive argument for $\prop_2$, hence, $i$’s deliberated choice is $\prop_2$.
\end{example}
...
Given an exposure protocol $(I, \allargs, {\leadsto})$, we want to phrase some claims under the form of a logical sentence $C$ such that:
...
For example, if $C$ is $\alpha \ileadsto \phi \lor \beta \leadsto[j] \phi$, $O$ is $\alpha \ileadsto ¬\phi \land \beta \leadsto ¬\phi$, then $O$ implies $¬C$.
...
Consider $\leadsto$ such that $\exists \ar, \phi \suchthat \forall i: \ar \ileadsto \phi$.
...
Now consider $\leadsto$ such that $\exists J, K \subset I \suchthat \ar \ileadstost[j] \phi, \ar' \ileadstost[k] \phi$.
...
Given an exposure protocol $(I, \allargs, {\leadsto})$, recall that ${\leadsto} \in P^{I × \allhist}$.

Given a pair $(I, \allargs)$, a claim $C$ is a set of possibilities for $\leadsto$, thus, $C \subseteq P^{I × \allhist}$.
When writing $C$ as a formula, it is intended to mean the set $\set{{\leadsto} \in P^{I × \allhist} \suchthat C({\leadsto})}$.

Given a pair $(I, \allargs)$, a general observation $O$ is a finite subset of $I × \allhist × P$ such that $\forall i \in I, (i, \hist, p), (i, \histp, q) \in O: \card{\hist} ≤ \card{\hist'} \implies \hist \subseteq \hist'$.
General observations are the a priori possible observations, that is, those that can be considered possible when nothing is known about $\leadsto$.
Let $S_O = \set{f \in P^{I × \allhist} \suchthat O \subseteq f}$ denote the possibilities for $\leadsto$ compatible with the observations.

A claim $C$ is falsifiable iff $\exists O \suchthat O \implies ¬C$, by which I mean $S_O \cap C = \emptyset$.
\begin{example}
	$(\ar) \ileadsto \phi \lor (\ar, \ar) \ileadsto \phi \lor …$ is not falsifiable.
%	Its negation is $\forall k: (\ar^k) \nileadsto \phi$.
%	The claim represents the set $\set{{\leadsto} \in P^{I × \allhist} \suchthat (\ar) \ileadsto \phi \lor  (\ar, \ar) \ileadsto \phi \lor …}$.
	Given any $O$, define $l = \max_{\hist \suchthat \exists p \in P \suchthat (i, \hist, p) \in O} \card{\hist} + 1$. Pick any $\leadsto$ such that $O \subset {\leadsto} \land (\ar)^l \ileadsto \phi$. We have ${\leadsto} \in S_O \cap C ≠ \emptyset$.
\end{example}
\begin{example}
	$(\ar) \ileadsto \phi \lor (\ar') \ileadsto \phi$ is not falsifiable.
	Given any $O$, it may not contain both $((\ar), i, \phi)$ and $((\ar'), i, \phi)$, thus letting $t$ denote one of these triples not in $O$, pick any $\leadsto$ such that $O \subset {\leadsto} \land t \in {\leadsto}$. We have ${\leadsto} \in S_O \cap C ≠ \emptyset$.
\end{example}
\begin{example}[Fals. but shouldn’t]
	\label{ex:fbs}
	$\hist \ileadsto[j] \phi \land [(\ar) \ileadsto \phi \lor (\ar') \ileadsto \phi]$ is falsifiable.
	Consider $O = \set{(j, \hist, ¬\phi)}$.
\end{example}
\begin{example}
	$\forall i: \ar \ileadstost \phi$ is falsifiable.
	Consider any $i \in I$ and $O = \set{(i, (\ar), ¬\phi)}$.
\end{example}
\Cref{ex:fbs} shows that this definition of falsifiability is wrong.

We now say that $C$ is falsifiable iff $\forall {\leadsto} \in P^{I × \allhist}: ¬C({\leadsto}) \implies \exists O \subseteq {\leadsto} \suchthat O \implies ¬C$, meaning that
$\forall {\leadsto} \notin C: \exists O \subseteq {\leadsto} \suchthat S_O \cap C = \emptyset$.
\begin{example}
	$(\ar) \ileadsto \phi \lor  (\ar, \ar) \ileadsto \phi \lor …$ is falsifiable.
	Consider any ${\leadsto} \notin C$, thus satisfying $\exists k \suchthat (\ar)^k \nileadsto \phi$. Consider $O = \set{(i, (\ar)^k, {\leadsto}(i, (\ar)^k) ≠ \phi)}$, thus with $O \subset {\leadsto}$. We have $S_O \cap C = \emptyset$: consider any $O \subset f$, it is not in $C$.
\end{example}
\begin{example}
	\label{ex:unfals}
	$(\ar) \ileadsto \phi \lor (\ar') \ileadsto \phi$ is not falsifiable.
	Consider any ${\leadsto} \notin C$, thus satisfying $(\ar) \nileadsto \phi \land (\ar') \nileadsto \phi$.
	Consider any $O \subseteq {\leadsto}$ and let us prove that $S_O \cap C ≠ \emptyset$.
...
\end{example}
\begin{example}
	$\hist \ileadsto[j] \phi \land [(\ar) \ileadsto \phi \lor (\ar') \ileadsto \phi]$ is not falsifiable.
	Consider any $\leadsto$ satisfying $(\ar) \nileadsto \phi \land (\ar') \nileadsto \phi$, thus with ${\leadsto} \notin C$.
	\Cref{ex:unfals} concludes.
\end{example}

Deliberated choices are an element $\pi({\leadsto}) \subseteq I × P$ which is right unique (meaning that $\forall i \in I: \card{\pi({\leadsto})(i)} ≤ 1$).
Equivalently, let $(P \cup \set{\emptyset})^I$ denote the right unique relations over $I × P$; and then the deliberated choices are an element $\pi({\leadsto}) \in (P \cup \set{\emptyset})^I$.
We say that $C$ is informative iff $\exists f \in (P \cup \set{\emptyset})^I \suchthat C \implies ¬f$, meaning that $\pi(C) = \set{\pi(\leadsto) \suchthat {\leadsto} \in C} ≠ (P \cup \set{\emptyset})^I$.

Given an exposure protocol, we say that a claim $C$ holds iff ${\leadsto} \in C$.
$C$ is $\leadsto$-suitable iff it is informative and falsifiable and ${\leadsto} \in C$.

The following theorem verifies that when $C$ holds, it will not be falsified ex post (though it may be (a priori) falsifiable).
\begin{theorem}
	\label{th:ex-post}
	If ${\leadsto} \in C$, $\nexists O \subseteq {\leadsto} \suchthat S_O \cap C = \emptyset$.
\end{theorem}
\begin{proof}
	Equivalently, considering any $O \subseteq {\leadsto} \in C$: $\exists O \subseteq f \suchthat f \in C$.
	This holds as ${\leadsto}$ satisfies this.
\end{proof}
\Cref{th:ex-post} together with the definition of falsifiability leads to \cref{th:holdsiff}.
\begin{corollary}
	\label{th:holdsiff}
	$C$ is falsifiable iff [${\leadsto} \in C$ iff $\nexists O \subseteq {\leadsto} \suchthat S_O \cap C = \emptyset$].
\end{corollary}
%\begin{proof}
%	Assume that $C$ does not hold, thus, ${\leadsto} \notin C$.
%	Because $C$ is falsifiable, $\exists O \subseteq {\leadsto} \suchthat S_O \cap C = \emptyset$.

\begin{example}[To be corrected]
Consider any $\leadsto$ such that $\exists \ar, \phi \suchthat \forall i: \ar \ileadstost \phi$.
...
\begin{example}[To be corrected]
Now consider $\leadsto$ such that $\exists J, K \subset I \suchthat \forall j \in J: \ar \ileadstost[j] \phi \land \forall k \in K: \ar' \ileadstost[k] \phi$.
...
\begin{definition}[Truthfulness]
	\label{def:truth}
	An argumentative theory $(\pga, \arga, \dg)$ for the exposure protocol $(I, \allargs, {\leadsto})$ is \emph{truthful} iff 
