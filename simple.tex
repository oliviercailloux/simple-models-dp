...
It may be argued that because this proposition needs a normative axiom, which cannot be validated empirically, it does not fare better than the alternative propositions existing in the literature. \ac{SEU}, for example, also can be considered to claim to obtain reflective preferences on the basis of normative axioms.
...
However, it is good to lighten as much as possible the content of normative axioms, and defer as much content as possible to descriptive claims. The descriptive parts of theories can be confronted to observations, a natural way to lift disagreements that has often proved useful in the history of science (again, accepting this claim does not require to ignore that observations themselves rest upon theories). Arguably, the normative axiom proposed here is much weaker than those of a theory such as \ac{SEU}, in the sense of having less substance. (This claim can only be valid at an intuitive level: because \ac{SEU} does not use arguments, its axioms cannot be compared formally to mine.) This does not prevent theories of \ac{DJ} to propose substantive, non-trivial claims about reflective preferences. It simply moves the debate from the world of discussions between experts about the validity of the axioms to the world of observations of acceptability of arguments by the persons to which these theories claim to be applicable.
It is also worth noting that the proposed axiom has a specific form that may permit decision makers to reconstruct the reasoning proposed by the theory so as to be able to check that they apply to their situation. Detailed exploration of this aspect is deferred to future work.
...
\commentOCf{Il me semble assez naturel de, étant donné un ensemble de caractères $S$, définir l’ensemble des chaines de caractères par $S^*$ (représentant le fait de prendre toutes les puissances possibles de $S$), mais c’est peut-être un tropisme d’informaticien qui ne sera donc pas compris par des lecteurs économistes. J’ai vu la notation $Seq(\allargs)$ être utilisée, mais elle me semble un peu longue, vu l’usage intensif de ce symbole. Oui, peut-être en viendrai-je à l’invention que tu proposes, mais inventer ses propres symboles n’est légitime que si aucun n’existe déjà. Je vais essayer d’interroger d’autres gens, dis-moi en attendant si tu penses qu’on peut s’y habituer ou si c’est vraiment très dérangeant.} 
Throughout the article, $\N$ includes zero. %and $\N^* = \N \setminus \set{0}$. 
Given $j, k \in \N$, the notation $\intvl{j, k}$ represents $\set{l \suchthat j ≤ l ≤ k}$. 
...
To make it useful, an axiom is required that links the observable reactions of the individuals to their \ac{DJ}.
This will be done through the notion of a decisive argument, that is, intuitively, an argument that is able to convince an individual of the acceptability of a proposition whatever the argumentative path the individual has been submitted to. For formal definitions, some supplementary notations must be defined.
...
To end the presentation of the normative theories of \ac{DJ}, it may be useful to analyze the status of contradictory propositions. \Cref{th:protcoh} shows that two propositions which are readily judged to be incompatible by $i$ cannot both be acceptable. In supplement, it may be interesting to observe that theories of \ac{DJ} do not require to postulate that $i$’s \ac{DJ} is complete, in the sense that when the agenda is closed under negation, it must not necessarily be the case that either the proposition or its negation is in $i$’s \ac{DJ}. This justifies that the normative axiom asks only for an implication between acceptability and \ac{DJ}, instead of an equivalence.

\subsection{Coherence and completeness}
A relation $n \subseteq \allprops × \allprops$ is said to be an incompatibility relation iff $\forall \hist \in \allhist, (\prop, \prop') \in n: \hist \ileadsto \prop ⇒ \hist \nileadsto \prop'$. Two propositions $\prop, \prop' \in \allprops$ are said to be incompatible iff $(\prop, \prop') \in n$.
...
consider defined a negation operator $¬$ on $\allprops$. $¬$ is a negation operator iff it is an involution operator (thus a function $¬: \allprops → \allprops$ satisfying $\forall \prop \in \allprops: ¬¬\prop = \prop$) and furthermore satisfying, $\forall \phi \in \allprops: \hist \ileadsto \prop ⇒ \hist \nileadsto ¬\prop$.
...
\subsection{Empirical theories, informally}
\commentOCf{TODO : intégrer les explications dans la partie formelle et les reformuler.}
...
An empirical theory for a normative theory of \ac{DJ} is defined as a tuple $(\gleadsto, \gbeats, \alldargs, \dbeats, +)$, where ${\gleadsto} \subseteq \allargs × \allprops$ is an initial argumentation relation; ${\gbeats} \subseteq \allargs × \allargs$ is an anticipated attacks relation; $\alldargs$ is a primitive set of argument fragments; ${\dbeats} \subseteq \alldargs × \gbeatsinv(\allargs)$ is a defense relation; and $+$ combines arguments to coalitions of fragments to form new arguments; with the supplementary constraint that the tuple must have a bounded depth. These concepts are now defined formally.
...
More precisely, let $\gpropse$, the range of ${\gleadsto}$, be called the propositions supported by the descriptive theory.
...
A defense relation ${\dbeats} \subseteq \alldargs × \gbeatsinv(\allargs)$ is an injective and surjective binary relation over $\alldargs × \gbeatsinv(\allargs)$. \commentYM{le fait qu'elle soit surjective et injective est une exigence que tu poses dans la définiton ? pourquoi ? à quoi est-ce que cela sert ?} 
...
the set $\args \cap \range(\hist)$ denotes the arguments that are both in $\args$ and contained in the sequence $\hist$.

Define a falsification sequence for $i, \ar_0, \phi$, with $\ar_0 \gleadsto \phi$, thus $\ar_0$ an initial argument for $\phi$, as a finite sequence $\hist^1, \hist^3, …, \hist^{k + 1}$, $k \in 2\N$, such that it presents successive challenges, which means the following. 
Given $l \in 2\N \suchthat l ≤ k$, $\ar_l \in \allargs$ and $\hist^{l + 1} \in \allhist$, 
define $\args_{l + 1} = \gbeatsinv(\ar_l) \cap \range(\hist^{l + 1})$; 
define $\dargs[l + 2] = \dbeatsinv(\args_{l + 1})$ as those fragments defending against the anticipated attackers $\args_{l + 1}$; 
and, if $\args_{l + 1} ≠ \emptyset$ (equivalently, if $\dargs[l + 2] ≠ \emptyset$), define $\ar_{l + 2} = \ar_l + \dargs[l + 2]$. 
The sequence presents successive challenges iff, $\forall l \in 2\N \suchthat l ≤ k$, we have that $\ar_l \in \histend[l + 1]$, $\hist^{l + 1} \nileadstost \phi$, and $[\args_{l + 1} = \emptyset ⇔ l = k]$.
...
\end{lemma}
\begin{proof}
	First, from right to left, consider a falsification sequence $(\hist^{l + 1})_{l \in 2\N, l ≤ k}$ for some $k \in 2\N$, define, as in \cref{sec:fals}, $\args_{l + 1} = \gbeatsinv(\ar_l) \cap \range(\hist^{l + 1})$, $\dargs[l + 2] = \dbeatsinv(\args_{l + 1})$ and (for $l ≤ k - 2$) $\ar_{l + 2} = \ar_l + \dargs[l + 2]$, and let us show that $(i, \phi, \hist^{k + 1}, \ar_{k})$ is a falsification instance. This amounts to show that $\ar_k \in \gargs$, which can be done by observing that $\ar_0 \in \gargs$, and showing that, $\forall l \in 2\N, l ≤ k - 2$, $\ar_l \in \gargs ⇒ \ar_{l + 2} \in \gargs$.
	Indeed, \cref{th:dpiconstr} applies with $\ar_l$ for $\ar_0$ and $\ar_{l + 2}$ for $\ar_2$. As the second member of the equivalence is true by definition of a falsification sequence, $\ar_{l + 2} \in (\dip)(\ar_l)$, and because $\ar_l \in \gargs$, $\ar_{l + 2} \in \gargs$.
	
	Second, from left to right, consider a falsification instance $(i, \phi, \hist^{k + 1}, \ar_{k})$. As $\ar_k \in \gargs$, $\ar_k \in (\dip)^k({\gleadstoinv}(\prop))$ (equalizing the indices $k$ wlog). Given $\ar_{l + 2} \in (\dip)^j({\gleadstoinv}(\prop))$, with $j > 0$, thus $\ar_{l + 2} \in \dip(\ar_l)$ for some $\ar_l \in (\dip)^{j - 1}({\gleadstoinv}(\prop))$, apply \cref{th:dpiconstr}, obtain $\hist_{l + 1}$. Iterate while $j > 0$. When $j = 0$, the $\hist$ found corresponds to an initial argument.
