...
A supplementary problem might happen in some situations. Once an argumentative path has been presented to the individual, this cannot necessarily be undone: she might remember some of the arguments, and they may have a lasting influence on her perspective. Let me call this possibility the hypothesis of persistence, and the converse hypothesis, the hypothesis of immediacy.
Under the hypothesis of persistence, it is impossible to test twice the same path. Tests of the stability of the opinion of $i$ towards a given set of arguments can still be performed, as the next example indicates.
...
\begin{example}[Hypothesis of memory]
	 The definition of $\ileadsto$ as given so far implicitly assumes that a given path can be tested at most once with a given individual, as it does not indicate what to conclude when a path $\hist$ leads $i$ to adopt a proposition $\prop$ only some of the time. This is justified under the hypothesis of memory. 
	 Recall that in \cref{ex:incompl}, $\hist \ileadsto P ≥ D$ and $\hist' \ileadsto D > P$ for some $\hist, \hist' \in \allhist$. If we further assume that $\range(\hist) = \range(\hist')$, in other words, that these two paths use the same arguments, this illustrates the remark that instability of $i$ towards some set of argument can be observed even under the hypothesis of memory. Consider $\hist = \left<\ar_0, \ar_1\right>$ and $\alpha' = \left<\ar_0, \ar_1, \ar_0\right>$ as an example. As $\hist'$ is a sequence that starts with $\hist$, it is possible to test these two paths on $i$, even under the hypothesis of memory.

	If, on the contrary, it is considered that the individual forgets over time, we must define $\ileadsto$ differently. Define $T$ a series of time steps, and given $t \in T$, define $\hist \ileadsto^t \prop$ as the observation of the adoption of $\prop$ by $i$ when having been presented the arguments in $\hist$ at time $t$. Define then $\hist \ileadsto \prop$ iff $\forall t \in T: \hist \ileadsto^t \prop$. Under the hypothesis of immediacy, it could be that $\exists \hist \suchthat \hist \nileadsto P ≥ D \land \hist \nileadsto D > P$.
\end{example}
...
\Cref{sec:validity} defines formally the notion of falsification sequence and proves this central claim.
The practical meaningfulness of this result is not affected by the hypothesis of memory, under condition that the set $I$ be large enough: we can always then perform falsification tests on different individuals for each path that has to be tested, so that memory of individuals does not matter. Exploring settings where $I$ is too small to apply this strategy is left aside in this article.

This result permits to ensure that, under Normative adequacy, Validity is an empirical property. But it does not give a concrete way of arbitrating between conflicting theories. A supplementary important result, described in \cref{sec:compare}, is that when two theories disagree about incompatible propositions, one of the theories at least must fail some given sequence of empirical tests, which the proof describes. These tests can all be performed on a single individual and are robust against the hypothesis of memory, which suggest that arbitration between theories could be performed reasonably easily.
...
A defense relation $\dg \subseteq (\allargs × \allhist) × \allargs$ is a right-unique relation, thus given any $\ar \in \allargs, \hist \in \allhist$, $\dg(\ar, \hist)$ is empty or is a singleton. %, and must satisfy $\ar \notin \histend ⇒ \dg(\ar, \hist) = \emptyset$. 
...
\begin{example}[A theory for $P$]
	...
	In order to define a counter-argument to $\ar_0$, given a real number $r \in \R$, define $\ar^r_\text{non-linear}$ as the text “It is absurd to reason in terms of expected gain. For example, consider a choice between winning $r$ dollars with probability $0.8$ and winning $2r$ dollars with probability $0.4$. They have the same expected gain, but the first one is clearly preferrable”. (The value of $r$ and $2r$ is to be used in the relevant places in  the text.)
...
\begin{example}[Infinite $\dg$]
	...
	Define $\dg = \set{(\ar_0, \ar^r_\text{non-linear}, \ar_\text{linear}) \suchthat r \in \R}$. This strategy sets up the argument $\ar_\text{linear}$ as an answer to any argument of the form described in \cref{ex:twolottstrings}, arguing for non-linearity of utility of money.
	This defines a descriptive theory of depth $2$.
\end{example}
...
\begin{example}[Required arguments]
	...
	We see that for any $i$ in that set of individuals, $\gargs[D > P] = \set{\ar_0, \ar_\text{linear}}$, and for any $i$ in the complementary set of individuals, $\gargs[D > P] = \set{\ar_0}$.
\end{example}
...
\begin{example}[Convincingness]
	Pursuing with \cref{ex:required}, that theory will be convincing iff either $i$ finds some argument $\ar^r_\text{non-linear}$ convincing in presence of $\ar_0$ and both $\ar_0$ and $\ar_\text{linear}$ $i$-defend $D > P$, or $i$ does not find any argument $\ar^r_\text{non-linear}$ convincing in presence of $\ar_0$ and $\ar_0$ $i$-defends $D > P$. 
...
To verify that the property of Convincingness is purely empirical, it is necessary to show that whenever a theory fails it, it is possible to see it using a set of observations of $\ileadsto$. Conversely, it is also desirable to ensure that only the observations that ensure that Convincingness is false count as falsifying a theory.
The following example may help forging an intuition about what the possible falsifying sets of observations are.
\begin{example}[Falsification sequences]
	...
	The second possibility is slightly more demanding, and is the only one that is available if indeed $\ar_0$ $i$-defends $D > P$. It requires first to show that $\ar_\text{linear}$ is a required argument to convince $i$, then to show that this argument fails to convince $i$. For this, two paths are required: $\hist^0 \in \allhist \suchthat \ar_0 \in \histend[0] \land \hist^0 \nileadsto D > P \land [\exists r \in \R \suchthat \ar^r_\text{non-linear} \in \range(\hist^0)]$, and $\hist^1 \in \allhist \suchthat \ar_\text{linear} \in \histend[1] \land \hist^1 \nileadsto D > P$.
\end{example}

NB suffices to prove that any $\ar_k \in (\dgip)^k$ iff $\exists (\ar_0, \hist^0, \ar_1 = \dg(\ar_0, \hist^0), …, \ar_k)$ with $\ar_0 \in \histend \land \hist \nileadsto \phi$.
...
A falsification sequence presents successive challenges iff [$\ar_{k + 1}$ is undefined exactly when $k = N$] and [$\forall k \in \intvl{0, N}: \ar_k \in \histend[k] \land \hist^k \nileadsto \phi$].
