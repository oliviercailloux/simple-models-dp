...
Omitting many other relevant arguments for now (such as observing that utility should be linear for small amounts of money, for example), define $\allargs = \set{\ar_0, \ar_1}$.

Define $I$ as a set of individuals facing a choice between $P$ and $D$ in a laboratory experiment.

Define $\ileadsto$ as follows. Given $\hist$, $\hist \ileadsto (P ≥ D)$ iff, in a situation of $i$ having been exposed, in order, to the arguments in $\hist$, saying: “Considering these arguments, would you please choose either to play $P$, or play $D$, or declare to be indifferent (in which case one will be picked randomly for you)” leads $i$ to choose $P$ than $D$, or declare indifference. $\hist \ileadsto \prop$ is defined similarly for other elements $\prop \in \allprops$.
