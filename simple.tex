...
Given $\phi \in \Phi$ and $\hist \in \allhist$, say that $\hist$ leads strongly to $\phi$ iff $\hist \ileadstoe \phi \land \hist \nileadstoe ¬\phi$.
Define $\hist \ileadstosts \phi$ iff $[\hist \text{ leads strongly to } \phi] \land [\phi = \propsure ⇒ \hist \text{ leads strongly to } \propposs]$.
Define $\ar \ileadstosts \phi$ iff $\forall \hist \in \allhist \suchthat \hist \text{ has } \ar \text{ as last or before last position }: \hist \ileadstosts \phi$.
