...
Note that $\forall \ar_0 \in \allargs, \args \subseteq \gbeatsinv(\ar_0): \args \ibeatscoalstr \ar_0 ⇒ \args \ibeatscoal \ar_0$. That is because $\ar_0 \ngbeats \ar_0$, because $\gbeats$ is irreflexive by definition of an empirical theory, and thus suffices to consider any sequence of arguments $\hist$ using only and all the arguments $\args \cup \set{\ar_0}$ and placing $\ar_0$ last: because $\args \ibeatscoalstr \ar_0$, $\hist \nileadstost \phi$, and the existence of $\hist$ proves that $\args \ibeatscoal \ar_0$.

Given $i \in I, \phi \in \Phi$ and $\ar_0, \ar_1 \in \allargs$, define $\ar_1 \ibeats \ar_0$ iff $\exists \args \subseteq \gbeatsinv(\ar_0) \suchthat \ar_1 \in \args \land \args \ibeatscoal \ar_0 \land \args \setminus \set{\ar_1} \nibeatscoalstr \ar_0$. 
...
\begin{proof}
	...
	Define $\args = \gbeatsinv(\ar_0) \cap \hist$.
	Because $\ar_0$ $i$-defends $\phi$, $\args ≠ \emptyset$. 
	From the preceding conclusion, $\exists \ar_1 \in \args \suchthat \ar_1 \gbeats \ar_0$ and $\ar_1 \ibeats \ar_0$.
