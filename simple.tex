...
Given $i \in I, \phi \in \Phi, \ar_1, \ar_0 \in \allargs$, define $\ar_1 \ibeatsi \ar_0$ iff $\exists \hist \in \allhist \suchthat \set{\ar_1, \ar_0} \subseteq \histend \land \hist \nileadstosti \phi$, and say that $\ar_1$ is an effective attack against $\ar_0$ (from $i$’s perspective).

Given $i \in I, \phi \in \gPhi, \ar \in \allargs$, define the \emph{$\phi$-defensors of $\ar$ required for $i$}, $\di(\ar) \subseteq \alldargs$, as $\di(\ar) = [{\dbeatsinv} \circ ({\gbeatsinv} \cap {\ibeatsiinv})](\ar)$. Define $\dip: \allargs → \allargs$ as $\dip(\ar) = \ar + \di(\ar)$.
...
Given $\hist \in \allhist$, $\hist(\intvl{1, l(\hist)})$, where $l(\hist) \in \N$ is the length of the sequence $\hist$, denotes the range of $\hist$, that is, all arguments contained in the sequence $\hist$. Abusing notation, given $\args_1 \subseteq \allargs$, I write $\args_1 \cap \hist$ to designate $\args_1 \cap \hist(\intvl{1, l(\hist)})$, therefore using simply $\hist$ in that expression to denote the \emph{range} of $\hist$.
...
\begin{proof}
	...
	For all $i \in I, \phi \in \gPhi$, suffices to show that for some $\ar \in \gargs[i, \phi]$, $\ar \ileadstosti \phi$.
	
	Consider any $\ar_0 \gleadsto \phi$.
	By definition of $\dip$, $\forall k \in \N^*: (\dip)^k(\ar_0) \subseteq \dfp^k(\ar_0)$.
	By definition of an empirical theory, $\exists k \in \N^* \suchthat \dfp^k(\ar_0) = \emptyset$. 
	Therefore, $\exists k \in \N^* \suchthat (\dip)^k(\ar_0) = \emptyset$. 
	Define $l = \min \set{k \in \N^* \suchthat (\dip)^k(\ar_0) = \emptyset}$.
	
	Consider any $\ar \in (\dip)^{l-1}(\ar_0)$, and let us show that $\ar \ileadstosti \phi$. 
	
	If $\exists \ar_1 \in \allargs \suchthat \ar_1 \ibeatsi \ar$, then $\exists \hist \in \allhist \suchthat \set{\ar_1, \ar} \subseteq \histend \land \hist \nileadstosti \phi$, and a falsification instance $(i, \phi, \hist)$ is found, which contradicts convincingness. \commentOC{Unless $\hist$ contains some planned attack!}
	Therefore, no $\ar_1 \ibeatsi \ar$.
	
	Because $\ar \in \gargs[i, \phi]$, $\ar$ $i$-defends $\phi$. Furthermore, ${\gbeatsinv}(\ar) = \emptyset$, because by definition of an empirical theory, if $\ar_1 \gbeats \ar$, then $\ar_2 \dbeats \ar_1$, which contradicts $\dip$ NO!
\end{proof}
