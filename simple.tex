...
(The symbol $¬$ is also used in this article to negate classical mathematical propositions; this should create no confusion.)
...
%where $\allprops = \allprops × \set{\mathit{poss}, \mathit{sure}}$.

As usual with binary relations, $\hist \nileadsto \prop$ means $(\hist, \prop) \notin {\ileadsto}$.

Given $i \in I, \prop \in \allprops, \hist \in \allhist$, say that $\hist$ leads strongly to $\prop$, $\hist \ileadstost \prop$, iff $\hist \ileadsto \prop \land \hist \nileadsto ¬\prop$.
...
\begin{remark}
	I’d rather drop the negation and observe that if the protocol is defined so that $\hist \ileadsto \prop_1 ⇒ \hist \nileadsto \prop_2$, then $\prop_1 \in \iprops ⇒ \prop_2 \notin \iprops$. Observe that this is always possible. The definitions about completeness should use a negation operator in supplement to the normative theory tuple.
\end{remark}
...
\begin{axiom}[Normative adequacy]
	...
	$\forall i \in I, \prop \in \allprops: 
		[(\exists \ar \in \allargs \suchthat \ar \ileadstost \prop) \land ¬(\exists \ar \in \allargs \suchthat \ar \ileadstost ¬\prop)] ⇒ \prop \in \iprops \land ¬\prop \notin \iprops.$
\end{axiom}
...
\begin{property}[Simple normative adequacy]
	\label{ax:snorm}
	$\forall i \in I, \prop \in \allprops: 
		[\exists \ar \in \allargs \suchthat \ar \ileadstost \prop] ⇒ \prop \in \iprops \land ¬\prop \notin \iprops.$
\end{property}
...
\end{proposition}
\begin{proof}
	Protocol coherence is satisfied by definition of $\ileadstost$. Indeed, if for some $\ar \in \allargs$, $\ar \ileadstost \prop$, then $\forall \ar_1 \in \allargs: (\ar_1, \ar) \nileadsto ¬\prop$; thus, for any $\ar' \in \allargs$, $\ar' \ileadstost ¬\prop$ is false, because it requires that $\forall \ar_1 \in \allargs: (\ar', \ar_1) \ileadsto ¬\prop$, and thus in particular, that $(\ar', \ar) \ileadsto ¬\prop$.
\end{proof}
...
To further analyze this and related matters, given a normative theory and $i \in I$, say that a proposition $\prop \in \allprops$ is \emph{$i$-decidable} iff $\exists \ar \in \allargs \suchthat \ar \ileadstost \prop \lor \ar \ileadstost ¬\prop$.
...
The next property ensures an unambiguous interpretation of the content of $\iprops$ for all $i$-decidable propositions. In this definition, $\oplus$ denotes the exclusive disjunction; thus $\prop \in \iprops \oplus ¬\prop \in \iprops$ is equivalent to: $\prop \in \iprops ⇔ ¬\prop \notin \iprops$. It can be seen to hold by using \cref{th:normeqsnorm}.
...
\end{proposition}
\begin{proof}
	Consider an agenda containing two propositions, $\allprops = \set{\prop, ¬\prop}$, an infinite set of arguments $\allargs = \set{\ar_k \suchthat k \in \N}$ with $\forall j, k \in \N: \ar_j ≠ \ar_k$, and consider any set of individuals $I$ and observation protocol $\allleadsto$. 
...
\begin{definition}[Validity]
	An empirical theory is \emph{valid} iff $\forall i \in I, \prop \in \gpropse: [\prop \in \iprops \land ¬\prop \notin \iprops]$.
\end{definition}
...
\end{theorem}
\begin{proof}
	Given any $i \in I$ and $\prop \in \gpropse$, suffices to show that for some $\ar \in \allargs$, $\ar \ileadstost \prop$; the conclusion then follows thanks to simple normative adequacy (that \cref{th:normeqsnorm} shows to be equivalent to normative adequacy). 
...
Given $\prop \in \allprops$ and two empirical theories $\gamma$ and $\delta$ for a given normative theory, say that they disagree about $\prop$ iff $\prop \in \gpropse[\gamma] \land ¬\prop \in \gpropse[\delta]$. Say that they are incompatible iff there is at least one $\prop \in \allprops$ that they disagree about.
...
Given $\prop \in \allprops, \ar_\gamma \in \gargs, \ar_\delta \in \gargsdelta$, say that $(\hist, \ar_\gamma, \ar_\delta)$ is a $\prop/¬\prop$-challenge iff $(\hist, \ar_\gamma)$ challenges $(\gamma, \prop)$ or $(\hist, \ar_\delta)$ challenges $(\delta, ¬\prop)$.

\begin{lemma}
	\label{th:endchal}
	Given $\ar_\gamma \in \gargs$, $\ar_\delta \in \gargsdelta$ and $\hist \in \allhist$ such that $\set{\ar_\gamma, \ar_\delta} \subseteq \histend$, $(\hist, \ar_\gamma, \ar_\delta)$ is a $\prop/¬\prop$-challenge.
\end{lemma}
\begin{proof}
	Either $\hist \nileadstost \prop$, and $(\hist, \ar_\gamma)$ challenges $\prop$, or $\hist \ileadstost \prop$, hence, by \cref{th:protcoh}, $\hist \nileadstost ¬\prop$, and $(\hist, \ar_\delta)$ challenges $¬\prop$. 
\end{proof}

\begin{lemma}
	\label{th:nextchal}
	Given $(\hist, \ar_\gamma)$ challenging $\prop$, with $\ar_\gamma \in \gargs \cap (\dfp)^k(\gleadstoinv(\prop))$, and $\ar_\delta \in \gargsdelta$: either $(i, \prop, \hist, \ar_\gamma)$ falsifies $\gamma$, or there exists a $\prop/¬\prop$-challenge $(\histp, \ar'_\gamma, \ar_\delta)$, with $\ar'_\gamma \in \gargs \cap (\dfp)^{k + 1}(\gleadstoinv(\prop))$.
\end{lemma}
\begin{proof}
...
\begin{procedure}
	...
	$(\hist, \ar_\alpha, \ar_\beta)$ is a $\prop/¬\prop$-challenge, $\ar_\alpha \in (\dfp[\alpha])^{k_\alpha}(\gleadstoinv[\alpha](\prop))$, and $\ar_\beta \in (\dfp[\beta])^{k_\beta}(\gleadstoinv[\beta](¬\prop))$.
	
	Pick any $\ar_{-1} \in \allargs \suchthat \ar_{-1} \gleadsto[\beta] ¬\prop$. Observe that $\ar_{-1} \in \gargsbeta \cap (\dfp[\beta])^0(\gleadstoinv[\beta](¬\prop))$. Test whether $(\ar_{-1}) \nileadstost \prop$. If so, $(i, \prop, (\ar_{-1}), \ar_{-1})$ falsifies $\beta$, which ends the procedure. 
	Thus, assume that $(\ar_{-1}) \ileadstost \prop$. 
	
	Pick any $\ar_0 \in \allargs \suchthat \ar_0 \gleadsto[\alpha] \prop$. Observe that $\ar_0 \in \gargsalpha \cap (\dfp[\alpha])^0(\gleadstoinv[\alpha](\prop))$.  Define $\hist = (\ar_{-1}, \ar_0)$ and test whether $\hist \nileadstost \prop$. By \cref{th:endchal}, $(\hist, \ar_0, \ar_{-1})$ is a $\prop/¬\prop$-challenge, thus, $(\hist, \ar_0, \ar_{-1})$ is a challenge of complexity $(0, 0)$.
...
And if $(\hist, \ar_\beta)$ challenges $¬\prop$, apply \cref{th:nextchal} with $¬\prop$ for $\prop$, $\beta$ for $\gamma$, $\alpha$ for $\delta$, and obtain either that $\hist$ falsifies $\beta$, or a new tuple $(\histp, \ar'_\beta, \ar_\alpha)$ such that $(\histp, \ar_\alpha, \ar'_\beta)$ is a challenge of complexity $(k_\alpha, k_\beta + 1)$.
	
	Given that $\exists K_\alpha \suchthat (\dfp[\alpha])^{K_\alpha}(\gleadstoinv[\alpha](\prop)) = \emptyset$ and $K_\beta \suchthat (\dfp[\beta])^{K_\beta}(\gleadstoinv[\beta](¬\prop)) = \emptyset$ by definition of empirical theories, this process must end with a falsifying instance of $\alpha$ or $\beta$.
\end{procedure}
