...
$\forall \ar \in {\gleadsto}(\allargs): \exists k \in \N^* \suchthat \dfp^k(\ar) = \emptyset$.
...
Given $i \in I, \phi \in {\gleadsto}(\allargs), \ar \in \allargs$, define the \emph{$\phi$-defensors of $\ar$ required for $i$}, $d^{i, \phi}(\ar) \subseteq \alldargs$, as $d^{i, \phi}(\ar) = [{\dbeatsinv} \circ ({\gbeatsinv} \cap {\ibeatsiinv})](\ar)$. Define $d^{i, \phi}_+$ as $d^{i, \phi}_+(\ar) = \ar + d^{i, \phi}(\ar)$.
Given $i \in I, \phi \in {\gleadsto}(\allargs)$, define $\gargs[i, \phi] \subseteq \allargs$ as the transitive closure of ${\gleadstoinv}(\phi)$ under $d^{i, \phi}_+$: $\gargs[i, \phi] = \cup_{k \in \N} (d^{i, \phi}_+)^k({\gleadstoinv}(\phi))$, where $(d^{i, \phi}_+)^0$ is the identity function.
...
\begin{definition}[Convincingness]
	$\forall i \in I, \phi \in {\gleadsto}(\allargs), \ar \in \gargs[i, \phi]: \ar$ $i$-defends $\phi$.
\end{definition}

A \emph{falsification instance} of an empirical theory is a tuple $(i, \phi, \hist) \in I × {\gleadsto}(\allargs) × \allhist$ such that $\exists \ar \in \histend \cap \gargs[i, \phi] \suchthat {\gbeatsinv}(\ar) \cap \hist = \emptyset \land \hist \nileadstosti \phi$.

\begin{theorem}
	An empirical theory is convincing iff it has no falsification instances.
\end{theorem}
\begin{proof}
	For all $i \in I, \phi \in {\gleadsto}(\allargs)$: $\exists \ar \in \gargs[i, \phi]$ that does not $i$-defend $\phi$ iff $\exists \ar \in \gargs[i, \phi], \hist \in \allhist \suchthat \left[\ar \in \histend \land {\gbeatsinv}(\ar) \cap \hist = \emptyset \land \hist \nileadstosti \phi\right]$ iff $\exists \hist \in \allhist$ such that $(i, \phi, \hist)$ is a falsification instance.

	For all $i \in I, \phi \in {\gleadsto}(\allargs)$, the following equivalences hold. $\forall \ar \in \gargs[i, \phi]$, $\ar$ $i$-defends $\phi$ iff $\forall \ar \in \gargs[i, \phi], \hist \suchthat \left[\ar \in \histend \land {\gbeatsinv}(\ar) \cap \hist = \emptyset\right]$, $\hist \ileadstosti \phi$ iff $\forall \hist \suchthat \left[\ar \in \histend \land {\gbeatsinv}(\ar) \cap \hist = \emptyset\right]$, $(i, \phi, \hist)$ is not a falsification instance.
\end{proof}
...
\begin{equation}
	...
	\ar_0 \nileadstosti \phi ⇔ [\exists \ar_1 \in \allargs \suchthat \ar_1 \ibeatsi \ar_0].
\end{equation}
\end{proposition}
\begin{proof}
	By definition of ${\ileadstosti}$, $\ar_0 \ileadstosti \phi ⇔ \forall \hist \in \allhist \suchthat \ar_0 \in \histend: \hist \ileadstosti \phi$. 
Equivalently, $\ar_0 \ileadstosti \phi ⇔ \forall \hist \in \allhist \suchthat [\exists \ar_1 \in \allargs \suchthat \set{\ar_1, \ar_0} \subseteq \histend]: \hist \ileadstosti \phi$. 
Still equivalently, $\ar_0 \nileadstosti \phi ⇔ \exists \hist \in \allhist \suchthat [\exists \ar_1 \in \allargs \suchthat \set{\ar_1, \ar_0} \subseteq \histend] \land \hist \nileadstosti \phi$. 
It can also be written $\ar_0 \nileadstosti \phi ⇔ \exists \hist \in \allhist, \ar_1 \in \allargs \suchthat \set{\ar_1, \ar_0} \subseteq \histend \land \hist \nileadstosti \phi$. By definition of ${\ibeatsi}$, this is the proposition to be proven.
\end{proof}
