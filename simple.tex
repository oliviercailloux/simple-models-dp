...
\begin{axiom}[Normative adequacy of $\allargs$ and $\ileadstost$]
...
This notation is justified only if it is impossible that both $\phi \in \iphi$ and $\phi \notin \iphi$ be implied. \commentOC{TODO clarify.} The following conditions permit to obtain this result.
...
\begin{definition}[Bounded width]
	$\forall \ar \in \gargs: \card{\ibeatseinv(\ar)} ≤ b_w$.
\end{definition}

\begin{definition}[Bounded length]
	$\forall n \in \N$, if $n > b_l$, there is no sequence of arguments $(\ar_k)_{0 ≤ k < n}$ such that $[\exists \phi \suchthat \forall (0 ≤ k < 2n): \ar_{2k} \in \gargs[\phi]]$ and $(\ar_{k + 1}, \ar_k) \ileadstoe \phi$ and $(\ar_{k + 1}, \ar_k) \ileadstoe ¬\phi$. \commentOC{This requires that $\ar_4$ have no attacker, not only in the sense that $\ar_4$ argues successfully against $\ar_3$, but in the (too strong) sense that $\ar_4$ argues successfully for $\phi$. It makes $\ar_4$ decisive, thus, requires that $\ar_4$ also argues against $\ar_1$, $\ar'_1, …$, and it makes the other arguments non useful. (Though may be useful for other dms, as we do not know in advance which one will be decisive.) I suppose I could simplify the presentation by making $\gargs[2]$ contain every good arguments and using only this one.}
\end{definition}

\begin{definition}[Reinstatement]
	A theory satisfies \emph{reinstatement} iff $\forall \ar_1, \ar_3 \in \gargs[\phi], \ar_2 \in \allargs: [\ar_1 \ileadstoe \phi, (\ar_2, \ar_1) \ileadstoe ¬\phi, (\ar_3, \ar_2) \ileadstoe \phi] ⇒ (\ar_1 + \ar_3, \ar_2) \ileadstoe \phi \land [\forall \ar_4 \ibeatse \ar_1 + \ar_3: \ar_4 \ibeatse \ar_1 \lor \ar_4 \ibeatse \ar_3]$.
\end{definition}

\begin{definition}[Operational validity]
	A theory is \emph{operationally valid} iff $\forall \phi \in {\gleadsto}(\allargs), \ar_0 \in \gargs[\phi], \ar_1 \in \allargs: (\ar_1, \ar_0) \ileadstoe ¬\phi ⇒ [\ar_1 \gbeats \ar_0] \land [\exists \ar_2 \in {\gbeatsinv}(\ar_1) \suchthat (\ar_2, \ar_1) \ileadstoe \phi]$.
\end{definition}

\begin{theorem}
	If a theory is answerable, satisfies reinstatement, has bounded width and length, and is operationally valid, it is valid. \commentOC{We don’t seem to need most of these conditions!}
\end{theorem}
\begin{proof}
	Given $\phi \in {\gleadsto}(\allargs), \args_2, \args_0 \subseteq \gargs$, define $\args_2$ as \emph{$\phi$-defending} $\args_0$ iff $\forall \ar_1 \in \allargs: [\exists \ar_0 \in \args_0 \suchthat (\ar_1, \ar_0) \ileadstoe ¬\phi] ⇒ [\exists \ar_2 \in \args_2 \suchthat (\ar_2, \ar_1) \ileadstoe \phi]$. Define $\args_2$ as \emph{decisively $\phi$-defending} $\args_0$ iff $\forall \ar_1 \in \allargs: [\exists \ar_0 \in \args_0 \suchthat (\ar_1, \ar_0) \ileadstoe ¬\phi] ⇒ [\exists \ar_2 \in \args_2 \suchthat (\ar_2, \ar_1) \ileadstost \phi]$.

	Fix $\phi \in {\gleadsto}(\allargs)$. We have to show that $\exists \ar \in \allargs \suchthat \ar \ileadstost \phi$.
	
	For all $k \in \N$, define $\gargs[2k] = [({\gbeatsinv} \cap {\ibeatseinv[\phi]}) \circ ({\gbeatsinv} \cap {\ibeatseinv[!\phi]})]^{k}({\gleadstoinv}(\phi))$.
	Note that $[\exists k \in 2\N \suchthat \ar \in \gargs[k]] ⇒ \ar \in \gargs[\phi]$.
	
	Lemma: $\forall k \in 2\N: \gargs[k+2]$ $\phi$-defends $\gargs[k]$. To prove this, fix $\ar_1 \in \allargs, \ar_0 \in \gargs[k] \suchthat (\ar_1, \ar_0) \ileadstoe ¬\phi$. We need to exhibit $\ar_2 \in \gargs[k+2] \suchthat (\ar_2, \ar_1) \ileadstoe \phi$. Indeed, we know that $\ar_1 \ibeatse[!\phi] \ar_0$, and we can use Operational validity as $\ar_0 \in \gargs[\phi]$, thus, $\ar_1 \gbeats \ar_0$ and $\exists \ar_2 \in {\gbeatsinv}(\ar_1) \suchthat (\ar_2, \ar_1) \ileadstoe \phi$.
	
	\commentOC{I suppose that, therefore, $\gargs$ $\phi$-defends itself!}
	
	Lemma: $\forall k \in 2\N$, if $\gargs[k+2] = \emptyset$, then $\forall s_0 \in \gargs[k]: s_0 \ileadstost \phi$.
	From the previous lemma, $\gargs[k+2]$ $\phi$-defends $\gargs[k]$, and, considering the implication in the definition of $\phi$-defense and observing that its consequent is false, we can negate its antecedent, obtaining: $\forall \ar_1 \in \allargs, \ar_0 \in \gargs[k]: (\ar_1, \ar_0) \ileadstost \phi$. This proves the lemma.
	\commentOC{Stuff at level $k$ should only take care of rebutting attackers at level $k-1$, and we would instead in this lemma obtain that they do take care of them successfully, enabling the use of reinstatement to get to the $k-2$ level. Still, it’s strange that Answerability is not needed.} 
	
	Pick any even $b > b_l$. 
	Thanks to Bounded length, $\gargs[b] = \emptyset$. 
	Define $d = \max\set{k \in 2\N \suchthat k < b, \gargs[k] ≠ \emptyset}$ (which exists because $\gargs[0] ≠ \emptyset$). 
	Thus, the last lemma can be applied with $k=d$ to obtain $\forall s_0 \in \gargs[d]: s_0 \ileadstost \phi$. Because $\gargs[d] ≠ \emptyset$, this concludes the proof.
\end{proof}
