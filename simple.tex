...
\begin{definition}[Truthfulness]
	\label{def:truth}
	An argumentative theory $(\pga, \arga, \dg)$ for the exposure protocol $(I, \allargs, {\leadsto})$ is \emph{truthful} iff $\forall i \in I, k \in \N^*, (\ar_j)_{j \in \intvl{1, k}} \in \allargs^k, \hist \in \allhist \suchthat [\ar_1 = \arga \land \ar_k \in \histend \land \forall j \in \intvl{1, k}: \ar_j \in R(\hist) \land \forall j \in \intvl{2, k}: \ar_j \in \dgi(\ar_{j - 1}) \land \hist \nileadsto \pga]: [d(\ar_k, \hist) ≠ \emptyset]$.
\end{definition}
