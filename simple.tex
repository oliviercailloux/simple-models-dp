...
It is now necessary to verify that, first, the claim of convincingness is falsifiable, and second, that it suffices to prove validity. 

Let us first clarify what falsifiability formally means in our context. Define a claim $C \subseteq \powerset{I × \allhist × \Phi}$ as a subset of the possible relations $\leadsto$. When a theory claims that $C$, it claims that $\leadsto$ is indeed a member of $C$. Say that a set of observations $O \subseteq I × \allhist × \Phi$ falsifies the claim $C$ iff $C \cap \set{R \subseteq O} = \emptyset$. In such a case, $C$ contains no possible relation $R$ compatible with the observations in $O$, or in other words, the observations $O$ prove that ${\leadsto} \notin C$. Say that $C$ is falsifiable iff some finite set of observation falsifies it.
...
Let the claim of a descriptive theory by defined as $C = …$.
\begin{theorem}
	…
