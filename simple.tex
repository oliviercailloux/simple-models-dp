...
\commentOC{Intuition. When last defense is burried under multiple other arguments so that it is not at end any more, just repeat the last defense. Otherwise the theory also needs to be able to compress the arguments against it. We prefer to keep this separated. As a result, an argument $\ar_0$ needs defense only when it is challenged by some path $\hist \suchthat \ar_0 \in \histend$, thus, letting $\dg$ define counter-arguments to paths not satisfying this condition would be of no use.}
