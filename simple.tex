...
Given $\phi \in \Phi$, define ${\ibeatse[!\phi]} \subseteq (\allhist × \allargs) × \allargs$ such that $(\hist, \ar_1) \ibeatse[!\phi] \ar_0$ iff $[(\hist \ileadstoe ¬\phi) \lor (\hist \nileadstoe \phi)] \land [\hist \text{ ends with } \set{\ar_1, \ar_0}]$.
I also write that $\ar_1 \ibeatse[!\phi] \ar_0$ iff $\exists \hist \in \allhist \suchthat (\hist, \ar_1) \ibeatse[!\phi] \ar_0$, and $\hist \ibeatse[!\phi] \ar_0$ iff $[(\hist \ileadstoe ¬\phi) \lor (\hist \nileadstoe \phi)] \land [(\exists \ar_1 \suchthat \hist \text{ ends with } \set{\ar_1, \ar_0}) \lor (\hist = \set{\ar_0})]$.

Given any $\hist \in \allhist, \ar_0 \in \allargs, \phi \in \Phi$, define $(\hist, \ar_0) \ileadstost[i] \phi$ iff $\forall \ar_1 \in \allargs: (\hist, \ar_0) \nileadstoe[i] ¬\phi \land (\hist, \ar_0) \ileadstoe[i] \phi \land (\hist, \ar_0, \ar_1) \nileadstoe[i] ¬\phi \land (\hist, \ar_0, \ar_1) \ileadstoe[i] \phi$. 
Define $(\hist, \ar_0) \ileadstosts \phi$ iff $[(\hist, \ar_0) \ileadstost \phi] \land [\phi = \propsure ⇒ (\hist, \ar_0) \ileadstost \propposs]$.
Define $\ar_0 \ileadstost[i] \phi$ iff $\forall \hist \in \allhist, (\hist, \ar_0) \ileadstost[i] \phi$, and define similarly $\ar \ileadstosts \phi$ iff $\forall \hist \in \allhist, (\hist, \ar_0) \ileadstosts \phi$.
Note in particular that: 
\begin{equation}
	\label{eq:st-in-terms-of-e}
	\ar_0 \ileadstosts \phi ⇔ [\forall \ar_1: \ar_1 \nibeatse[!\phi] \ar_0] \land [\ar_0 \nileadstoe ¬\phi] \land [\ar_0 \ileadstoe \phi].
\end{equation}
...
\begin{axiom}[Normative adequacy]
	\label{ax:norm}
	$\forall \phi \in \Phi: 
		[\exists \ar \in \allargs \suchthat \ar \ileadstosts \phi] ⇒ \phi \in \iPhi.$
\end{axiom}
...
For analytical clarity, it is desirable to check that \cref{ax:nocontr,ax:norm} do not hide a requirement on the content of $\ileadstost$, thus, that they are together satisfiable without constraining $\ileadstost$. To define this more precisely, consider temporarily these axioms as properties of a set $\iPhi \subseteq \Phi$. I write $A^1(\iPhi)$ if “axiom” 1 (interpreted as a property of $\iPhi$) holds on $\iPhi$, and similarly for $A^2(\iPhi)$. The required check amounts to prove that, given any $T, ¬, \allargs, I$, $\forall {\ileadstoe} \subseteq  \allhist × \Phi: \exists \iPhi \subseteq \Phi \suchthat A^1(\iPhi) \land A^2(\iPhi)$. 

Given a normative theory, define $\Box\Phi \subseteq \Phi$ as 
$\Box\Phi = \set{\phi \in \Phi \suchthat \exists \ar \in \allargs \suchthat \ar \ileadstosts \phi}$.
...
For any decidable proposition: $\phi \in \Box\Phi \oplus ¬\phi \in \Box\Phi$, where $\oplus$ denotes the exclusive disjunction; equivalently: $\phi \in \Box\Phi ⇔ ¬\phi \notin \Box\Phi$. This is proven below, and shows that \cref{ax:nocontr,ax:norm} are satisfiable together (suffices to define $\iPhi = \Box\Phi$). Also proven below is that, for any decidable proposition: $\phi \in \iPhi \oplus ¬\phi \in \iPhi$, which ensures an unambiguous interpretation of the content of $\iPhi$ for those propositions whose status is known thanks to the model.
...
\end{proposition}
\begin{proof}
	Protocol coherence is satisfied by definition of $\ileadstost$. Indeed, if for some $\ar \in \allargs$, $\ar \ileadstost \phi$, then $\forall \ar_1 \in \allargs: (\ar_1, \ar) \nileadstoe ¬\phi$; thus, for any $\ar' \in \allargs$, $\ar' \ileadstost ¬\phi$ is false, because it requires that $\forall \hist \in \allhist, \ar_1 \in \allargs: (\hist, \ar', \ar_1) \ileadstoe ¬\phi$, and thus in particular, that $(\ar', \ar) \ileadstoe ¬\phi$.
\end{proof}
...
\end{proposition}
\begin{proof}
	Consider $\phi \suchthat [\phi \in \Box\Phi \lor ¬\phi \in \Box\Phi]$. Proving the claim only requires to show that $\phi \in \Box\Phi ⇒ ¬\phi \notin \Box\Phi$. By definition, $\phi \in \Box\Phi$ implies that $\exists \ar \suchthat \ar \ileadstost \phi$, and the implication is thus proven by using \cref{prop:protcoh}: $\phi \in \Box\Phi ⇒ [\exists \ar \suchthat \ar \ileadstost \phi] ⇒ ¬[\exists \ar \suchthat \ar \ileadstost ¬\phi] ⇒ \phi \notin \Box\Phi$.
\end{proof}
...
Define the crossing of $\args_{j-2}, \argsd_j \subseteq \allargs$ as $\args_j = \set{\ar_{j-2} + \ard_j \suchthat \ar_{j-2} \in \args_{j-2}, \ard_j \in {\gbeatsmt}(\ar_{j-2}) \cap \argsd_j}$.
\begin{definition}[Convincingness (outdated)]
	$\forall i \in I, \phi \in {\gleadsto}(\allargs), \exists k \in 2\N$ and finite sequences $(\args_j)_{j \in 2\N, j ≤ k}, (\argsd_j)_{j \in 2\N^*, j ≤ k}$ such that:
	\begin{itemize}
		\item $\emptyset ≠ \args_0 \subseteq {\gleadstoinv}(\phi)$;
		\item $\forall j \in 2\N^*, j ≤ k: \argsd_j \subseteq {\gbeatsmt}(\args_{j-2})$;
		\item $\forall j \in 2\N^*, j ≤ k: \args_j$ is the crossing of $\args_{j-2}, \argsd_j$;
		\item $\forall j \in 2\N, j ≤ k, \forall \ar_j \in S_j: \ar_j$ $i$-defends $\phi$ under d-protection of $\argsd_{j + 2}$ (defining $\argsd_{k + 2} = \emptyset$).
	\end{itemize}
\end{definition}

Now prove that Convincingness is the same thing as requiring that the elements be selected exactly when $\ar_1$ attacks $\ar_0$ (with single-try models).
