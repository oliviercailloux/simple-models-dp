...
A claim about types of individuals is a set $C^\text{types} \wsub P^{\allhists}$. Given such a set, define the associated claim $C \wsub \allleadstos$ as $C = (C^\text{types})^I = \set{{\allleadstog} \in \allleadstos \suchthat \forall i \in I: {\ileadstog} \in C^\text{types}}$.
\begin{lemma}
  \label{th:typesfalsifiable}
  A claim about types of individuals $C^\text{types} \wsub P^{\allhists}$ 
  satisfies $\forall {\ileadstog} \in P^{\allhists} \setminus C^\text{types}: \exists H \in \finite(\incr(\allhists)) \suchthat \forall {\ileadstog[j]} \in C^\text{types}: \exists \hist \in H \suchthat {\ileadstog}(\hist) \neq {\ileadstog[j]}(\hist)$,
...
\begin{proof}
  ...
  Pick any $H$ such that $\forall {\ileadstog[j]} \in C^\text{types}: \exists \hist \in H \suchthat {\ileadstog}(\hist) \neq {\ileadstog[j]}(\hist)$.
  Define $\theta = \set{(\hist, i, {\ileadstog}(\hist)) \suchthat \hist \in H} \in \Theta \cap \powerset{\allleadstog}$.
  Then, $\theta \notin C^\wsub$, because $\forall {\ileadstog[j]} \in C^\text{types}: \theta \nsubseteq {\ileadstog[j]}$. 

  Conversely, assume that for some ${\ileadstog} \in P^{\allhists} \setminus C^\text{types}$, no $H \in \finite(\incr(\allhists))$ satisfies the indicated condition.
  Define ${\allleadstog}$ as including ${\ileadstog}$ and for all other individuals, relations from $C^\text{types}$.
  Then, ${\allleadstog} \notin C$, but any observation will be in $C$, thus the claim is not falsifiable and not trivial.
\end{proof}
\begin{lemma}
  \label{th:typesholds}
  Consider a claim about types of individuals $C^\text{types} \wsub P^{\allhists}$. Its associated claim holds iff $\set{{\ileadsto[j]} \suchthat j \in I} \wsub C^\text{types}$.
\end{lemma}
\begin{lemma}
  \label{th:typesanonymous}
  Given any claim about types of individuals $C^\text{types} \wsub P^{\allhists}$, its associated claim is anonymous.
\end{lemma}
...
\begin{theorem}
  ...
  Then, the claim $C$ associated to the claim about types of individuals $C^\text{types} = \set{{\ileadsto[j]} \suchthat j \in I}$ holds, is anonymous, and is falsifiable or trivial.
\end{theorem}
\begin{proof}
...
\begin{remark}
  The opposite of \cref{th:suffFals} is the negation of $\forall {\ileadstog} \in P^{\allhists} \setminus \set{{\ileadsto[k]} \suchthat k \in I}: \exists H \in \finite(\incr(\allhists)) \suchthat \forall j \in I: \exists \hist \in H \suchthat {\ileadstog}(\hist) \neq {\ileadsto[j]}(\hist)$.
  ...
  TODO
  In that case, if restricting our focus to claims about types of individuals, we need to include the real types, then close under suitable indistinguishability relation, so as to include for example $i$.

  Define $S$ as the relations in the claim. Define $R(S)$ as the adequate closure: those that we can’t reject.
  Does $S$ include $R(S)$? If so, it is falsifiable. Otherwise, there is in $R(S)$ a relation that we can’t reject.
  Let $Impl$ be all impliciations in $S$, of the form: if $\hist$ goes to $p$, then $\hist'$ goes to $p'$ (in the above example: if $a$ goes to $\phi$ then $aa$ goes to $\phi$). Any relation that breaks one of these implications is rejectable. For example, sending $a$ to $\phi$ and $ab$ to $¬\phi$ is rejectable.We need to include all relations that break no implication.
  TODO check all this. 
\end{remark}
\begin{definition}
  Say that an attitude $g \in P^{\allhists}$ is indistinguishable from a set of attitudes $S \wsub P^{\allhists}$ iff $\forall H \in \finite(\incr(\allhists)): \exists f \in S \suchthat \restr{g}{H} = \restr{f}{H}$.

  Define the extension of a claim about types $C^\text{types} \wsub P^{\allhists}$ to its indistiguishible attitudes as the the claim about types $R(C^\text{types}) = \set{g \in P^{\allhists} \suchthat \forall H \in \finite(\incr(\allhists)): \exists f \in C^\text{types} \suchthat \restr{g}{H} = \restr{f}{H}}$.
  Note that $C^\text{types} \wsub R(C^\text{types})$. 
  Also note that $S_1 \wsub S_2 \implies R(S_1) \wsub R(S_2)$.

  Define the closure under indistinguishability of a claim about types $C^\text{types}$ as $R^\infty(C^\text{types})$, equivalently, as the smallest set $C^\text{types}_\text{ind}$ such that $R(C^\text{types}_\text{ind}) \wsub C^\text{types}_\text{ind}$.
\end{definition}
The following result follows from \cref{th:typesfalsifiable}.
\begin{theorem}
  If a claim about types of individuals $C^\text{types} \wsub P^{\allhists}$ is not closed under indistinguishability, then it is not falsifiable and not trivial.
\end{theorem}
\begin{theorem}
  Given any situation, if the claim related to the claim about types of individuals $R^\infty({\allleadsto}(I))$ is trivial, then there exists no claim related to a claim about types of individuals that holds and is falsifiable.
\end{theorem}
\begin{proof}
  Consider a claim related to a claim about types of individuals $S$ that holds and that is falsifiable or trivial.
  For it to hold, we need ${\allleadsto}(I) \wsub S$.
  From \cref{th:typesfalsifiable}, for it to be falsifiable or trivial, we need that $\forall g \in P^{\allhists} \setminus S: \exists H \in \finite(\incr(\allhists)) \suchthat \forall f \in S: \restr{g}{H} \neq \restr{f}{H}$. In other words, all $g$ must be distinguishable from $S$. In other words, $S$ must be closed under indistinguishability. Thus, we need $R^\infty(S) \wsub S$.
  Also, because $S_1 \wsub S_2 \implies R(S_1) \wsub R(S_2)$, we need
  $R^\infty({\allleadsto}(I)) \wsub R^\infty(S) \wsub S$.
  It follows that $S$ is trivial.
\end{proof}

\begin{conjecture}
  Given any situation, the claim about types of individuals $R^\infty({\allleadsto}(I))$ holds, is anonymous, and is falsifiable or trivial.
\end{conjecture}

\begin{theorem}[To be deleted, probably]
  Consider the attitude where all individuals are either constant to $\phi$ except that they map $\emptyset$ to anything, or constant to $¬\phi$ except that they map $\emptyset$ to anything, say, $(\phi, \phi)$, $(¬\phi, \phi)$, $(\phi, ¬\phi)$, $(¬\phi, ¬\phi)$.
  Consider a claim about types of individuals containing all those ones. Assume that for some attitude $i$ not in the claim, no $H$ permit to distinguish $i$ from the claim. Then from the lemma above, the claim is not falsifiable. So, we need to include all attitudes that no $H$ permit to distinguish from the claim. Iterating, this is the closure under the indistinguishability relation.
  So, the claim about types must include the real ones and be closed under the indistinguishability relation.
\end{theorem}
\begin{theorem}
