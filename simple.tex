...
Given $\phi \in {\gleadsto}(\allargs)$, define ${\ibeatse[!\phi]} \subseteq \allargs × \allargs$ such that $\ar' \ibeatse[!\phi] \ar$ iff $(\ar', \ar) \ileadstoe ¬\phi$. 
...
\commentOC{TODO next: check $\gbeatsstar$ can be replaced. Then, comment that Bounded length does not imply finiteness of arguments used as counter-arguments by the model: model is able to counter infinitely many arguments with infinitely many counter-args, but needs to declare that an (unknown) finite subset of those will suffice to convince $i$.}
