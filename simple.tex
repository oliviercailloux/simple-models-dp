...
A claim about types of individuals is a set $C^\text{types} \wsub P^{\allhists}$. Given such a set, define the associated claim $C \wsub \allleadstos$ as $C = \set{{\allleadstog} \in \allleadstos \suchthat \forall i \in I: {\ileadstog} \in C^\text{types}}$.
\begin{lemma}
...
\begin{proof}
  ...
  Define ${\allleadstog}$ as including ${\ileadstog}$ and for all other individiduals, relations from $C^\text{types}$.
...
\begin{theorem}
  ...
  Then, the claim $C$ associated to the claim about types of individuals $C^\text{types} = \set{{\ileadsto[j]} \suchthat j \in I}$ holds and is anonymous and falsifiable.
\end{theorem}
\begin{proof}
