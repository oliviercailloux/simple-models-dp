...
Thus, given an argumentative path $\hist \in \allhist$, $\forall k \in \intvl{1, \card{\hist}}: \hist(k) = \hist_k \in \allargs$, and $\hist = \emptyset ⇔ \card{\hist} = 0$.
...
Say that $\gamma$ has depth at least $k \in \N^*$ iff $\exists (\alpha_i, 1 ≤ i ≤ k) \suchthat \forall 1 ≤ i ≤ k, d(\alpha_i) ≠ \emptyset \land \forall 1 ≤ i ≤ k - 1, \alpha_i \cup \set{d(\alpha_i)} \subseteq \alpha_{i + 1}$.
...
For example, $\gamma$ has depth $0$ iff $\dg = \emptyset$; and $\gamma$ has depth $1$ iff $\exists \alpha_1 \suchthat d(\alpha_1) ≠ \emptyset \land \forall \alpha_1 \cup \set{d(\alpha_1)} \subseteq \alpha_2: d(\alpha_2) = \emptyset$.

Given $\ar \in \allargs$, define $\dlg(\ar) = \dg(\set{\hist \in \allhist \suchthat \ar \in R(\hist)}) = \set{\dg(\hist) \suchthat \hist \in \allhist \land \ar \in R(\hist)} \subseteq \allargs$ as all the defenders of $\ar$.
Given $\args \subseteq \allargs$, define $\dlg(\args)$ as $\set{\dlg(\ar) \suchthat \ar \in \args}$.
This is imperfect as $\dlg(\emptyset)$ should include the stuff that $\dg$ answers to when its own arguments are not involved.
This permits to define defenders of defenders of an argument, $\dlg(\dlg(\ar))$, defenders of defenders of defenders of an argument, … 

This yields the following notion of depth.
...
$\exists k \in \N \suchthat (\dlg)^k(\dg(\emptyset)) = \emptyset$.
The smallest such $k$ is called the depth of the theory.
For example, $γ$ has depth $0$ iff $\dg(\emptyset) = \emptyset$.
TODO that seems wrong: could be hist to which the theory wishes to reply.
