...
Given an exposure protocol $(I, \allargs, {\allleadsto})$, we say that a claim $C$ \emph{holds} iff $C \ni {\allleadsto}$. We are interested in procedures to determine whether a claim holds.
...
$θ$-verifiability mandates that some (possibly counter-factual) observation at least as strong as $θ$ proves the claim (makes sure that it holds), thus, $C$ is $θ$-verifiable iff $\exists θ ⊆ θ' \in \Theta \suchthat \forall θ' \wsub {\allleadstog}: {\allleadstog} \in C$.

Factual verifiability mandates that some feasible observation proves the claim (makes sure that it holds), thus, $C$ is factually verifiable iff $\exists \theta \in \Theta \cap \powerset{\allleadsto} \suchthat \forall \theta \wsub {\allleadstog}: {\allleadstog} \in C$.
...
Given $\theta \in \Theta$, a claim $C$ is $θ$-\emph{indicative} iff $∃ i \in I \cap \powerset{\allleadsto} \suchthat \pi(\set{{\allleadstog} \suchthat \theta \wsub {\allleadstog}} \cap C) ≠ (P \cup \set{\emptyset})^I$. 
A claim is \emph{indicative} iff $\exists \theta \in \Theta$ such that $C$ is $\theta$-indicative.
Indicativity mandates that some feasible observation permits to deduce something about DJ’s.

\begin{remark}
  Informativeness, and indicativeness, permit mere counting: the claim that exactly two persons deliberately believe $\phi$ is informative.
  ...
  Note also that 
  strong informativeness beyond counting implies informativeness beyond counting and indicativeness; 
  and that informativeness beyond counting implies informativeness.
\end{remark}
\begin{remark}
  A claim $C$ is \emph{possibly indicative} iff $\forall {\allleadstog} \in C: ∃ i \in I, \theta \in \Theta \cap (\allhists × \set{i} × P) \wsub {\allleadstog} \suchthat \emptyset ≠ \pi(\set{{\allleadstog} \suchthat \theta \wsub {\allleadstog}} \cap C)(i) \ssub P$.
  Possible indicativity mandates that for each (possibly counter-factual) world in $C$, when knowing a finite number of observations about a given $i$ compatible with it, $C$ permits to deduce something (different than $\emptyset$) about $P_i$.
  The claim $C = \set{\suchthat (a) \ileadsto φ ∨ ¬φ \in π(\allleadstog)(i)}$ (that if path $(a)$ does not lead a given $i$ to $φ$, then $¬φ \in P_i$) is possibly indicative, and if actually $(a) \nileadsto φ$ (thus $C$ holds), then the claim is not effectively indicative, as it does not entail that $¬φ \in P_i$: it is only counterfactually indicative, as it would entail that $¬φ \in P_i$ if $(a) \ileadsto φ$. And it is verifiable: checking that path $(a)$ leads $i$ to $φ$ proves that the implication holds vacuously.
\end{remark}
\begin{remark}
  All of this is more complicated than required.
  Informativeness suffices, as the claim $C = \set{∃ i \suchthat (a) \nileadstog φ ∧ [(a) \nileadstog φ ⇒ ¬φ \in π(\allleadstog)(i)]}$ is informative.
\end{remark}
...
\begin{theorem}
  If $\card{\allargs} ∉ ℕ$, any indicative claim is not factually verifiable.
\end{theorem}
\begin{proof}
  Consider any $θ ∈ Θ \cap \powerset{\allleadsto}$.
  By indicativeness, $∃ \theta' \in \Theta \cap \powerset{\allleadsto}, f: I \rightarrow (P \cup \set{\emptyset}) \suchthat f ∉ \pi(\set{{\allleadstog} \suchthat \theta' \wsub {\allleadstog}} \cap C)$.
  
  Apply the lemma to $θ ∪ θ'$.

  It follows that $π({\allleadstog}) = f$, thus ${\allleadstog} ∉ C$, which contradicts factual verifiability.
\end{proof}

\begin{theorem}
  If $\card{\allargs} ∉ ℕ$, any $θ$-indicative claim is not $θ$-verifiable.
\end{theorem}
\begin{proof}
  
\end{proof}

The fundamental reason that this theorem holds is that any (combined) feasible observation can be completed in a way that contradicts any conclusion that indicativeness mandates.

\begin{remark}
  The theorem ceases to hold if $(P \cup \set{\emptyset})^I$ replaces $P^I$ in the definition of indicativeness, but is restaured under infiniteness of $\allargs$.
\end{remark}
...
A claim is $D$-verifiable iff $\exists \theta \in \Theta \cap \powerset{\allleadsto} \suchthat \forall \theta \wsub {\allleadstog} \in D: {\allleadstog} \in C$.
