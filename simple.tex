...
\commentOC[Let also $\max\incr(\allhists) = \set{A \in \incr(\allhists) \suchthat \nexists A' \in \incr(\allhists) \suchthat A \subset A'}$ being equal to $\set{A \in \incr(\allhists) \cap (\allhists)^\Z}$ denote the maximal elements of $\incr(\allhists)$, in the sense of containing sequences that are not a subsequence of another sequence; equivalently, containing those sequences whose sequence of cardinals of their elements equals $\Z$.]{To be deleted.}
...
A claim $C$ is \emph{indicative} iff $\exists i \in I, \theta \in \Theta \cap (\allhists × \set{i} × P) \cap C^\wsub \suchthat \emptyset ≠ \pi(\set{{\allleadstog} \suchthat \theta \wsub {\allleadstog}} \cap C)(i) \ssub P$. Indicativity mandates that when knowing a finite number of observations about a given $i$ compatible with any (possibly counter-factual) world in $C$, $C$ permits to deduce something (different than $\emptyset$) about $P_i$.
A claim $C$ is \emph{truly indicative} iff $\exists i \in I, \theta \in \Theta \cap (\allhists × \set{i} × P) \wsub {\allleadsto} \suchthat \emptyset ≠ \pi(\set{{\allleadstog} \suchthat \theta \wsub {\allleadstog}} \cap C)(i) \ssub P$.
A claim $C$ is \emph{newly indicative} iff $\forall {\allleadstog} \in C: \exists i \in I, \theta \in \Theta \cap (\allhists × \set{i} × P) \wsub {\allleadstog} \suchthat \emptyset ≠ \pi(\set{{\allleadstog} \suchthat \theta \wsub {\allleadstog}} \cap C)(i) \ssub P$. Indicativity mandates that when knowing a finite number of observations about a given $i$ compatible with any (possibly counter-factual) world in $C$, $C$ permits to deduce something (different than $\emptyset$) about $P_i$.
...
\begin{theorem}
  Indicativeness does not imply true indicativeness, even when considering only anonymous and falsifiable claims that hold.
\end{theorem}
\begin{proof}
  Consider the situation where everybody is constant to $\phi$ except that emptyset to $¬\phi$.
  The claim “C HA or everybody is constant to $\phi$” is anonymous and falsifiable and indicative and holds, but is not truly indicative.
  Thus, the claim is “(everybody satisfies emptyset to $\phi$ or everybody satisfies emptyset to $¬\phi$) and (everybody satisfies every path but emptyset to $\phi$)”.
  It is falsifiable: any behavior function not in the claim satisfies “(somebody fails emptyset to $\phi$ and somebody fails emptyset to $¬\phi$) or (somebody fails every path but emptyset to $\phi$)”, which is observable in either case.
  It is indicative, as the behavior function “everybody constant to $\phi$” is what remains in the claim if assuming observing someone sending $\emptyset$ to $\phi$.
  
  But it is not truly indicative: this would require some truthful observation that leaves only a subset of the claim with no $\pi({\allleadstog})(i) = \emptyset$, which requires excluding “everybody satisfies emptyset to $¬\phi$”.
\end{proof}

Let us determine which situations admit anonymous and falsifiable claims that hold and are truly indicative (or other?).
...
\begin{theorem}
  If an anonymous claim holds and is truly indicative, then $\exists i \in I \suchthat P_i ≠ \emptyset$.
\end{theorem}
\begin{proof}
  By anon and holding, we have $C^\text{HA} \wsub C$.
  By true indicativeness, for some $i$ and $\hist$ and $f \wsub {\ileadsto}$ on that $\intvl{\hist}$, all and some completion of $f$ in $C$ lead to $\pi({\allleadstog})(i) ≠ \emptyset$, thus, in particular, 
  ${\allleadsto}$ leads to $\pi({\allleadsto})(i) ≠ \emptyset$, 
  whence 
  $\exists i \in I \suchthat P_i ≠ \emptyset$.
\end{proof}
...
\begin{conjecture}
  If $P_{i_1} = \phi$ with a single decisive argument $\ar \ileadstost \phi$ and $i_2$ satisfies ${\ileadsto[i_2]} = {\ileadsto[i_1]}$ except that $(\ar) \ileadsto[i_2] ¬\phi$, then there is no anonymous and truly indicative claim that holds.
\end{conjecture}
\begin{proof}
  True indicativeness requires that
