...
Classical descriptive theories of choice in economics are limited to shallow observations.

This article defines theories that are prescriptive and empirical.
They are prescriptive in that the claim $a \succ b$ has the semantics of recommending choosing $a$ rather than $b$ when both are available.
They are empirical in that the validity of their recommendations rests on the basis of a mild axiom (compared to usual normative theories such as Savage’s Expected Utility theory) together with empirical claims, thus, claims that observations can falsify.
...
\section{The model}
Let $\Phi = \set{\phi, ¬\phi}$ represent the decision at stake. Let $I$ be a set of individuals. 

A prescriptive theory is a tuple $(\allargs, {\allleadsto})$.

$\allargs$ is a set of arguments. 
Throughout the article, $\N$ includes zero. %and $\N^* = \N \setminus \set{0}$. 
Define $\allhist = \bigcup_{k \in \N} \allargs^{\intvl{1, k}}$ as the finite sequences of arguments. A sequence of arguments is called an argumentative path, or simply a path.
...
The function ${\allleadsto}: I × \allhist → \powersetz{\Phi}$ associates to each individual and path a non-empty subset of $\Phi$. The case $\Phi$ is used to represent indifferent choice, in case this is observable.
The observation that $\phi$ is considered among the best by $i$ considering $\alpha$ is represented by $\alpha \ileadsto \phi$, considering ${\ileadsto} \subseteq \allhist × \Phi$ as a binary relation.
As usual with binary relations, $\hist \nileadsto \prop$ means $(\hist, \prop) \notin {\ileadsto}$.

% which associates to each $i \in I$ an element of $\Phi$ or nothing, with the semantics that this decision is the right one for that individual or that the theory abstains from recommending.
...
\begin{example}[Two disagreeing theories]
	...
	Consider a set $\alts = \set{\alt_1, \alt_2} \subset \allalts$ of two items, and assume the two theories of \ac{DJ} are opposed on their prediction of the reflexive choice of individuals having to choose one item among $\alts$. Say, $\alt_1$ is satisfying and $\alt_2$ is not, failing on the speed criterion; while $v(\alt_2) > v(\alt_1)$.
