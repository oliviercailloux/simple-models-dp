...
Verifiability mandates that some feasible observation proves the claim (makes sure that it holds), thus, $C$ is verifiable iff $\exists \theta \in \Theta \cap \powerset{\allleadsto} \suchthat \forall \theta \wsub {\allleadstog}: {\allleadstog} \in C$.
...
A claim $C$ is \emph{indicative} iff $∃ i \in I, \theta \in \Theta \cap \powerset{\allleadsto} \suchthat \emptyset ≠ \pi(\set{{\allleadstog} \suchthat \theta \wsub {\allleadstog}} \cap C)(i) \ssub P$. Indicativity mandates that some feasible observation permits to deduce something (different than $\emptyset$) about some individual’s DJ.
...
\begin{theorem}
  An indicative claim is not verifiable.
\end{theorem}
\begin{proof}
  Consider any $θ \wsub {\allleadsto}$.
  By indicativeness, $∃ i \in I, \theta' \in \Theta \cap \powerset{\allleadsto} \suchthat \emptyset ≠ P' = \pi(\set{{\allleadstog} \suchthat \theta' \wsub {\allleadstog}} \cap C)(i) \ssub P$.
  Pick any $p ∉ P'$ and any ${\allleadstog}$ that extends both $θ$ and $θ'$ and such that $∀ α \suchthat k < \card{α}: α \ileadsto p$.
  It follows that ${\allleadstog} ∉ C$, which contradicts verifiability.
\end{proof}
The fundamental reason that this theorem holds is that we can, given any (combined) feasible observation, still build a completion that contradicts any conclusion that indicativeness mandates.
