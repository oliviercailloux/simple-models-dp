...
\end{remark}
  ...
  \begin{theorem}
    \label{th:suffIndic}
    Assume that for some $H \in \finite(\incr(\allhists))$ and $i_1 \in I \suchthat P_{i_1} \in P$, we have $\forall i_2 \suchthat P_{i_2} \neq P_{i_1}$ that $\exists \hist \in H \suchthat {\allleadsto}(\hist, i_1) \neq {\allleadsto}(\hist, i_2)$.
  ...
  \begin{proof}
    ...
    The claim is indicative for $i = i_1$ and $\theta = \set{(\hist, i_1, {\allleadsto}(\hist, i_1)) \suchthat \hist \in H}$ for the following reason.
...
\begin{conjecture}
  The converse of \cref{th:suffIndic} holds.
\end{conjecture}
\begin{proof}
  HA requires that the claim includes at least $C^\text{HA}$.
  Indicativenes requires a $\theta$ that keeps a single type $i_1$ and rejects all $i_2$ (those with a deliberated judgment different from $i_1$).
  Can we build a suitable $H$ with that $\theta$?
  The question is: can we have $\theta$ about two individuals but still single out a single type?
  Yes: claim is that all individuals reason in the same way about a given $\hist$.
  In such a case, the observations about $i_2$, $i_3$ permit to know that the world is such that my observations about $i_1$ (if any) permit to single it out. 
  In such a case, I conjecture that as the claim holds, adding those observations about $i_2$, $i_3$, we obtain a set of possible worlds which contain the real one; out of this we can build $H$ using the observations in $\theta$.
  In other words, from $\theta$, consider only the observations about $i_1$ and define $H$ using those. Prove that this singles out $i_1$, from the fact that the world conforms to the remaining observations (about $i_2$, $i_3$) in $\theta$.
\end{proof}
