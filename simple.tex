...
\begin{example}
	$(\ar) \ileadstog \phi \lor  (\ar, \ar) \ileadstog \phi \lor …$ is not falsifiable.
	Given any $O$, define $l = \max_{\hist \suchthat \exists p \in P \suchthat (i, \hist, p) \in O} \card{\hist} + 1$ if $\exists p \in P \suchthat (i, \hist, p) \in O$, and $l = 1$ otherwise. Pick any $\allleadstog$ such that $O \subset {\allleadstog} \land (\ar)^l \ileadstog \phi$. We have ${\allleadstog} \in S_O \cap C ≠ \emptyset$.
\end{example}
\begin{example}
	\label{ex:unfals}
	$(\ar) \ileadstog \phi \lor (\ar') \ileadstog \phi$ is not falsifiable.
...
\end{remark}
\begin{theorem}
	The claim \eqref{eq:cl} and not \eqref{eq:cln}, thus, \eqref{eq:cl} and $\forall i, \ar_k \ileadstostg p \lor \ar_l \ileadstostg p$ is not falsifiable.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Formal]
	...
	\begin{multline}
		\label{eq:answerable}
		\forall i \in I, \exists \set{\ar_1, \ar_2} \suchthat
		\set{\ar_1, \ar_2} = \set{\ar_k, \ar_l} \\ \land
		\forall \hist \in (…, \ar_1, ?) \cap \nileadstoinv(p): 
		(\hist, …, \ar_2, ?) \nsubseteq \nileadstoinv(p).
	\end{multline}
